...
class Select extends BaseComponent {
    ...
    invokeBehavior(behavior, data) {
        ...
        case 'getSelectedValue':
            return $(`#${this.getComponentId()}`).dropdown('get value');
        case 'getCheckedValue':
            return (() => {
                let val = [];
                $(`#${this.getComponentId()} input[name=${this.data['@title']}]:checked`).each((i) => {
                    val[i] = $(':checked');
                });
                if (val.length > 0) {
    ...
    render() {
        ...
        if (this.getDisplayStyle() === 'select') {
            ...
            defaultOption.textContent = jsonData['@placeholderValue'];
            ...
            if (dataValues.length > 0) {
                dataValues.forEach((data) => {
                    ...
                    if (data['@slider'] && this.getDisplayStyle() === 'checkbox') {
                        innerUiDiv.classList.add(data['@slider']);
                    }
