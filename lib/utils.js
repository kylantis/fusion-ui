...
module.exports.deepClone = (o, preserveReferences = false) => {

  const objectReferences = {};

  const storeObjectReferences = (key, val) => {
    if (val && val.constructor.name === 'Object') {
      if (val[this.objectReferenceKey]) {
        assert(typeof val[this.objectReferenceKey] == 'string')
        objectReferences[val[this.objectReferenceKey]] = val;
      }
    }
    return val;
  }

  const jsonString = preserveReferences ?
    JSON.stringify(o, storeObjectReferences, null)
    : JSON.stringify(o);

  const restoreObjectReferences = (key, val) => {
    if (val && val.constructor.name === 'Object') {
      if (val[this.objectReferenceKey]) {
        return objectReferences[val[this.objectReferenceKey]];
      }
    }
    return val;
  }

  return preserveReferences ? JSON.parse(jsonString, restoreObjectReferences) : JSON.parse(jsonString);
}
