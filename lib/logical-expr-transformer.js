    ...
    addLogicGate({ stmt }) {
        ...
        const accept = (stmt) => {
            ...
            if (stmt.ternaryPath) {
                ...
                const addParticipant = (stmt) => {
                    participants.push(stmt);
                    // Todo: If <isLogicGateParticipant> is not used, remove
                    stmt.isLogicGateParticipant = true;
                }
                    ...
                    .forEach(stmt => {
                        if (stmt.type == B_EXPR) {
                            if (isDataPath(stmt.left)) {
                                addParticipant(stmt.left)
                            }
                            if (isDataPath(stmt.right)) {
                                addParticipant(stmt.right)
                            }
