...
class TemplatePreprocessor {
  ...
  static outerTransformHashKey = 'outerTransform';

  static innerTransformHashKey = 'innerTransform';
  ...
  static getDefaultHelpers() {
    const {
      loadInlineComponentHelperName, ternaryHelperName, logicalHelperName, concatenateHelperName,
      noOpHelperName, fnHelperName,
    } = TemplatePreprocessor;
  ...
  loadCompiledComponentClasses() {
    const { getComponentsDistPath, getComponentListRelativePath } = TemplatePreprocessor;
    ...
    Object.entries(this.metadata.distComponentList)
      .forEach(([className, assetId]) => {
        ...
        RootProxy.getGlobalSchemasObject()[className] = utils.deepClone(this.getConfig(className).schema);
  ...
  getSerializedComponent(className, loadable) {
    if (this.resolver.processing) {
      ...
      if (!componentInstances[className]) {
        componentInstances[className] = this.getSerializedComponent0(className, loadable);
      }
      return componentInstances[className];
    } else {
      return this.getSerializedComponent0(className, loadable);
    }
  }
   ...
   */
  getSerializedComponent0(className, loadable) {
    ...
    return new testComponentClass({ input: sample, config: { loadable: false, serialization: { loadable, } } });
  }
  ...
  parseComponentClass({
    ...
    try {
      ...
      this.logger.error(`Syntax error detected in file: ${filePath}`);
      throwError(e);
    }
  ...
  addDependencyAsset({ stmt, type }) {
    ...
    const predicateValue = getHashValue({ stmt, key: 'predicate' });
    ...
    validate('predicate', predicateValue)

    let { original: url } = urlValue;
    let { original: namespace } = namespaceValue || {};
    let { original: predicate } = predicateValue || {};
    ...
    if (withProvisionalHost) {
      url = url.replace(provisionalHost, '');
    }

    const dep = { url };

    if (predicate) {
      dep.predicate = predicate;
    }
    ...
    this[`${type}Dependencies`].push(dep);
  }
  ...
  emitHelpers() {
    this.emitMethodReturningArray({ name: 'helpers', canMergeWithParent: false });
  }
  ...
  emitAllowedPaths() {
    this.emitMethodReturningArray({ name: 'allowedPaths', canMergeWithParent: false });
  }
  ...
  getSampleValueForType(type) {
    ...
    switch (type) {
      ...
      case componentRefType:
        return this.getSerializedComponent(BaseComponent.name, false);
    }
  }
  ...
  resolvePathFromContext({
    ...
    const {
      ...
      objectType, appendSuffix, hasObjectPrefix, defaultIndexResolver, getTargetType, getLine, getSuffix, isResolvableInScope,
      throwCannotAccessIterateContextInAsync, getSampleValueForLiteralType, getOuterInlineBlock,
    } = TemplatePreprocessor;
    ...
    switch (true) {
      case synthetic:
        ...
        value = this.getSyntheticMethodValue({
          ...
          method: original
            // If this methodName was generated inside a synthetic block, _@ will be appended
            .replace(/_@$/g, ''),
          validType,
          line,
        });
  ...
  static throwError(err, stmt) {
    ...
    if (typeof err == 'string') {
      throw new TemplateError(`${stmt ? `[${getLine(stmt)}] ` : ''}${err}`);
    } else {
      throw err;
    }
  }
  ...
  addDataBindTransformations(ast) {
    ...
    const {
      ...
      createContentStatement, createMustacheStatement, throwError,
    } = TemplatePreprocessor;
    ...
    const onMustacheStatement = (stmt) => {
      ...
      const { path, params, dataBinding, createHtmlWrapper, forceHtmlWrapper } = stmt;
    ...
    const onBlockStatement = (stmt) => {
      const { createHtmlWrapper, forceHtmlWrapper } = stmt;

      if (createHtmlWrapper && canCreateBlockHtmlWrapper()) {
  ...
  pruneStatements(ast) {
    ...
    ASTParser.prototype = new handlebars.Visitor();
  ...
  /**
   * This method is called when data binding is disabled for the entire component. It visits the ast to
   * check for blocks and mustache statements that need HTML wrappers. If if finds any, an error is thrown
   */
  ...
  process0({ contextList, bindParents, program }) {
    ...
    const getOuterTransform = ({ stmt }) => {
      const { outerTransformHashKey } = TemplatePreprocessor;
      return this.getMethodNameHashValue({ stmt, key: outerTransformHashKey, allowMultiple: true });
    };

    const getInnerTransform = ({ stmt }) => {
      const { innerTransformHashKey } = TemplatePreprocessor;
      return this.getMethodNameHashValue({ stmt, key: innerTransformHashKey, allowMultiple: true });
    };

    const getTransform = ({ stmt }) => {
      const { transformHashKey } = TemplatePreprocessor;
      return this.getMethodNameHashValue({ stmt, key: transformHashKey });
    };

    const getHook = ({ stmt, allowHookOrder = false, allowHookPhase = false }) => {
      const { hookHashKey, hookOrderHashKey, hookPhaseHashKey } = TemplatePreprocessor;

      const { original: hookOrder } = getHashValue({
        stmt, key: hookOrderHashKey, type: 'NumberLiteral', cleanup: false,
      }) || {};;

      if (hookOrder && !allowHookOrder) {
        throwError(`hookOrder cannot be specified here`, stmt);
      }
      ...
      if (hookPhase) {
        ...
        if (!allowHookPhase) {
          throwError(`hookPhase cannot be specified here`, stmt);
        }

        const knownHookPhases = RootCtxRenderer.getSupportedHookPhases();

        hookPhase.trim().split(/\s*,\s*/g).forEach(s => {
          if (!knownHookPhases.includes(s)) {
    ...
    const acceptPathExpressionInCustomCtx = ({ stmt, visitor }) => {
      ...
      if (globalVariable) {
        ...
        original = invokeDataPathTransform(globalVariable.original, stmt);
    ...
    const visitHeaders = function (stmt) {
      ...
      visitNodes({
        ...
        consumer: ({ stmt }) => {
          ...
          if (isRootCtxValue(original) && !_this.allowedPaths.includes(original)) {
            _this.allowedPaths.push(original);
          }
        },
      });
    ...
    ASTParser.prototype.BlockStatement = function (stmt) {
      ...
      const {
        globalsBasePath, wrapInvocationWithProxyMethodName, getPathFromSyntheticAlias, getChildPathFromSyntheticAlias, getTargetType,
        isLookupAllowed, throwError, addBlockParamHashKey, getIterateDataVariables, invokeDataPathTransform, getReservedBlockHashKeys,
        getLine, getBlockSource, createPathExpression, createStringLiteral,
      } = TemplatePreprocessor;

      const {
        blockName, validType, contextSwitching, scopeQualifier,
        indexQualifier, canIterate, conditional, custom,
      } = stmt.processed ? {} : _this.getBlockOptions(stmt);


      const innerTransform = getInnerTransform({ stmt });
      const outerTransform = getOuterTransform({ stmt });

      if (!stmt.processed) {
        if (getPredicate({ stmt }) && !canIterate) {
          throwError(`A predicate cannot be specified in #${blockName} block`, stmt);
        }
      ...
      const hook = getHook({ stmt, allowHookPhase: true, allowHookOrder: true });
      ...
      if (innerTransform || outerTransform) {
      ...
      const processBlockInRootCtx = () => {
        ...
        if (logicGate && logicGate.participants.length) {
          const logicGatePath = addLogicGate({
            ...stmt,
            logicGate,
            logicGatePruneKey,
          });
          ...
          if (logicGatePath) {
            ...
            _this.allowedPaths.push(`${logicGatePath}!!`);

            isSynthetic = false;

            resetPathExpression({
              stmt: stmt.params[0],
              original: `${toCanonical(logicGatePath)}!!`,
              properties: {
                processed: true,
              },
            });
      ...
      const containsHtmlError = () => {
        ...
        if (htmlErrors.length) {
          ...
          if (htmlWrapperErrors.length) {
            ...
            throwError(`HTML errors were found on #${blockName} block`, stmt);
          }
      ...
      switch (stmt.path.original) {
        ...
        case variableHelperName:
          ...
          (() => {
            ...
            // Before calling addHashesToContext(...), register canonicalSource (if applicable), so that
            // it is saved on the contextObject
            hashPairs
              .forEach(({ value }) => {
      ...
      logicGate.participants
        .forEach((participant) => {
          ...
          if (!participants.includes(_original)) {
            participants.push({ original: _original, loc: { ...loc } });
          }
        });
      ...
      if (isCustomContext()) {
        ...
        const wrapInResolveMustacheHelper = () => {
          ...
          return {
            ...
            fromMustache: true,
            // Note: A HTML wrapper is required for rendering inline components,
            createHtmlWrapper: isInlineComponent,
            forceHtmlWrapper: isInlineComponent,
          };
        }
    ...
    const addHashesToContext = function ({ hashPairs, contextObject, ctxIndex }) {
      const {
        literalPathPrefixRegex, stringifyHandlebarsNode, getLiteralValueFromPathExpression,
      } = TemplatePreprocessor;
    ...
    const getPartial = ({ ctxList, partialName, stmt }) => {
      ...
      const {
        partialBaseDirHashKey, partialBaseLocHashKey, runtimeDecoratorHashKey, eagerDecoratorHashKey,
        variableHelperName, visitNodes, createStringLiteral, getDecoratorParams, createLocSource,
        createAst0, getHashValue, registerProgram, throwError, getBlockSource, setLocSource,
      } = TemplatePreprocessor;
      ...
      const ensureBlockParamsAreAvailable = ({ requiredParams, optionalParams, defaultParams, blockName }) => {
        const { hash } = stmt;
      ...
      (() => {
        ...
        for (const blockName in inlineBlocks) {
          ...
          const { requiredParams, optionalParams, defaultParams, templateSource, eager } = block;

          ensureBlockParamsAreAvailable({ requiredParams, optionalParams, defaultParams, blockName });
          ...
          for (const node0 of classInfo.templateAst.body) {
            ...
            if (node0.type == DECORATOR) {
              ...
              if (decoratorName == partialName) {
                ...
                ensureBlockParamsAreAvailable({ requiredParams, optionalParams, defaultParams, blockName: decoratorName });

                templateSrc = node.program.templateSource;

                node.program.body.unshift(...scopeNodes);
              ...
              if (!node0.loc.source) {
                setLocSource(
                  ...
                  createLocSource({ base: parent, fileName })
                );
              }
    ...
    const processPartial = function ({ stmt }) {
      ...
      const {
        partialDeferHashKey, partialRuntimeHashKey, getSuffix, throwError,
      } = TemplatePreprocessor;
    ...
    ASTParser.prototype.PartialStatement = function (stmt) {
      ...
      const {
        componentRefType, loadInlineComponentHelperName, inlineComponentHashKey, pathHashKey, wordPattern,
        getReservedPartialHashKeys, throwError, createBooleanLiteral, createStringLiteral,
      } = TemplatePreprocessor;
          ...
          ...isComponentClass ? createStringLiteral(partialName) : {
            ...
            onResolve: (original) => {
              ...
              _hash.pairs.push({
                type: 'HashPair',
                key: pathHashKey,
                value: createStringLiteral(original),
                synthetic: true,
              });
            },
            immutable: true,
    ...
    ASTParser.prototype.DecoratorBlock = function (stmt) {
      const {
        ...
        getDecoratorParams, getHashValue, throwError, visitNodes, createMustacheStatement, createPathExpression, getNonEmptyLiteralTypes,
      } = TemplatePreprocessor;
      ...
      if (runtime && !fromMarker) {
        // Runtime decorator blocks are only available at index 0 of the root context because these decorators are executed 
        // as high-level handlebars functions and uses the rootProxy as the context, see RootCtxRenderer.renderDecorator(...)
  ...
  addInlineBlock({
    bindParents, decoratorName, stmt, requiredParams, optionalParams, defaultParams, isCustomCtx, runtime, eager,
  }) {
    const {
  ...
  static getReservedMustacheHashKeys() {
    const { ctxHashKey, inlineComponentHashKey, pathHashKey } = TemplatePreprocessor;
    return [ctxHashKey, inlineComponentHashKey, pathHashKey];
  }
