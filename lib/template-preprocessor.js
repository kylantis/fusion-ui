...
class TemplatePreprocessor {
  ...
  static getComponentsDistPath() {
    return pathLib.join(process.env.PWD, 'dist', 'components');
  }
  ...
  writeAssetsToFileSystem() {
    ...
    this.writeSampleFileToFileSystem();
  }
      ...
      get: (obj, prop) => {
        ...
        if (assetId && enableComponentCaching) {
          ...
          if (srcDirName) {
            ...
            // to processFile(...). The instances will be used later to generate models
  ...
  writeSampleFileToFileSystem() {
    fs.writeFileSync(
      `${this.getDistPath()}/sample.js`,
      `module.exports=${this.sampleString}`,
    );
  }
  ...
  getSerializedComponent(className) {
    ...
    return new testComponentClass({ input: sampleData, loadable: false });
  }
  ...
  process() {
    ...
    if (!this.component) {
      ...
      const { parent, isAbstract } = this.metadata.componentSources[this.className];
  ...
  loadComponentSources() {
    ...
    for (const dirName of utils.getAllComponentNames()) {
      ...
      const parents = getComponentParents(ComponentClass);
  ...
  generateSchemaAndSample() {

    const { getComponentsDistPath, getAssetIdFromClassName } = TemplatePreprocessor;

    let sampleString = global.clientUtils.stringifyComponentData(
      this.resolver.getSample()
    );
    ...
    if (parent) {
      ...
      const parentSampleString = fs.readFileSync(pathLib.join(dir, 'sample.js'), 'utf8');
      ...
      sampleString = sampleString === "{}" ?
        `${parentSampleString.replace('module.exports=', '')}` :
        `${sampleString.replace(/}$/g, '')},${parentSampleString.replace('module.exports={', '')}`;
    }
  ...
  */
  ...
  serializeAst() {
    const { getTransientFields, getComponentClass } = TemplatePreprocessor;
    const fields = {};
    ...
    if (this.resolver.processing) {
      ...
      if (!this.metadata.initialized) {
        ...
        const initFn = getOwnMethod({
          component, name: 'init', className: this.className
        });
