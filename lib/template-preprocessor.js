    ...
    streamTokenizer
      .on('data', (tokens) => {
        ...
        if (startTagEnd >= 0) {
          ...
          const attributeValues = Object.values(attributeList);

          let hasDataPaths = [...attributeKeys, ...attributeValues]
            .filter(v => typeof v === 'string' && v.startsWith('{{')).length

          if (attributeKeys.includes('id') && (attributeList['id'] !== `{${setSyntheticNodeIdHelperName}}`)) {
