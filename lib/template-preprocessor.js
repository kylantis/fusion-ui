...
class TemplatePreprocessor {
  ...
  // If this is updated, it also needs to be updated in app-context.js as well
  ...
  constructor({
    ...
    const {
      reservedComponentClassNames, getMethodNames, getComponentClass,
      createAst, parseScript
    ...
    if (!component) {
      ...
      interfaces = this.validateComponentInterfaces();
  ...
  validateComponentInterfaces() {

    const { getDefinedComponentInterfaces } = TemplatePreprocessor;
    const { interfaces } = this.resolver.loadConfig0();

    const knownInterfaces = Object.keys(getDefinedComponentInterfaces());

    interfaces.forEach(i => {
      if (!knownInterfaces.includes(i)) {
        throw Error(`[${this.assetId}] Unknown interface '${i}'`);
      }
    })
  ...
  updateComponentList() {
    ...
    if (!assetId) {
      ...
      throw Error(`Duplicate component class name - ${this.componentClassName}, assetIds: [${assetId}, ${this.assetId}]`);
    }
  ...
  containsComponentImport(srcDir) {
    ...
    fs.readdirSync(srcDir)
      ...
      .forEach((template) => {
        ...
        const regex = RegExp(`\\{\\{\\s*${componentImportPath}\\s+`);
        if (tpl.match(regex)) {
          b = true;
        }
      });
      ...
      get: (obj, prop) => {
        ...
        if (assetId && enableComponentCaching) {
          ...
          if (srcDirName) {
            ...
            const preprocessors = this.preprocessors || (this.preprocessors = []);
  ...
  */
  ...
  loadComponentIndexFile() {
    const { isRootCtxValue, getComponentClass, parseScript } = TemplatePreprocessor;
  ...
  addBrowserGlobals() {
    ...
    global.AbstractComponent = importFresh('../src/assets/js/abstract-component');
  ...
  static getDefinedComponentInterfaces() {
    const interfacesFile = pathLib.join(process.env.PWD, 'src', 'components', 'interfaces.json');

    return fs.existsSync(interfacesFile) ? JSON.parse(
      fs.readFileSync(interfacesFile, 'utf-8')
    ) : null;
  }
    ...
    if (instantiate) {
      ...
      if (this.resolver.processing) {
        ...
        if (global.components) {
          ...
          component.requiredComponents().forEach(className => {
            global.components[className]
          });
        }
      }
