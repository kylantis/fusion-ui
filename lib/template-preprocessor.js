...
class TemplatePreprocessor {
     ...
     * @param {*} param0 
     */
    resolvePathFromContext({ contextList, contextIndex, original, validTypes = [], strictTypeValidation, lookup = true }) {
        ...
        if (lookup && !type.endsWith('Literal')) {
            ...
            const value = this.lookupDataPath({
                fqPath: original,
                validTypes,
                strict: strictTypeValidation
            });

            targetType = value ? value.constructor.name : null;
        }
    ...
    resolvePath({ contextList, value, validTypes = [], strictTypeValidation = true }) {
     ...
     */
    addParameterizedExpression({ contextList, context, method, params = [], hash = {} }) {
    ...
    createSubExpression({ contextList, stmt }) {
    ...
    process0({ contextList, astNodes }) {
        ...
        ASTParser.prototype.BlockStatement = function (stmt) {
            ...
            let cleanupBlock = explode;

            let { type, original, lookup, targetType } = _this.resolvePath({
                contextList,
                value: stmt.params[0],
                validTypes,
                strictTypeValidation
            ...
            if (contextSwitching) {
                ...
                if (!lookup) {
                    throw new Error(`A synthetic function cannot be the target of the 
                                    ${stmt.path.original} block`
                    );
                }
            }
            ...
            if (!explode) {
                ...
                if (lookup && type == 'PathExpression') {
                    path = _this.trimDataPath(original);
                    _this.dataPaths.push(path);
                }
            ...
            // Todo: test if t's valid for this to be done
            // assuming the current block if an #block
            ...
            if (explode) {
                // Replace block with the content of it's body
                replacements.push({
                    parent,
                    replacementIndex: parent.indexOf(stmt),
            ...
            return cleanupBlock ? false : stmt;
        }
    ...
    lookupDataPath({ fqPath, validTypes = [], strict=true }) {
        ...
        if (validTypes && validTypes.length) {
                        ...
                        case type == 'Array':
                            if (value.constructor.name === 'Array') {
                                if (strict && !value.length) {
                                    throw new Error(`${fqPath} resolves to an empty array`);
                                }
                                return value;
                            }
                        ...
                        case type == 'Object':
                            if (value.constructor.name === 'Object') {
                                if (strict && !Object.keys(value).length) {
                                    throw new Error(`${fqPath} resolves to an empty object`);
                                }
                                return value;
                            }
            ...
            throw new Error(`${fqPath} must resolve to a value with one of the types: ${validTypes}`);
        }

        if (value == undefined) {
            throw new Error(`${fqPath} must resolve to a value`);
        } else {
            return value;
        }
    }
