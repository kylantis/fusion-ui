...
class TemplatePreprocessor {
  ...
  static componentRenderMethodName = 'render';
  ...
  updateComponentList() {
    const { getComponentsDistPath, getComponentList } = TemplatePreprocessor;
    const filePath = pathLib.join(getComponentsDistPath(), 'list.json');
  ...
  writeComponentJsToFileSystem({ incremental } = {}) {
    ...
    if (!incremental) {
      this.addComponentToGlobalScope();
    }
  }

  /**
   * This adds the component class to the global scope. Note that
   * we are using the AST based off the test class
   */
  // Todo: do the same for the client
  addComponentToGlobalScope() {
    const { getComponentsGlobal, makeRequire } = TemplatePreprocessor;
    if (!global.components) {
      global.components = getComponentsGlobal();
    }

    // eslint-disable-next-line no-unused-vars
    const require = makeRequire(this.getTestFile());

    // eslint-disable-next-line no-eval
    const ComponentClass = eval(this.componentSrc);
    global.components[this.componentClassName] = ComponentClass;

    this.updateComponentList();
  }

  static getComponentsGlobal() {
    const {
      makeRequire, getComponentList, getComponentsDistPath, findMatchingComponent,
    } = TemplatePreprocessor;
    const o = {};
    return new Proxy(o, {
      get: (obj, prop) => {
        ...
        if (componentList[prop]) {
          const folder = pathLib.join(
            getComponentsDistPath(), componentList[prop],
          );

          const templateScript = fs.readFileSync(
            pathLib.join(folder, 'template.min.js'), 'utf8',
          );
          eval(templateScript);

          const filePath = pathLib.join(folder, 'index.test.js');

          const compponentSrc = fs.readFileSync(
            filePath, 'utf8',
          );
          ...
          const componentClass = eval(compponentSrc);

          obj[prop] = componentClass;
          return componentClass;
        }

        const err = `Could not find component class: ${prop}`;
        const dir = findMatchingComponent({ className: prop });

        if (dir) {
          processFile({ dir, Preprocessor: TemplatePreprocessor });

          const componentClass = global.components[prop];
          assert(!!componentClass, err);
  ...
  loadComponentIndexFile() {
    const { isRootCtxValue, makeRequire } = TemplatePreprocessor;
  ...
  // Todo: Inform the user that the script should be CJS-based
  ...
  static createASyncMethodFromFunctionDeclaration({
    ...
    const {
      createArrowFunctionExpression, getValue,
  ...
  static getMethodFromFunctionDeclaration({
    ...
    return (
      async
        ? createASyncMethodFromFunctionDeclaration
        : createSyncMethodFromFunctionDeclaration
    )({ expression, addSyntheticPrefix, siblingNodeId });
  }
  ...
  static createSyncMethodFromFunctionDeclaration({
    ...
    if (siblingNodeId) {
      ...
      body.push({
        type: 'ExpressionStatement',
        expression: getCallExpression({
          methodName: componentRenderMethodName,
          args: [
            getValue({
              data: getIdentifier(variableName),
              target: getValue(siblingNodeId),
            }),
          ],
        }),
      });
  ...
  createSubExpression({
    ...
    hash, syntheticAlias, stmt, siblingNodeId, async,
  }) {
  ...
  createCustomBlockPath({
    ...
    const {
      ...
      getValue, getOptionsRetrivalStatements,
    } = TemplatePreprocessor;
    const ast = {
      type: 'FunctionDeclaration',
      id: {
  ...
  process0({
    ...
    ASTParser.prototype.SubExpression = function (stmt) {
      ...
      if (isCustomContext()) {
        ...
        _this.createSubExpression({
          ...
          params: stmt.params || [],
          hash: stmt.hash,
          siblingNodeId: stmt.siblingNodeId,
          async: stmt.async,
        });
        ...
        if (stmt.type === 'PathExpression') {
          ...
          if (!fromMustache) {
            ...
            // moreover, the target function will return a String, DOMElement
            // or Component that is to be rendered on the DOM - and in which
            // case this synthetic function will return an empty string
    ...
    const processComponentImport = ({ stmt }) => {
      if (stmt.params.length !== 2) {
        throw new Error('Component import statements require 2 params');
      }

      stmt.params.forEach((param) => {
        if (param.type !== 'PathExpression') {
          throw new Error(`'${param.original}' must be a PathExpression, line: ${getLine(stmt)}`);
        }
      });

      const { original: componentName } = stmt.params[0];
      const { original: alias } = stmt.params[1];

      if (componentName === alias) {
        throw new Error(`componentName should not be the same as alias, line: ${getLine(stmt)}`);
      }

      // The component should exist
      if (!fs.existsSync(pathLib.join(_this.srcDir, `../${componentName}`))) {
      ...
      const path = this.component.getExecPath({
        fqPath: `${contextList.pop()[rootQualifier].value}${pathSeparator}${alias}`,
        indexResolver: defaultIndexResolver,
      });

      if (_this.resolver.pathExists(path)) {
        throw new Error(`Alias name ${alias} already exists in the current context`);
      }

      _this.resolver.resolve(`${path}%${resolverComponentRefType}/${componentName}`);
    };
    ...
    ASTParser.prototype.MustacheStatement = function (stmt) {
      ...
      if (stmt.hash && stmt.hash.pairs.length && !_this.methodNames.includes(stmt.path.original)) {
        throw new Error(`Hashes not allowed, line ${getLine(stmt)}`);
      }

      const isSubExpression = stmt.params.length || stmt.hash
        || _this.methodNames.includes(stmt.path.original);
      ...
      if (async) {
        if (!isSubExpression) {
          throw new Error(`Illegal async invocation, line: ${getLine(stmt)}`);
        }

        if (bindParents
          .filter(({ original }) => original === 'each').length) {
          throw new Error(`An async invocation cannot exist in a loop, line: ${getLine(stmt)}`);
        }

        if (isCustomContext()) {
          throw new Error(`An async invocation cannot exist in a custom context, line: ${getLine(stmt)}`);
        }

        stmt.async = true;
      }

      if (isSubExpression && !async && !stmt.siblingNodeId) {
        // At the top of stmt, create a sibling DOM element
        const parent = bindParents.peek();
        const siblingNodeId = utils.generateRandomString();
        replacements.push({
          parent: parent.body,
          replacementIndex: parent.body.indexOf(stmt),
          replacementNodes: [
            createContentStatement({
              original: `<div id='${siblingNodeId}'></div>`,
            }),
          ],
        });
        stmt.siblingNodeId = siblingNodeId;
      }

      if (isCustomContext()) {
        if (getMetaPaths().includes(stmt.path.original)) {
  ...
  */
  getComponent({ componentSrc, instantiate = true } = {}) {
    ...
    const { window } = new jsdom.JSDOM(
      '<!DOCTYPE html><div id="parent"></div>',
      {
