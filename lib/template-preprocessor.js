...
class TemplatePreprocessor {
  ...
  loadCompiledComponentClasses() {
    ...
    Object.entries(this.metadata.distComponentList)
      .forEach(([className, assetId]) => {
        ...
        testComponentClass.schema = utils.deepClone(this.getConfig(className).schema);
        ...
        testComponentClass.metadata = eval(contents);
      });
  }
   ...
   */
  getSerializedComponent(className, declaredClassName, loadable) {
    ...
    if (this.resolver.processing) {
      ...
      instance.addMetadata('resolverInfo', resolverInfo);
  ...
  getSampleValueForType(type) {
    ...
    switch (type) {
      ...
      case componentRefType:
        return this.getSerializedComponent(BaseComponent.name, null, false);
    }
  }
      ...
      default:
        ...
        if (original.match(literalPathPrefixRegex)) {
          ...
          if (value == null && validType == componentRefType) {
            ...
            const { [typeProperty]: type, [nameProperty]: name } = this.resolver.getPathInfo(p);
  ...
  #getExecPath(fqPath) {
    ...
    return this.component.getExecPath0({
      fqPath,
      indexResolver: defaultIndexResolver,
      addBasePath: false,
    });
      ...
      switch (true) {
        case param.type.endsWith('Literal'):
          ...
          if (isInlineComponent) {
            param.targetComponent = param.original;
          }
          break;
        ...
        case param.type === 'PathExpression':
          ...
          resetPathExpression({
            stmt: param,
            original: _path,
            properties: { literalType, targetComponent, processed: true },
          });
  ...
  addDataBindTransformations(ast, decoratorConfig, terminal) {
    ...
    const getIterateBlockFactor = (outerIterateBlocks) => {
    ...
    const addToHookSize = (factor, stmt) => {
      ...
      if (decoratorConfig) {
        factor *= getIterateBlockFactor(decoratorConfig.outerIterateBlocks);
      }
      ...
      if (bindParents.length) {
        ...
        i = factor * getIterateBlockFactor(
  ...
  process0({ contextList, bindParents, program, validateHtml }) {
    ...
    ASTParser.prototype.MustacheStatement = function (stmt) {
      const {
        loadInlineComponentHelperName, getReservedMustacheHashKeys,
      } = TemplatePreprocessor;
              ...
              setLocSource(
                {
                  ...
                  body: [{
                    ...
                    hash: node.hash, params: node.params, xyz: true
                  }]
                },
                getProgramId()
              );
  ...
  static getVisitor() {
    ...
    Visitor.prototype.NodeRelacement = function () {
    }
    ...
    Visitor.prototype.Program = function (program) {
      ...
      for (let i = 0, l = array.length; i < l; i++) {
        ...
        switch (true) {
          ...
          case array[i].type == 'NodeRelacement':
            const { repl } = array[i];
            array.splice(
              i, 1, ...repl,
            );
  ...
  static addGlobals() {
    ...
    global.K_IndexedDB = importFresh('../src/assets/js/lib/indexed_db');
