...
class TemplatePreprocessor {
  ...
  process() {
    const {
      ...
      generateClassesInfo, createLocSource,
    } = TemplatePreprocessor;
    ...
    if (!this.ast) {
      ...
      const program = this.createProgram({
        locSource: createLocSource({
          base: this.className, fileName: 'index.view',
        }),
      ...
      this.bindParents = [{ type: 'root', body: this.ast.body }];
    }
  ...
  resolvePathFromContext({
    contextList, inlineBlock, pathInfo, declaredOriginal, original, validType, stmt, syntheticAlias, nameQualifier, create,
  }) {
    const {
  ...
  resolvePath({
    ...
    const globalVariable = this.resolveGlobalVariable({ pathInfo, original, validType, stmt });
  ...
  ensureParamsOrder({ params, bindParents }) {
    const { inlineParameterPrefix, getOuterInlineBlock, throwError } = TemplatePreprocessor;
      ...
      switch (true) {
        ...
        case param.type === 'PathExpression':
          if (param.original.startsWith(inlineParameterPrefix)) {
            ...
            throwError(`The param: ${param.original} is in an invalid position`, param);
          }
          break;
        ...
        default:
          throwError(`Unknown type "${param.type}"`, param);
      }
    }
  ...
  addDataBindTransformations(ast) {
    ...
    const {
      ...
      createContentStatement, createMustacheStatement, getLine, htmlWrapperCssClassname, throwError,
    } = TemplatePreprocessor;
  ...
  process0({ contextList, bindParents, program }) {
    ...
    const acceptPathExpressionInCustomCtx = ({ stmt, visitor }) => {
      const {
        ...
        getSuffix, invokeDataPathTransform, toPathExpressionLiteral, throwError,
      } = TemplatePreprocessor;
      ...
      if (prev.startsWith(inlineParameterPrefix)) {
        throwError(`Inline parameter prefixes are not allowed in custom context`, stmt);
      }
    ...
    const ensureCanCreateHtmlWrapper = (stmt) => {
      const { throwError } = TemplatePreprocessor;
      const { canonicalType, type } = stmt;

      if (!canCreateHtmlWrapper()) {
        throwError(`A ${canonicalType || type} is not allowed at this template location`, stmt);
      }
    }
    ...
    ASTParser.prototype.BlockStatement = function (stmt) {
      ...
      if (!stmt.visited) {
        ...
        if ((!isCustomContext() || htmlWrapperErrors.length) && !containsHtmlError()) {
          ...
          // If this is a custom context, syntheticNodeId was pre-emptively set to true above prior to ast generation...
          // We need to set back to false before calling createHtmlWrapper(...)
          stmt.syntheticNodeId = false;
    ...
    const processMetaStatement = ({ stmt }) => {
      const {
        componentImportPath, variablePath, fnHelperName, throwError
      } = TemplatePreprocessor;
    ...
    const getPartialContextList = function ({ stmt }) {
      ...
      if (isCustomContext()) {
        ...
        for (const pair of getHashPairs(stmt.hash)) {
          ...
          contextObject[pair.key] = {
            ...
            asVariable: true,
            index: ctxIndex,
            canonicalSource: pair.value.canonicalSource || stringifyHandlebarsNode(pair.value),
          };
        }
      } else {
        ...
        for (const pair of getHashPairs(stmt.hash)) {
          ...
          if (!pair.value.type.endsWith('Literal')) {
            ...
            const processed = pair.value.type.endsWith('Literal') || pair.value.processed;
    ...
    const getPartial = ({ partialName, stmt }) => {
      ...
      const {
        partialBaseDirHashKey, partialBaseLocHashKey, runtimeDecoratorHashKey, eagerDecoratorHashKey, visitNodes,
      ...
      (() => {
        ...
        for (const blockName in inlineBlocks) {
          ...
          const { requiredParams, optionalParams, templateSource } = block;
        ...
        for (const parent of _this.metadata.parents) {
          ...
          const { inlineBlocks } = _this.getConfig(parent);

          if (!inlineBlocks || !inlineBlocks.includes(partialName)) {
            continue;
          }
          ...
          for (const node of classInfo.templateAst.body) {
            if (node.type != 'DecoratorBlock') {
              continue;
            }

            const decoratorName = node.params[0].original;

            if (!node.program.programId) {

              setPartialSourceInfo(node.program, dir, parent);

              registerProgram({
                locSource: createLocSource({ base: parent, fileName, decoratorName }),
                templateSrc: classInfo.templateSrc,
            ...
            if (decoratorName == partialName) {
              ...
              block = {
                program: utils.deepClone(node.program),
              };
    ...
    ASTParser.prototype.PartialStatement = function (stmt) {
      ...
      const {
        componentRefType, loadComponentHelperName, wordPattern, getReservedPartialHashKeys, throwError,
      } = TemplatePreprocessor;
    ...
    ASTParser.prototype.DecoratorBlock = function (stmt) {
      const {
        reservedDecoratorNames, allowOverlappingQualifiersInDecorator, runtimeDecoratorHashKey, eagerDecoratorHashKey,
        getDecoratorParams, getHashValue, throwError, getOuterInlineBlock,
      } = TemplatePreprocessor;

      const { customContextList } = stmt;
      ...
      const { original: eager } = runtime ? { original: true } : getHashValue({ stmt, key: eagerDecoratorHashKey, type: 'BooleanLiteral', cleanup: true }) || {};
      ...
      const { requiredParams, optionalParams } = getDecoratorParams(stmt.params);

      const paramCount = [...requiredParams, ...optionalParams].length;
      ...
      // We need to perform any node replacements that exists inside this inline block
    ...
    /**
       * This is a custom AST type that is wrapped around a partial's AST program, inorder to create 
       * a clearly defined boundary between decorators defined inside and outside the partial declaration
       */
  ...
  addInlineBlock({
    bindParents, decoratorName, stmt, requiredParams, optionalParams, isCustomCtx, runtime,
  }) {
    const {
      dataPathRoot, createContentStatement, visitNodes, getBlockSource0, getSourceIndex, sanitizeValue,
    } = TemplatePreprocessor;

    // Add a reference to the parent
    const parent = utils.peek(bindParents);

    // We need some sort of marker on the ast to be used for replacement purpose, if we need to add 
    // a 'storeContext' block later on
    ...
    decorators[decoratorName] = {
      ...
      isCustomCtx,
      runtime,
      templateSource: getBlockSource0(this.templateSrc, { startIndex, endIndex })
    };
  ...
  static validateW3cHtmlString(src, validator, dictionary, messages, loc) {
    ...
    if (!report.valid) {
      report.results[0].messages.forEach(msg => {
        ...
        messages.push({
          message: `[${loc}] ${ruleId}: ${transformMessageWithDictionary({ message, dictionary })} [${line}:${column}]`
        });
      });
    }
  ...
  // Todo: Remove if not used
  static getAllHandlebarsBlockTypes() {
    return [
