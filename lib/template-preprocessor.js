...
class TemplatePreprocessor {
    ...
    if (checkType) {
      ...
      switch (true) {
        ...
        default:
          ...
          if (validTypes[0] == arrayType) {

            // Do a lookup for the 0th index, which will in effect, update <original>
            // to contain a literal at index 0. This will be used for lookup
            // purposes. It is important to note that the index resolver used at compile-time
            // defaults to 0, and this will be used for all data lookup in the program of
            // the #each block. This is the reason we need to initialize index 0 at the 
            // barest minimum
  ...
  resolvePath({
    ...
    const {
      inlineParameterPrefix, processLiteralSegmentMethod, literalType,
      dataPathRoot, pathSeparator,
      resetPathExpression, getAllValidTypes, getOuterInlineBlock,
      getPathOffset, getLine,
    } = TemplatePreprocessor;
    ...
    const globals = Object.keys(RootProxy.getUserGlobalVariables());

    if (globals.includes(stmt.original)) {

      const { original } = stmt;
      const targetType = RootProxy.getUserGlobalVariables()[original];

      if (!validTypes.includes(targetType)) {
        throw Error(`Expected ${original} to have type ${validTypes} not ${targetType}, ${getLine(stmt)}`)
      }

      return {
        type: 'PathExpression',
        original: `${dataPathRoot}${pathSeparator}${original}`,
  ...
  process0({
    ...
    ASTParser.prototype.BlockStatement = function (stmt) {
      ...
      if (hookMethod && !_this.methodNames.includes(hookMethod)) {
        throw Error(`Unknown method: ${hookMethod} specified as ${hookHashKey}, ${getLine(stmt)}`);
      }
      ...
      if (!custom) {
        ...
        const htmlErrors = _this.getHtmlErrors0(
          _this.getBlockSource(stmt), null, [],
        );

        if (htmlErrors.length) {

          // htmlErrors.forEach((error) => _this.logger.warn(error));
          _this.logger.warn(`Disabled data-binding for #${stmt.path.original} block, ${getLine(stmt)}`);

          if (hookMethod) {
            throw Error(`${hookHashKey} method cannot be specified for this block due to invalid html markup ${getLine(stmt)}`);
          }
    ...
    const processPartial = function ({ stmt }) {
      ...
      if (stmt.params.length) {
        // Todo: Why am I doing this, and then calling addParamsAsHashes(...) just below
  ...
  getHtmlErrors0(src, fileName, messages, validator) {

    // const htmlTagRegex = /(\&|>|<)/g;
    const { validateHtmlString, getHtmlValidator } = TemplatePreprocessor;
    ...
    ASTParser.prototype.PartialStatement = function (stmt) {
      ...
      _this.getHtmlErrors0(
        fs.readFileSync(_this.getPartialPath(partialName), 'utf8'),
        partialName, messages, validator
      );
    }
    ...
    return validateHtmlString(src, validator, dictionary, messages, fileName);
  }

  static validateHtmlString(src, validator, dictionary, messages, fileName) {
    ...
    if (!report.valid) {
      ...
      report.results[0].messages.forEach(msg => {
        ...
        messages.push(`${fileName ? `[${fileName}.view] ` : ''}${message} ${line}:${column}`);
      });
    }
