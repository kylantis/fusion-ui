...
class TemplatePreprocessor {
  ...
  static invokeSyntheticMethodsOnCompile = false;
  ...
  static isResolvableInScope(pathInfo) {
    const { path } = pathInfo;
    return path.length && !path.startsWith('@');
  }
  ...
  getSyntheticMethodValue({ stmt, source, method }) {
    const { invokeSyntheticMethodsOnCompile, throwError, getLine } = TemplatePreprocessor;
  ...
  visitSubExpressionMetaStatement(stmt, bindParents) {
    ...
    switch (stmt.path.original) {
      case fnHelperName:
        ...
        const isValidMethod = type == 'StringLiteral' && this.methodNames.includes(original);
