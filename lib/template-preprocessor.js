    ...
    for (const className of keys) {
      ...
      try {
        ...
        if (e.message.includes('Class extends value undefined is not a constructor or null')) {
          this.logger.warn(`Did you recently update the parent component of ${className}? If so, remove ${className} from dist/components/list.json, then re-compile ${className}`);
        }
        throw e;
      }
    ...
    const processComponentImport = ({ stmt }) => {
      ...
      if (
        ...
        if (
          ...
          // is consistent.
          v[typeProperty] == componentRefType && v[nameProperty] == className) {
        } else {
          throw Error(`Alias name ${alias} already exists in the current context`);
        }
      }
