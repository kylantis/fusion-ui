...
class TemplatePreprocessor {
  ...
  constructor({
    ...
    const { getMethodNames, getComponentClass, createAst } = TemplatePreprocessor;
    ...
    if (!ast) {
      ast = createAst({ templateSrc, initial: true });
    }
  ...
  static createAst({ templateSrc, initial = false }) {

    const { visitNodes, OPEN_SEXPR_REPL_PATH } = TemplatePreprocessor;

    try {
      handlebars.parse(templateSrc);
    } catch (e) {
      if (!e.message.endsWith("got 'OPEN_SEXPR'")) {
        // A parsing error occured that we have no idea about, rethrow
        throw e;
      }
    }
    ...
    // Get subexpressions that are paths
    const m = templateSrc.match(/(\{\{|\()\s*\(/g);


    while (m && m.length) {

      const ind = templateSrc.indexOf(m[0]);

      if (ind >= 0) {

        // We need to skip {{ or (, depending on the scenario
        const offset = m[0][0] == '{' ? 2 : 1;
        ...
        while (j < templateSrc.length) {
          const char = templateSrc[j];
          ...
          if (
            noOpStack.length == 0 &&
            // There could be a space between {{|( and ( in which
            // case stack.length will be 0
            parts.includes('(')
          ) {
            break;
          }
        ...
        const expr = templateSrc.substring(start, end + 1);

        const variable = `$${utils.generateRandomString()}`;

        templateSrc = utils.update(templateSrc, expr, variable)

        const ast = handlebars.parse(
          `{{${expr}}}`
            .replace(/(?<=(\{|\()\s*)\(/g,
              `${OPEN_SEXPR_REPL_PATH} (`
            )
        );

        visitNodes({
          Visitor: handlebars.Visitor,
          types: ['MustacheStatement', 'SubExpression'],
          ast,
          consumer: ({ stmt }) => {
            if (stmt.path.original == OPEN_SEXPR_REPL_PATH) {
              assert(stmt.params[0].type == 'SubExpression');
              stmt.path = stmt.params[0];
              stmt.params.shift();
            }
          }
        })
    ...
    // In the ast, replace all variables with their sub expressions
    visitNodes({
      Visitor: handlebars.Visitor,
      types: ['MustacheStatement', 'SubExpression'],
      ast,
      consumer: ({ stmt }) => {
        ...
        const expr = variables[stmt.path.original];

        if (expr) {

          const { type, path, params } = expr;

          utils.clear(stmt.path);

          stmt.path.type = type;
          stmt.path.path = path;
          stmt.path.params = params;

          // Todo: slight adjust this to indicate the accurate loc
          stmt.path.loc = stmt.loc;
        }
      },
    });
  ...
  emitMetadata() {
    ...
    this.emitMethodReturningArray({ name: 'mapPaths' });
  }
      ...
      switch (true) {
        ...
        case param.type === 'PathExpression':
          ...
          if (this.methodNames.includes(param.original)) {
            _path = synthethicMethodPrefix + param.original;
      ...
      switch (true) {
        ...
        case value.type === 'PathExpression':
          ...
          if (this.methodNames.includes(value.original)) {
            _path = synthethicMethodPrefix + value.original;
  ...
  addDataVariablesToContext({ contextObject, path, dataVariables, synthetic }) {
  ...
  process0({
    ...
    ASTParser.prototype.BlockStatement = function (stmt) {
      ...
      if (contextSwitching) {
        ...
        if (canIterate) {
          ...
          const { blockPathHashKey, customEachHelperName } = TemplatePreprocessor;
          ...
          if (!syntheticAlias) {
            ...
            stmt.params = [createStringLiteral(path)];
          }
      ...
      if (stmt.path.original === 'if' && type === 'PathExpression') {
        ...
        const path = stmt.params[0].original
    ...
    ASTParser.prototype.MustacheStatement = function (stmt) {
      ...
      if (isSubExpression) {
        ...
        let { logicGate, logicGatePruneKey } = stmt;
        ...
        if (stmt.processed) {
          ...
          const { logicGatePathRoot, pathSeparator,
            resolveMustacheHelperName, hasDataPathFormat
          } = TemplatePreprocessor;

          if (logicGate) {

            // Prune the synthetic method generated for this logic gate
            _this.pruneComponentAst({ pruneKey: logicGatePruneKey });

            const getValue = (item) => {
              const isNumber = (s) => !Number.isNaN(parseInt(s, 10));
              if (isNumber(item)) {
                item = {
                  type: 'LogicGate',
                  original: item,
      ...
      let { processed, original, type, dataVariable } = stmt.path;
  ...
  static visitNodes({ Visitor = handlebars.Visitor, types, ast, consumer, parentFirst = true }) {
    function ASTParser() {
    }
    ASTParser.prototype = new Visitor();
