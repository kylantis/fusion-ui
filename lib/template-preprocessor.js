...
class TemplatePreprocessor {
  ...
  process0({ contextList, bindParents, program }) {
      ...
      const processBlockInRootCtx = () => {
        ...
        if (contextSwitching) {
          ...
          if (canIterate) {
            ...
            // Todo: The statements added below should have a loc object
          ...
          (() => {
            ...
            hashPairs
              ...
              .forEach((pair) => {
                ...
                if (isCustomContext()) {
                  contextObject[key] = { ..._contextObject[key], contextId };
                } else {

                  if (value.processed) {

                    value.original = toCanonical(`${value.original}!!`);
                    value.parts = toParts(value.original);

                    contextObject[key] = _contextObject[key];

                  } else {

                    contextObject[key] = {
                      variable: true, lookup: true, contextId,
                    }
                  }
                }
              });
    ...
    const addHashesToContext = function ({ hashPairs, contextObject, ctxIndex }) {
      ...
      if (isCustomContext()) {
        ...
        for (const pair of hashPairs) {
          ...
          let {
            type, original, lookup = false, synthetic = false, canonicalSource,
            isResolvedPath, variable, contextId
          } = pair.value;
    ...
    const processPartial = function ({ stmt }) {
      ...
      const { partialDeferHashKey, getSuffix } = TemplatePreprocessor;
    ...
    ASTParser.prototype.PartialWrapper = function (stmt) {
      ...
      bindParents.push({ type: 'PartialWrapper', body: stmt.body, parent: utils.peek(bindParents), loc: stmt.loc, });
