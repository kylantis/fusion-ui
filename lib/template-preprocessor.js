...
class TemplatePreprocessor {
  ...
  getMethodNameHashValue({ stmt, key, cleanup = false, allowMultiple }) {
    ...
    if (methodName) {
      if (allowMultiple) {
        methodName.split(/,\s*/g).forEach(n => validate(n));
      } else {
        validate(methodName);
      }
    }
  ...
  process0({ contextList, bindParents, program }) {
    ...
    const getHook = ({ stmt }) => {
      ...
      // Validate "hookOrder" hash value if specified
      getHashValue({
        stmt, key: hookOrderHashKey, type: 'NumberLiteral', cleanup: false,
      })

      return this.getMethodNameHashValue({ stmt, key: hookHashKey });
    };
    ...
    const getMustacheMetaPaths = () => {
      const { componentImportPath, variableHelperName } = TemplatePreprocessor;
      return [componentImportPath, variableHelperName];
    };

    const processMustacheMetaStatement = function ({ stmt }) {

      const {
        componentImportPath, variableHelperName, throwError, createStringLiteral, toParts,
      } = TemplatePreprocessor;
      ...
      if (isCustomContext()) {
        ...
        const wrapInResolveMustacheHelper = () => {
          ...
          return {
            ...
            fromMustache: true,
            // Note: The only reason we will ever need a sub expression in a custom context is
            // for rendering inline components,
    ...
    ASTParser.prototype.DecoratorBlock = function (stmt) {
      ...
      if (runtime && !fromMarker) {
        ...
        if (paramCount) {
          ...
          // Note: Runtime decorators are not exclusively consumed by Partial statements but rather may also be triggered at 
          // runtime using the rootProxy as the context. Therefore, if params are allowed, then not all  path expressions will
          // be transformed as root paths inside this decorator's program, as they will be seen as inline parameters, hence 
