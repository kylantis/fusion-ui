...
class TemplatePreprocessor {
  ...
  static startAttrCtxHelperName = 'startAttributeContext';

  static endAttrCtxHelperName = 'endAttributeContext';
  ...
  static loadComponentMethodName = 'loadInlineComponent';
  ...
  static stencilCssClassname = 'k-stencil';
  ...
  static getDefaultHelpers() {
    const { loadComponentMethodName } = TemplatePreprocessor;
    return [loadComponentMethodName];
  }
  ...
  static getMethodNames({ component }) {
    const { getDefaultHelpers } = TemplatePreprocessor;
  ...
  static getMethodFromFunctionDeclaration({
    ...
    return (
      async
        ? createASyncMethodFromFunctionDeclaration
        : createSyncMethodFromFunctionDeclaration
    )({ expression, addSyntheticPrefix, wrapperNodeId });
  }
  ...
  createSubExpression({
    ...
    hash, syntheticAlias, stmt, wrapperNodeId, async,
  }) {
  ...
  getCustomBlockAsyncFuntion({ methodName, stmt }) {
    const {
      ...
      chainedLoadingStratedyFieldName,
          ...
          })),
          {
            ...
            expression: getCallExpression({
              ...
              args: [{
                ...
                target: getValue(stmt.wrapperNodeId),
  ...
  static getTokens({ ast }) {
    ...
    const { customEachHelperName, stencilCssClassname, startTNBCtxHelperName,
    ...
    const createHtmlWrapper = ({ id = utils.generateRandomString(), stmt, tagName = 'span', addStencilClass = false }) => {
      const parent = bindParents.peek();
      const wrapperNodeId = id;
      ...
      replacements.push({
        ...
        replacementNodes: [
          createContentStatement({
            original: `<${tagName} id='${wrapperNodeId}'${addStencilClass ? ` class='${stencilCssClassname}'` : ''}>`,
          }),
        ],
      });
      ...
      stmt.wrapperNodeId = wrapperNodeId;
    };
    ...
    streamTokenizer
      .on('data', (tokens) => {
        ...
        for (let i = 0; i < tokens.length; i++) {
          ...
          // console.info(`${token.type} ${token.content}`);
        ...
        if (startTagEnd >= 0) {
          const attributeList = getCurrentAttributesList({
            index: startTagEnd,
          });

          const attributeKeys = Object.keys(attributeList);
          let hasDataPaths = false

          if (attributeKeys.includes('id')) {
            if (isInIterateBlock() || hasDataPaths) {
              // not allowed!
            }
          }
    ...
    ASTParser.prototype.MustacheStatement = function (stmt) {
      ...
      const { rawDataPrefix, hasDataPathFormat, getLine } = TemplatePreprocessor;

      let { processed, original } = stmt.path;
      let tokenizerOriginal;

      if (processed && hasDataPathFormat(original)) {

        if (original.includes(rawDataPrefix)) {
        ...
        if (isTextNode >= 0) {
          console.info('createTextNodeBindWrapper');
          console.info(getLine(stmt), stmt.path.original);
    ...
    const isCustomContext = () => customBlockCtx.peek().value;

    ASTParser.prototype.BlockStatement = function (stmt) {
      const { getHandleBarsBlockHelpers } = TemplatePreprocessor;
      const custom = !getHandleBarsBlockHelpers().includes(stmt.path.original);
  ...
  process0({
    ...
    const createHtmlWrapper = ({ stmt, tagName = 'span', addStencilClass = true }) => {
      const parent = bindParents.peek();
      const wrapperNodeId = utils.generateRandomString();
    ...
    ASTParser.prototype.SubExpression = function (stmt) {
      ...
      if (isCustomContext()) {
        ...
        _this.createSubExpression({
          ...
          params: stmt.params || [],
          hash: stmt.hash,
          wrapperNodeId: stmt.wrapperNodeId,
          async,
        });
    ...
    ASTParser.prototype.MustacheStatement = function (stmt) {
      ...
      const reservedMethodNames = [loadComponentMethodName];
    ...
    ASTParser.prototype.PartialStatement = function (stmt) {
      const { componentType, loadComponentMethodName } = TemplatePreprocessor;
      const partialName = stmt.name.original;
      ...
      return ASTParser.prototype.MustacheStatement.call(this, Object.assign(stmt, {
        ...
        path: createPathExpression({
          original: loadComponentMethodName,
        }),
        generated: true,
      }));
    };
