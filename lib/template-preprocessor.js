...
class TemplatePreprocessor {
  ...
  emitMetadata() {
    const { getScalarValue, getObjectValue, getArrayValue } = TemplatePreprocessor;
  ...
  static validatePathExpression(stmt) {
    const {
      pathSeparator, isRootCtxValue, getLine
    } = TemplatePreprocessor;

    const wordPattern = /^\w+$/g;
    const hasIndex = /\[[0-9]+\]/g;

    const { original, parts } = stmt;
    ...
    parts.forEach(p => {
      if (!p.match(wordPattern) && !p.match(hasIndex)) {
        throw Error(`Invalid PathExpression: ${original}, ${getLine(stmt)}`);
      }
    });
  }
  ...
  static getPathOffset({ original, contextList, stmt, }) {
    const {
      getDataVariables,
      getScopeQualifier,
      hasObjectPrefix,
    ...
    if (isDataVariable) {
      ...
      if (hasObjectPrefix({ value: original, key: 'this' })) {
        if (hasOffset) {
          throw Error(`Glob clause not expected when using 'this': ${prev}, ${getLine(stmt)}`);
        }
      ...
      // Ensure that index >= 0, 0 meaning we are on the root context
      ...
      if (!index) {
        ...
        if (startsWithIndex) {
          ...
          throw Error(`The root context should not be accessed like an array: ${prev}, ${getLine(stmt)}`);
        }

      } else if (hasOffset || !original.length || startsWithIndex) {

        // Make original a pure path. A pure path is one whose first "part" corresponds to a scope qualifier
        ...
        // blocks, and for good reason. So, we know that with the every non-root context (including
        // custom contexts) has a scope
        ...
        // Note: This is useful if <startsWithIndex> == true, as this will prevent
  ...
  resolveSyntheticContext({ k, v, original, syntheticAlias, stmt }) {
    const { getSuffix } = TemplatePreprocessor;

    assert(v.synthetic && v.canonicalSource && original.startsWith(k));

    let canonicalSource;

    if (
      // <suffix> needs to be processed 
      k !== original ||
      ...
      original = this.createRootProxyIndirection({
        stmt,
        canonicalSource: v.canonicalSource,
        path: v.value,
        suffix: k === original ? false : suffix,
        syntheticAlias,
      });

      if (suffix) {
        canonicalSource = `${v.canonicalSource}.${suffix}`;
      }
  ...
  resolvePathFromContext({
    ...
    const {
      rootQualifier, pathSeparator: separator, dataPathRoot, syntheticMethodPrefix,
          ...
          if (
            ...
            if (type.endsWith('Literal')) {
              ...
              if (!v.lookup) {
                ...
                if (syntheticAlias) {
                  // <syntheticAlias> is only passed in when used in context-switching block (see resolvePathParam(...)),
                  // so we do not expect that <v> will have a scalar value
                  throw Error(`Path: ${k} cannot be used in a context switching context`);
                }
              }

              const r = this.resolveSyntheticContext({ k, v, original, syntheticAlias, stmt });
  ...
  resolvePath({
    ...
    const {
      ...
      resetPathExpression, getOuterInlineBlock, getPathOffset, getLine, getRootOriginalFromPathOffset
    } = TemplatePreprocessor;
    ...
    if (type === P_EXPR) {
      ...
      offset.path = this.component.processLiteralSegment({
    ...
    if (type && type.endsWith('Literal')) {
      if (validTypes.length > 0 && !validTypes.includes('Literal')) {
        throw Error(`Path: ${prevOriginal} cannot resolve to a Literal value, validTypes=[${validTypes}]`);
      }
    }
        ...
        case param.type === 'PathExpression':
          ...
          if (param.original.startsWith(literalPrefix)) {
            ...
            statement.declarations[0].init = getProxyStatement({
              path: param.transformFn ? param.transformFn(param.original) : param.original,
            });
          ...
          if (this.methodNames.includes(param.original)) {
            ...
            if (path.type.endsWith('Literal')) {
              ...
              statement = getVariableEnvelope(name);
              statement.declarations[0].init = getProxyStatement({
                path: param.transformFn ? param.transformFn(path.original) : path.original,
              });
            }
          }
            ...
            hashObject.properties.push(
              getProperty({
                ...
                value: getProxyStatement({
                  path: value.transformFn ? value.transformFn(value.original) : value.original,
                }),
                stmt: value,
              }),
            );
            ...
            if (path.type.endsWith('Literal')) {
              ...
              hashObject.properties.push(
                getProperty({
                  key,
                  value: getProxyStatement({
                    path: value.transformFn ? value.transformFn(path.original) : path.original,
                  }),
                  stmt: value,
                }),
              );
  ...
  static getPreservedPathExpressionAttibutes() {
    return [
      'loc', 'participantType', 'transformFn', 'isLogicGateCondition',
      'rootProxyIndirection',
    ];
  }
  ...
  process0({
    ...
    const acceptPathExpressionInCustomCtx = ({ stmt, visitor }) => {
      ...
      if (original.split('.')[0] === '@root') {
        ...
        // Todo: Since we want to transform @root to reference the nearest custom context scope qualifier,
        // shouldn't we assert that code never reaches this point? throw an error maybe?
      }
    ...
    ASTParser.prototype.BlockStatement = function (stmt) {
      const {
        blockTransformHashKey, getPathFromSyntheticAlias, getChildPathFromSyntheticAlias
      } = TemplatePreprocessor;
      ...
      const blockOptions = _this.getBlockOptions(stmt);
      ...
      if (isCustomContext()) {
        ...
        switch (true) {
          ...
          case blockOptions.contextSwitching:
            const contextObj = {};
      ...
      if (contextSwitching) {
        ...
        if (canIterate) {
          ...
          if (!isSynthetic) {
            ...
            // <this.syntheticContext[alias].current> has already through an invocation to
            // setSyntheticContext(...) above
          }
    ...
    ASTParser.prototype.MustacheStatement = function (stmt) {
      ...
      if (stmt.path.type != 'PathExpression') {
        throw Error(
          `Only a PathExpression must be used as the path in a MustacheStatement, line: ${getLine(stmt)}`
        );
      }
      ...
      if ((!isSubExpression) && !isMetaPath && (stmt.params.length)) {
        throw Error(`Unkown method name: ${stmt.path.original}, line: ${getLine(stmt)}`);
      }
    ...
    const getPartialContextList = function ({ stmt, inline }) {
      ...
      // PartialStatements are not context switching nodes hence, we don't create any 
      // new context, but rather update the prior context
          ...
          if (block.requiredParams) {
            ...
            block.requiredParams.forEach((param) => {
              if (!hashKeys.includes(param)) {
                throw Error(`The hash key: '${param}' is required to load inline block: ${blockName}, line: ${getLine(stmt)}`);
              }
            });
          }
    ...
    ASTParser.prototype.PartialStatement = function (stmt) {
      ...
      const { typeProperty, componentRefType } = PathResolver;
      ...
      if (!isComponentClass && !isCustomContext()) {
        // Ensure that this resolves to a component
        const fqPath = `${utils.peek(contextList)[rootQualifier].value
          }${partialName == '.' ? '' : `${pathSeparator}${partialName}`}`;

        const path = _this.component.getExecPath0({
          fqPath: fqPath.replace(/^data__/g, ''),
          indexResolver: defaultIndexResolver,
        });

        const v = _this.resolver.getPathInfo(path);

        if (!v || v[typeProperty] != componentRefType) {
          throw Error(
            `${fqPath} does not resolve to a component, hence cannot be used in a PartialStatement, line ${getLine(stmt)}`
          );
        }
      }
