...
class TemplatePreprocessor {
  ...
  static getComponentsSrcPath() {
    return pathLib.join(process.cwd(), 'src', 'components');
  }

  static getDistPath() {
    return pathLib.join(process.cwd(), 'dist');
  }
  ...
  getDistPath() {
    const { getComponentDistPath } = TemplatePreprocessor;
    return getComponentDistPath(this.assetId);
  }

  static getComponentDistPath(assetId) {
    const { getComponentsDistPath } = TemplatePreprocessor;

    const p = pathLib.join(getComponentsDistPath(), assetId);
  ...
  writeAssetsToFileSystem() {
    ...
    this.writeComponentJsAstToFileSystem();
    ...
    TemplatePreprocessor.ensureGlobalDepsFileExists();
  }

  writeStringifiedViewFileAst() {
    const { stringifyHandlebarsNode } = TemplatePreprocessor;

    fs.writeFileSync(
      pathLib.join(this.getDistPath(), 'index.view.ast'),
      stringifyHandlebarsNode(this.ast),
    );
  }

  pruneAfterFatalError() {
    const { getComponentJsAstFile } = TemplatePreprocessor;

    const jsAstFile = getComponentJsAstFile(this.assetId);

    if (fs.existsSync(jsAstFile)) {
      // We need to remove this file so that subsequent saves to the .js file will
      // trigger a full compilation

      fs.rmSync(jsAstFile);
    }
  }

  static getComponentJsAstFile(assetId) {
    const { getComponentDistPath } = TemplatePreprocessor;

    return pathLib.join(
      getComponentDistPath(assetId), 'index.js.ast'
    );
  }

  writeComponentJsAstToFileSystem() {
    const { getComponentJsAstFile } = TemplatePreprocessor;

    fs.writeFileSync(
      getComponentJsAstFile(this.assetId), JSON.stringify(this.componentAst)
    )
  }
  ...
  static getComponentListRelativePath() {
    ...
    return pathLib.relative(process.cwd(), getComponentListPath());
  }
  ...
  writeComponentJsToFileSystem() {
    const { writeComponentJsToFileSystem } = TemplatePreprocessor;

    const { srcDir, assetId, componentAst } = this;
    writeComponentJsToFileSystem({ srcDir, assetId, componentAst });
  }

  static writeComponentJsToFileSystem({ srcDir, assetId, componentAst }) {
    const {
      parseComponentClass, mergeAst, getComponentDistPath, writeMinifiedComponentJsFile
    } = TemplatePreprocessor;

    const distFolder = getComponentDistPath(assetId);

    const fileName = 'index.js';
    const testFileName = 'index.test.js';

    let testComponentSrc = fs.readFileSync(pathLib.join(srcDir, testFileName), 'utf8');

    let { data: componentSrc, ComponentClass } = parseComponentClass({ dir: srcDir });

    // Merge AST into the component main js
    componentSrc = mergeAst(componentSrc, ComponentClass, componentAst);

    // Write js files
    fs.writeFileSync(pathLib.join(distFolder, testFileName), testComponentSrc);
    fs.writeFileSync(pathLib.join(distFolder, fileName), componentSrc);

    // Write minified js files (for use in the browser)
    writeMinifiedComponentJsFile({ data: testComponentSrc, fileName: testFileName, assetId });
    writeMinifiedComponentJsFile({ data: componentSrc, fileName: fileName, assetId });
  }
  ...
  loadCompiledComponentClasses() {
    const { loadCompiledComponentClasses } = TemplatePreprocessor;

    loadCompiledComponentClasses(this.metadata.distComponentList);
  }

  static loadCompiledComponentClasses(componentList) {
    const {
      getComponentsDistPath, getComponentListRelativePath, getSchemaFilePath, loadJson,
      parseComponentClass, throwError,
    } = TemplatePreprocessor;

    global.components = {};

    const componentClassMetadataMap = (global.self && self.appContext) ? self.appContext.getComponentClassMetadataMap() : null;
    ...
    Object.entries(componentList)
      .forEach(([className, assetId]) => {
        ...
        try {
          testComponentClass = parseComponentClass({
            dir,
            useTestClass: true,
          }).ComponentClass;
        } catch (e) {
          throwError(`Error occured while loading component: ${className}. To proceed, remove the entry "${className}" from ${getComponentListRelativePath()}`);
        }
  ...
  static getAppContextObject() {
    ...
    const enumsFile = pathLib.join(process.cwd(), 'src', 'components', 'enums.json');
      ...
      get: (obj, prop) => {
        ...
        const load = () => {
          ...
          if (!componentClass) {
            ...
            componentClass = parseComponentClass({
  ...
  writeHtmlFileToFileSystem() {
    const { htmlWrapperCssClassname } = TemplatePreprocessor;
  ...
  writeConfigFileToFileSystem() {
    ...
    const behaviourRequires = {};

    Object.entries(this.resolver.config.behaviours || {})
      .forEach(([behaviourName, { requires = [] }]) => {
        behaviourRequires[behaviourName] = requires;
      });
  ...
  writeBootConfigFileToFileSystem() {
    ...
    this.bootConfig = {
      hasDynamicBootConfig: !!this.metadata.hasDynamicBootConfig,
  ...
  static ensureGlobalDepsFileExists() {
    const { getComponentsDistPath } = TemplatePreprocessor;

    const filePath = pathLib.join(getComponentsDistPath(), 'deps.json');

    if (fs.existsSync(filePath)) return;

    const { getBaseDependencies, getDependencies, getComponentAssetURLs } = global.AppContext;

    const toURL = (dep) => (typeof dep == 'object') ? dep.url : dep;

    const contents = new TextEncoder().encode(
      JSON.stringify({
        bootstrapUrls: ClientHtmlGenerator.getScriptURLs(),
        staticUrls: [
          ...getBaseDependencies().map(toURL),
          '/components/enums.json',
          ...getDependencies().map(toURL),
        ],
        bootConfigUrl: '/components/${assetId}/boot-config.json',
        assetUrls: Object.values(
  ...
  getDeclaredBaseComponent() {
    const { throwError, getSrcConfig, getSrcConfigFile } = TemplatePreprocessor;

    let { declaredBaseComponent } = this.metadata;

    if (declaredBaseComponent) {
      return declaredBaseComponent;
    }

    const { baseComponent } = getSrcConfig() || {};

    if (!baseComponent) {
      throwError(
        `No base component defined in ${pathLib.relative(process.cwd(), getSrcConfigFile())}`
      );
    }

    global.components[baseComponent];

    this.declaredBaseComponent = baseComponent
  ...
  static getAnyScalarComponent() {
    ...
    if (!scalarComponent) {
      throwError(
        `No scalar components were defined in ${pathLib.relative(process.cwd(), getSrcConfigFile())}`
      );
    }
  ...
  #getLoadableImplClasses(className) {
    return this.getChildClasses(className, false)
      .filter(n => !Object.keys(this.parents).includes(n))
      .filter(n => {
        global.components[n];
        return !this.getNonNullConfig(n).recursive;
      });
  }

  getChildClasses(className, includeAbstract, root = true) {
    const { classesInfo } = global;
    ...
    classesInfo[className].children.forEach(n => {
      if (classesInfo[n].isAbstract) {
        ...
        arr.push(...this.getChildClasses(n, includeAbstract, false));
      } else {
        arr.push(n);
      }
    });

    return root ? [...new Set(arr)] : arr;
  }
  ...
  generateClassesInfo() {
    ...
    for (const dirName of componentsFolder) {
      ...
      const cd = getMainClassDeclaration(filePath, ast);
      ...
      classesInfo[className] = {
        ...
        getResolverConfig: function () {
          var config = this._resolverCconfig;
          if (!config) {
            config = JSON.parse(fs.readFileSync(PathResolver.getConfigPath(this.dir), 'utf8'));
            this._resolverCconfig = config;
          }
          return config;
        },
        assetId,
  ...
  static getMainClassDeclaration(filePath, ast) {
    const { throwError } = TemplatePreprocessor;

    const classDeclarations = ast.body.filter(({ type }) => type === 'ClassDeclaration');
    const len = classDeclarations.length;

    if (len != 1) {
      throwError(`[${filePath}] Expected exactly 1 top-level class declaration, but found ${len}`);
    }
  ...
  loadComponentClass() {
    const { parseComponentClass } = TemplatePreprocessor;

    const { ComponentClass } = parseComponentClass({
      dir: this.srcDir,
      disableRequire: true,
    });
  ...
  finalize() {
    ...
    this.emitBehavioursSignature();

    this.emitEventsSignature();
  ...
  getConfig(className, ensureNonNull = false) {
    const { getComponentDistConfigPath, loadJson } = TemplatePreprocessor;
    ...
    config = loadJson(getComponentDistConfigPath(assetId));

    componentConfigs[className] = config;
    return config;
  }

  static getComponentDistConfigPath(assetId) {
    const { getComponentDistPath } = TemplatePreprocessor;

    return pathLib.join(
      getComponentDistPath(assetId), 'config.json'
    )
  }
  ...
  generateSchemaAndSamples() {
    ...
    const { parents: [parent], overridenProperties = [] } = this.metadata;
      ...
      parentSchema.definitions[parent].required
        .forEach(k => {
          if (schema.definitions[this.className].required.includes(k)) {
            if (!overridenProperties.includes(k)) {
        ...
        .forEach(([k, v]) => {
          ...
          if ((k === parent && !v.isComponentRef) || k == `${parent}${recursiveRootSuffix}`) {
            ...
            Object.entries(v.properties)
              .filter(([k]) => !overridenProperties.includes(k))
              .forEach(([k, v]) => {
  ...
  static mergeAst(componentSrc, ComponentClass, componentAst) {
    ...
    for (const definition of componentAst.body[0].body.body) {
  ...
  addDependencyAsset({ stmt, type }) {
    ...
    const groupValue = getHashValue({ stmt, key: 'group' });
  ...
  static writeMinifiedComponentJsFile({ data, fileName, assetId }) {
    const { getComponentDistPath } = TemplatePreprocessor;

    const distPath = getComponentDistPath(assetId);
    ...
    const { error, code, map } = UglifyJS.minify(
      ...
      },
      {
        sourceMap: {
          ...
          url: `/components/${assetId}/${minifiedFileName}.map`,
        },
        compress: true,
        mangle: true,
      });
    ...
    const indexFileContents = new TextEncoder().encode(
      `${code}\n//# sourceURL=/components/${assetId}/${minifiedFileName}`
    );
  ...
  serializeAst() {
    const {
      getTransientFields, toAstString, parseComponentClass,
    } = TemplatePreprocessor;
  ...
  emitEventsSignature() {
    const { getObjectValue } = TemplatePreprocessor;
    const { resolver: { config: { events = {} } } } = this;

    const o = {};

    Object.entries(events).forEach(([evtName, { parameters = {} }]) => {
      o[evtName] = Object.values(parameters);
    });

    this.wrapExpressionAsMethod({
      name: 'eventsSignature',
      returnExpression: getObjectValue(o, false),
      canMergeWithParent: true,
    });
  }

  emitBehavioursSignature() {
    const { getObjectValue } = TemplatePreprocessor;
    const { resolver: { config: { behaviours = {} } } } = this;

    const o = {};

    Object.entries(behaviours).forEach(([behaviourName, { parameters={} }]) => {
      o[behaviourName] = Object.values(parameters);
    });

    this.wrapExpressionAsMethod({
      name: 'behavioursSignature',
      returnExpression: getObjectValue(o, false),
      canMergeWithParent: true,
    });
  }
  ...
  getServerEvents() {
    const { component, metadata: { parents, isAbstract }, resolver: { config: { events = {} } } } = this;
    const parentsConfig = parents.map(p => this.getNonNullConfig(p));
    ...
    if (!isAbstract && !hasConcreteSuperclassInHierarchy) {
      component.getBaseEvents().forEach(evtName => {
        const { clientOnly } = events[evtName] || { clientOnly: false };
    ...
    component.getOwnEvents().forEach(evtName => {
      const { clientOnly } = events[evtName] || { clientOnly: false };
  ...
  static validatePathExpression(stmt) {
    const {
      pathSeparator, wordPattern, isRootCtxValue, throwError, getReservedProperties,
      getDataVariables,
    } = TemplatePreprocessor;
  ...
  static isResolvableInScope(pathInfo) {
    ...
    return onRoot || (path.length && !path.startsWith('@'));
  }
  ...
  resolvePathFromContext({
    ...
    const {
      ...
      objectType, componentRefType, literalType, appendSuffix, hasObjectPrefix, defaultIndexResolver, getTargetType,
      getLine, getSuffix, isResolvableInScope, throwCannotAccessIterateContextInAsync, getSampleValueForLiteralType,
      getOuterInlineBlock,
    } = TemplatePreprocessor;
    ...
    if (validType && validType != literalType) {
  ...
  static getValue(value, allowCodeGenObjects = true) {
    ...
    switch (true) {
      ...
      case value.constructor.name === 'Object':
        return allowCodeGenObjects && isCodeGenObject(value) ? value : getObjectValue(value);
      default:
        assert(value.constructor.name === 'Array');
  ...
  /**
   * This returns a list of well-known types. When generating a codegen ast object from an
  ...
  static getObjectValue(json, allowCodeGenObjects = true) {
    ...
    for (const k in json) {
      if ({}.hasOwnProperty.call(json, k)) {
        ...
        envelope.properties.push(
          getObjectValueProperty(k, v, allowCodeGenObjects)
        );
      }
    }
    return envelope;
  ...
  static getObjectValueProperty(k, v, allowCodeGenObjects = true) {
    const { isCodeGenObject, getValue, getScalarValue } = TemplatePreprocessor;
    return {
      type: 'Property',
      key: getScalarValue(k),
      computed: true,
      value: v && allowCodeGenObjects && isCodeGenObject(v) ? v : getValue(v, allowCodeGenObjects),
      kind: 'init',
      method: false,
      shorthand: false,
    };
  ...
  #addToRenderTree(className) {
    ...
    if (!global.classesInfo[className]) {
      this.throwError(`Unknown component class "${className}"`);
    }
    ...
    if (bootConfig.hasDynamicBootConfig) {
      this.metadata.hasDynamicBootConfig = true;
    }
    ...
    for (let i = paramList.length - 1; i >= 0; i--) {
      ...
      switch (true) {
        ...
        case param.type === 'SubExpression':
          ...
          if (this.getSubExpressionMetaPaths().includes(invokeMethodName)) {
            this.visitSubExpressionMetaStatement(param, bindParents);
          }
  ...
  getSyntheticMethodValue({ stmt, source, method }) {
    const { throwError, getLine } = TemplatePreprocessor;
    const { invokeSyntheticMethodsOnCompile } = this.resolver.config;

    if (!invokeSyntheticMethodsOnCompile) {
      return;
    }
  ...
  #getIterateBlockFactor(outerIterateBlocks) {
    const { dataPathPrefixRegex, pathSeparator, dataPathRoot, defaultIndexResolver } = TemplatePreprocessor;
    ...
    outerIterateBlocks.forEach(path => {
      ...
      if (path.match(dataPathPrefixRegex)) {
        const p = clientUtils.toCanonicalPath(
          this.component.getExecPath({
            fqPath: path
              .replace(`${dataPathRoot}${pathSeparator}`, ''),
            indexResolver: defaultIndexResolver,
          })
  ...
  process0({ contextList, bindParents, program, validateHtml }) {
    ...
    ASTParser.prototype.SubExpression = function (stmt) {
      ...
      if (_this.getSubExpressionMetaPaths().includes(stmt.path.original)) {
        _this.visitSubExpressionMetaStatement(stmt, bindParents);
      }
        ...
        case variableHelperName:
          ...
          (() => {
            ...
            const hashPairs = getHashPairs(hash)
              ...
              .filter(({ value: { processed } }) => !processed);
    ...
    const getPartial = ({ ctxList, partialName, stmt }) => {
      ...
      const getKnownPartialOrigins = () => {
        ...
        addOrigin({
          dir: pathLib.join(process.cwd(), 'src', 'partials'),
        });
    ...
    const processPartial = function ({ stmt }) {
      const { partialDeferHashKey, getSuffix, throwError, getProgramLocRange } = TemplatePreprocessor;
    ...
    ASTParser.prototype.DecoratorBlock = function (stmt) {
      ...
      if (runtime && !fromMarker) {
        ...
        if (paramCount) {
          ...
          _this.throwError(
            `Runtime decorator block "${decoratorName}" should not have any parameters`,
            stmt
          )
        }
  ...
  getSubExpressionMetaPaths() {
    const { fnHelperName } = TemplatePreprocessor;
    return [fnHelperName];
  };

  visitSubExpressionMetaStatement(stmt, bindParents) {
    const { fnHelperName, throwError } = TemplatePreprocessor;
    ...
    switch (stmt.path.original) {
      case fnHelperName:
        ...
        if (type != 'StringLiteral') {
          throwError(`Target method "${original}" must be expressed as a string literal`, stmt);
        }

        const isValidMethod = this.methodNames.includes(original);

        if (!isValidMethod) {
          throwError(`Unknown methodName: "${original}"`, stmt);
        }

        this.addGlobalHelper(original, bindParents);
        break;
    }
  };
  ...
  addInlineBlock({
    ...
    const { visitNodes, registerProgram } = TemplatePreprocessor;
  ...
  static addComponentClassGlobals() {
  ...
  static addGlobals() {
    const { createJsDom, getSrcConfig, addComponentClassGlobals, addCssEscapePolyfill } = TemplatePreprocessor;

    addComponentClassGlobals();
    ...
    global.nodeInspect = util.inspect;
    global.nodeCustomInspect = util.inspect.custom;

    addCssEscapePolyfill();
  }

  static addCssEscapePolyfill() {
    global.CSS = global.CSS || {};
    global.CSS.escape = global.CSS.escape || function (identifier) {
      ...
      // Define characters that need escaping with backslash
      const needsEscaping = /[!"#$%&'()*+,./:;<=>?@[\\\]^`{|}~]/;
      ...
      for (let i = 0; i < identifier.length; i++) {
        ...
        // Handle control characters, delete, digits at start, etc.
        else if ((code >= 0x01 && code <= 0x1F) || code === 0x7F ||
          (i === 0 && /[0-9]/.test(char)) ||
          (i === 1 && /[0-9]/.test(char) && identifier.charAt(0) === '-')) {
          result += '\\' + code.toString(16) + ' ';
        }
        // Handle hyphen at start or alone
        else if ((i === 0 || (i === 1 && identifier.charAt(0) === '\\')) && char === '-' ||
          (char === '-' && i === 0 && identifier.length === 1)) {
          result += '\\' + char;
        }
        // Handle regular characters that need escaping
        else if (needsEscaping.test(char)) {
          result += '\\' + char;
        }
        // Normal character
        else {
