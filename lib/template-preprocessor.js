...
class TemplatePreprocessor {
  ...
  process0({
    ...
    const processComponentImport = ({ stmt }) => {
      ...
      const { typeProperty, nameProperty, componentRefType, literalType } = PathResolver;
      ...
      const v = this.resolver.getPathInfo(path);

      if (
        !!v &&
        // literalTypes are transitive types
        (v[typeProperty] != literalType)
      ) {
        if (
          // The current context has a similar alias, which is fine because the data model
          // is consistent.
          v[typeProperty] == componentRefType &&
          v[nameProperty] == className &&
          this.resolver.data[path]) {
        } else {
          throw Error(`Alias name ${alias} already exists in the current context`);
        }
      }

      this.resolver.resolve({ path: `${path}%${componentRefType}/${className}` });
    ...
    ASTParser.prototype.PartialStatement = function (stmt) {
      ...
      if (!isComponentClass && !isCustomContext()) {
        ...
        const path = _this.component.getExecPath({
          fqPath: fqPath.replace(/^data__/g, ''),
          indexResolver: defaultIndexResolver,
        });
