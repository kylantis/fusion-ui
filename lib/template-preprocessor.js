...
class TemplatePreprocessor {
  ...
  static createHtmlWrapperHashKey = 'createHtmlWrapper';
  ...
  static fetchDistComponentList(verify = true) {
    ...
    const filePath = pathLib.join(distPath, 'list.json');
  ...
  suggestFixSequelToComponentRename() {
    // Todo: do not hardcode "dist/components/list.json"
    this.logger.warn(
      `Did you rename the source folder for "${this.className}"? If so, remove "${this.className}" from dist/components/list.json, and recompile "${this.className}"`
    );
  }

  updateComponentList() {
    const { getComponentsDistPath, fetchDistComponentList } = TemplatePreprocessor;
    ...
    fs.writeFileSync(
      pathLib.join(getComponentsDistPath(), 'list.json'),
  ...
  simulateAppContext() {
    const {
      getComponentClass, getComponentsDistPath, getAppContextObject,
    } = TemplatePreprocessor;
    ...
    Object.entries(this.metadata.distComponentList)
      .forEach(([className, assetId]) => {
        ...
        const testComponentClass = getComponentClass({
          dir,
          useTestClass: true,
        }).ComponentClass;
        ...
        testComponentClass.schema = this.getConfig(className).schema;
  ...
  getTransientComponentsGlobal() {
    const {
      enableComponentClassCaching, getComponentsSrcPath, getComponentsDistPath,
      getComponentClass, getSkipFile,
    } = TemplatePreprocessor;
    ...
    return new Proxy({}, {
      get: (obj, prop) => {
        ...
        if (Object.keys(this.parents).includes(prop)) {
          throw new CircularDependencyError(prop, this.className);
        }
  ...
  writeClassesInfoToFileSystem() {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify(global.classesInfo, null, 2),
    );
  }
  ...
  finalize() {
    ...
    // we need to serialize ast here because it is required for "canMergeWithParent" to work
    this.serializeAst();
  ...
  writeMinifiedComponentJsFile({ data, fileName }) {
    ...
    // Integrate Babel here, also also update App Context => scope ? function : eval... support amd
  ...
  hasDeclaredAssetId() {
    ...
    const assetId = this.component.getAssetId();

    if (assetId == this.component.getAssetId0()) {
      return null;
    }
    ...
    if (this.getAssetIdFromClassName(parent) != assetId) {
      this.throwError(`getAssetId() has a wrong method signature`);
    }
  ...
  resolvePathFromContext({
    ...
    const {
      rootQualifier, pathSeparator, dataPathPrefixRegex, dataPathRoot, syntheticMethodPrefix,
        ...
        for (const k of contextKeys) {
          ...
          if (
            ...
            if (type.endsWith('Literal')) {
              ...
              canonicalSource = original.replace(k, v.canonicalSource);
            }
    ...
    if (!type) {
      ...
      if (contextObject[rootQualifier].synthetic) {
        ...
        canonicalSource = original.split(pathSeparator).join('.');
  ...
  wrapExpressionAsMethod({
    ...
    if (returnExpression) {
      ...
      if (canMergeWithParent && this.hasDeclaredAssetId()) {
  ...
  addParameterizedExpression({
    ...
    if (stmt && !stmt.canonicalSource) {
      stmt.canonicalSource = stringifyHandlebarsStatement(stmt);
    }
  ...
  static createSubExpressionFromPath({ stmt }) {
    ...
    const { original, loc = this.getDefaultLoc() } = stmt;
  ...
  static createMustacheStatement({ original, hash, params = [], loc }) {
    ...
    return {
      ...
      path: {
        ...createPathExpression({ original }), loc,
        processed: true,
      },
      params,
      hash,
  ...
  getBlockParam(stmt, index) {
    ...
    if (blockParam && this.methodNames.includes(blockParam)
      ...
      throw Error(
        `The blockParam '${blockParam}' already exists as a named method`,
      );
    }
  ...
  getBlockOptions(stmt) {
    const {
      ...
      createPathExpression, throwError,
    } = TemplatePreprocessor;
  ...
  static getReference(stmt) {
    const { reference, getLine } = TemplatePreprocessor;
    return `${`${reference}${stmt && stmt.loc ? ` ${getLine(stmt)}` : ''}`}`;
  }
  ...
  static throwError(err, stmt) {
    const { getReference } = TemplatePreprocessor;
    if (typeof err == 'string') {
      throw Error(`[${getReference(stmt)}] ${err}`);
    } else {
      assert(err.constructor.name.endsWith('Error'));
      throw err;
    }
  }
  ...
  getSyntheticMethodValue({ stmt, source, method, validType, line }) {
    const { throwError, getReference } = TemplatePreprocessor;
    ...
    try {
      ...
      this.logger.error(
        `[${getReference(stmt)}] Exception thrown while executing => ${source}`
      );
      throwError(e);
    }
  ...
  addDataBindTransformations(ast) {
    ...
    const {
      customEachHelperName, startTNBCtxHelperName, startAttrCtxHelperName, endAttrCtxHelperName,
      createContentStatement, createMustacheStatement, getLine, htmlWrapperCssClassname
    } = TemplatePreprocessor;
  ...
  process0({
    ...
    const getHook = ({ stmt }) => {
      const { hookHashKey, hookOrderHashKey } = TemplatePreprocessor;
      ...
      if (hookMethod && !this.methodNames.includes(hookMethod)) {
        throw Error(`Unknown hook method: ${hookMethod}, ${getLine(stmt)}`);
      }
    ...
    ASTParser.prototype.SubExpression = function (stmt) {
      ...
      const { resolveMustacheInCustomHelperName, isRootCtxValue, stringifyHandlebarsStatement } = TemplatePreprocessor;
    ...
    ASTParser.prototype.BlockStatement = function (stmt) {
      ...
      const {
        globalsBasePath, createHtmlWrapperHashKey, getPathFromSyntheticAlias, getChildPathFromSyntheticAlias,
        getTargetType, isLookupAllowed, throwError, addBlockParamHashKey, getIterateDataVariables, invokeDataPathTransform,
        isRootCtxValue, getReservedBlockHashKeys, getReference,
      } = TemplatePreprocessor;
      ...
      const containsHtmlError = () => {
        ...
        if (htmlErrors.length) {
          ...
          if (_this.canPrintHtmlWarnings()) {
            _this.logger.warn(`[${getReference(stmt)}] Disabled data-binding for #${stmt.path.original} block`);
          }

          if (hook) {
            throwError(
              `A hook cannot be specified for this block due to invalid html markup`,
              stmt,
            );
          }
      ...
      const addCustomBlockCtx = () => {
        ...
        if (async) {
          ...
          if (!stmt.syntheticNodeId) {
            throwError(
              'Block must be wrapped in an html wrapper inorder to be marked as "async". Does the block contain valid html markup?',
              stmt
            )
          }
        }
      ...
      if (!stmt.visited) {
        ...
        const { original: canCreateHtmlWrapper } = getHashValue({
          stmt, key: createHtmlWrapperHashKey, type: 'BooleanLiteral', cleanup: true,
        })
          || { original: true };


        if (canCreateHtmlWrapper && !containsHtmlError()) {
      ...
      const resolvePathParam = ({ stmt }) => {
        ...
        if (isSynthetic) {
          ...
          canonicalSource = stmt.canonicalSource || stringifyHandlebarsStatement(stmt);
    ...
    const processComponentImport = ({ stmt }) => {
      ...
      // The alias should not be a context key
      contextList.forEach((ctxObject) => {
        if (Object.keys(ctxObject).includes(alias)) {
          this.throwError(
            `Alias "${alias}" cannot be used because it is a qualifier in the current context`,
            stmt
          );
        }
      });
    ...
    ASTParser.prototype.MustacheStatement = function (stmt) {
      ...
      const { loadComponentHelperName, getReservedMustacheHashKeys, isRootCtxValue } = TemplatePreprocessor;
    ...
    const getPartialContextList = function ({ stmt }) {
      ...
      const { stringifyHandlebarsStatement } = TemplatePreprocessor;
      const uniqueKeys = getUniqueContextKeys();
      ...
      if (isCustomContext()) {
        ...
        for (const pair of hash.pairs) {
          ...
          contextObject[pair.key] = {
            ...
            asVariable: true,
            index: ctxIndex,
            canonicalSource: pair.value.canonicalSource || stringifyHandlebarsStatement(pair.value),
          };
        }
      } else {
    ...
    const processPartial = function ({ stmt }) {
      ...
      // Recurse partial ast
      const T = new TemplatePreprocessor({
        srcDir: _this.srcDir,
      ...
      const { reference: _reference } = TemplatePreprocessor;
      TemplatePreprocessor.reference = `${_reference}/${partialName}`;

      T.process();

      TemplatePreprocessor.reference = _reference;
    ...
    ASTParser.prototype.PartialStatement = function (stmt) {
      ...
      const {
        loadComponentHelperName, wordPattern, getReservedPartialHashKeys, throwError,
      } = TemplatePreprocessor;
      const { componentRefType } = PathResolver;
      ...
      if (stmt.params.length) {
        ...
        for (let i = 0; i < stmt.params.length; i++) {
          ...
          if (!param.original.match(wordPattern)) {
            throwError(`PartialStatements should not have params`, param);
          }
    ...
    ASTParser.prototype.DecoratorBlock = function (stmt) {
      const {
        reservedDecoratorNames, allowOverlappingQualifiersInDecorator, isConditionalParticipant, getHashValue,
      } = TemplatePreprocessor;

      const { original: decoratorName } = stmt.params.shift();

      // Note: runtime option is only available on the root context because these decorators are executed as a high-level
      // handlebars functions and uses the rootProxy as the context
      const { original: runtime } = isCustomContext() ? {} : getHashValue({ stmt, key: 'runtime', type: 'BooleanLiteral', cleanup: true }) || {};

      if (reservedDecoratorNames.includes(decoratorName)) {
        throw Error(`Decorator name: ${decoratorName} is reserved`);
      }
      ...
      if (runtime) {
        ...
        if ([...requiredParams, ...optionalParams].length) {
          ...
          throwError(
            `Runtime decorator "${decoratorName}" should not have any inline parameters`,
            stmt
          )
        }
  ...
  addInlineBlock({
    bindParents, decoratorBindParents, decoratorName, stmt, requiredParams, optionalParams, isCustomCtx, runtime,
  }) {
    const {
  ...
  static getMetaHelpers() {
    const {
      ...
      resolveMustacheInRootHelperName, fnHelperName, resolveMustacheInCustomHelperName,
    } = TemplatePreprocessor;
    return [
      storeContextBlockName, loadContextBlockName, customEachHelperName, conditionalHelperName,
      startAttrCtxHelperName, endAttrCtxHelperName, startTNBCtxHelperName, setSyntheticNodeIdHelperName,
  ...
  static getJsDomVirtualConsole({ errors }) {
    ...
    virtualConsole.on("jsdomError", ({ message, detail }) => {
      ...
      if (!knownDocumentErrors.includes(message)) {
        // We don't throw the error here, but instead put it inside the array that is provided 
        // by the caller for later processing
