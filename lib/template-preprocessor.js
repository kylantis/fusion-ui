...
class TemplatePreprocessor {
  ...
  static dataPathPrefix = RegExp(`^${this.dataPathRoot}${this.pathSeparator}`);
  ...
  static partialUnsafeHashKey = 'unsafe';
  ...
  // Note: If this is set to false, we also have to remove some assertions. For example,
  // after PathExpression visitaion in MustacheStatement, we assert that it resolved to a
  // PathExpression. If this is false, the visited PathExpression will resolve to a literal, 
  // hence the the assertion will fail
  ...
  static allowOverlappingQualifiersInDecorator = true;
  ...
  updateComponentList() {
    ...
    if (!assetId) {
      ...
      this.logger.warn(
        `Did you rename the folder for ${this.className}. If so, remove ${this.className} from dist/components/list.json, and recompile ${this.className}`
      );
    ...
    for (const className of keys) {
      ...
      try {
        ...
        if (e.message.includes('Class extends value undefined is not a constructor or null')) {
          // The parent component of <className> could not be loaded
          let msg = `The parent component of ${className} could not be loaded.`;

          const srcInfo = this.metadata.componentSources[className];
          msg += (
            srcInfo ?
              ` To fix, first remove ${className} from dist/components/list.json, then re-compile ${srcInfo.parents[0].name}`
              : ''
          );
      ...
      testComponentClass.schema = getConfig(className).schema;
  ...
  writeConfigFileToFileSystem() {
    ...
    const { getConfig } = TemplatePreprocessor;
    const { parents: [parent] } = this.metadata.componentSources[this.className];

    let scalars = this.resolver.getScalars();

    if (parent) {
      const { scalars: parentScalars } = getConfig(parent.name);
      scalars = {
        ...scalars,
        ...parentScalars,
      }
    ...
    fs.writeFileSync(
      pathLib
        .join(process.env.PWD, 'dist', 'components', this.assetId, 'config.json'),
      JSON.stringify({
        scalars,
  ...
  getSerializedComponent(className) {
    ...
    if (className === 'BaseComponent') {
      ...
      if (componentSource.isAbstract) {
        ...
        // Todo: show this component be marked as recursive? since there is possiblity for it to load itself
      }
    }
  ...
  process() {
    ...
    if (!this.component) {
      ...
      // Compile parents that have not yet been compiled. The reason this needs to be done eagerly
      // is because PathResolver.resolvePathFromParent() attempts to read the  parent config, hence
      // the parent need to have been compiled before then
      parents.forEach(p => global.components[p.name]);
    ...
    try {
      ...
      const pruneComponent = (className, assetId) => {
        ...
        this.logger.warn(`Pruning ${className}`);


        // ONLY REMOVE WELL KNOWN FILES, NOT THE ENTIRE FOLDER!!!. 
        ...
        // Note: If an error occurs while an inline partial is being inlined, this
  ...
  loadComponentSources() {
    ...
    for (const dirName of utils.getAllComponentNames()) {
      ...
      try {
        ...
        if (e.message.includes('Class extends value undefined is not a constructor or null')) {
          this.logger.warn(`Verify that parent component of ${dirName} is loaded before it. See components/config.js`);
        }
        throw e;
      }
  ...
  getComponentClassNames() {
    return Object.keys(this.metadata.componentSources);
  }
  ...
  getComponentConcreteSubclasses(clazz) {
    ...
    Object.entries(this.metadata.componentSources)
      .forEach(([className, { parents, isAbstract }]) => {
        if (parents.length && parents[0].name == clazz && !isAbstract) {
          arr.push(className);
        }
      })
  ...
  static getConfig(className) {
    const { getComponentList } = TemplatePreprocessor;
    const assetId = getComponentList()[className];
    ...
    return JSON.parse(fs.readFileSync(
      pathLib.join(
        process.env.PWD, 'dist', 'components',
        assetId, 'config.json'
      )
    ))
  }
  ...
  generateSchemaAndSamples() {
    ...
    const {
      getComponentsDistPath, getAssetIdFromClassName, getConfig
    } = TemplatePreprocessor;
    ...
    if (parent) {
      ...
      const { recursive, schema: parentSchema } = getConfig(parent.name);
  ...
  static getComponentClass({
    ...
    let data0 = filePath ? fs.readFileSync(
      filePath,
      'utf8',
    ) : data;
    ...
    // Remove browser file mappings
    data0 = data0.replace(RegExp(
      `${clientUtils.escapeRegExp('//# sourceURL=')}(.)+\\.js$`
    ), '');
  ...
  static toAstString(ast) {
    ...
    // We need to transform the ast to make up for shortcomings in the 
    // code generation library: astring
    acornWalk.simple(
      ast, {
  ...
  getComponentJsOutputFiles({ sourceURL }) {
    ...
    if (sourceURL) {
      // .info find a lasting solution to this issue
  ...
  emitAllowedPaths() {
    this.emitMethodReturningArray({ name: 'allowedPaths' });
  }
  ...
  static validatePathExpression(stmt) {
    const {
      pathSeparator, isRootCtxValue, getLine, throwError, getReservedProperties,
    } = TemplatePreprocessor;
    ...
    if (
      original.includes(pathSeparator) ||
      isRootCtxValue(original)
    ) {
      throw Error(`Invalid PathExpression: ${original}, ${getLine(stmt)}`);
    }
    ...
    parts.forEach((p, i) => {
      if (!p.match(wordPattern) && !p.match(indexPattern)) {
  ...
  static getReservedProperties() {
    const { globalsBasePath } = TemplatePreprocessor;
    return [
      // Todo: add a comment on why this is reserved
      'helpers',
      // <dataPathPrefix>__<globalsBasePath> is exclusively used to resolve global variables
      globalsBasePath
    ]
  }
  ...
  static hasObjectPrefix({ stmt, value, key, rangeAllowed = true }) {
    ...
    if (isRange && !rangeAllowed) {
      throwError(`Property: ${value} not allowed`, stmt);
    }
  ...
  // Todo: verify that this will be processed properly: {{#each [people]}}.
  // Note: [] is parsed into ''
  static getPathInfo({ original, contextList, stmt }) {
    ...
    if (isDataVariable) {
      ...
      const dataVariableName = original.split('.')[0];

      if (dataVariableName === '@root') {
      ...
      } else if (!getDataVariables().includes(dataVariableName)) {
      ...
      } else if (hasOffset || !original.length || startsWithIndex || forceUseOfScopeQualifier) {
        ...
        const contextConsumer = startsWithIndex ?
          ({ targetType }) => {
            ...
            if (targetType && targetType != 'Array') {
              throw Error(`Only an array-based scope can be accessed with an index, ${getLine(stmt)}`);
            }
          } :
          null;
        ...
        if (hasOffset) {
          ...
          for (let i = index + 1; i <= contextList.length - 1; i++) {
            ...
            if (Object.keys(ctx).includes(scopeQualifier)) {
              const lineX = getLine(ctx[scopeQualifier].stmt);
              const lineY = getLine(contextList[index][scopeQualifier].stmt);
  ...
  static getRootOriginalFromPathInfo({ path, index }) {
    switch (true) {
  ...
  static resolveGlobalVariable(pathInfo) {
  ...
  resolvePathFromContext({
    contextList, contextIndex, hasOffset, original, validTypes, stmt, syntheticAlias, nameQualifier, create,
  }) {
    const {
      rootQualifier, pathSeparator: separator, dataPathPrefix, dataPathRoot, syntheticMethodPrefix,
      trimObjectPath, hasDataPathFormat, hasObjectPrefix, defaultIndexResolver, getTargetType, getLine,
      getSuffix
    } = TemplatePreprocessor;

    assert(validTypes.length === 1);

    const canonicalSource = original;
    ...
    const isPurePath = original.startsWith('@')
      || (contextIndex === contextList.length - 1 && !hasOffset);
        ...
        for (const k of contextKeys) {
          ...
          if (
            ...
            if (!contextObject[rootQualifier]) {
              assert(Object.keys(v).includes('lookup'));

              return { terminate: true };
            }

            if (asyncContext && contextObject[rootQualifier].inIterateContext) {
              throw Error(`${canonicalSource} is unreachable, line ${getLine(stmt)}`);
            }
            ...
            if (type.endsWith('Literal')) {
              ...
              // Set <checkType> to false because <checkType> only applies to PathExpressions
              // who values we do not yet know, and in which case we want to validate. But in
              // the case of literals, the caller is responsible for performing validating.
              checkType = false;
              ...
              const p = trimObjectPath({
                value: original,
                repl: v.value,
              });

              original = p.split('.').join(separator);
            }
    ...
    if (!type) {
      ...
      if (hasObjectPrefix({ stmt, value: original, key: '@root' })) {
        ...
        // hasOffset - Use pathInfo.isDataVariable
      ...
      if (contextObject[rootQualifier].synthetic) {
        ...
        const r = this.resolveSyntheticContext({
          k: rootQualifier,
          v: contextObject[rootQualifier],
          original: `${rootQualifier}.${original}`,
          syntheticAlias,
          stmt,
        });
    ...
    // If applicable, we will then attempt to lookup the original path to verify the value matches
    ...
    if (checkType) {
      ...
      switch (true) {
        ...
        case original == `${dataPathRoot}${separator}`:
          value = {};

          this.component.validateType({
            path: stmt ? stmt.original : '@root',
            value,
            validTypes,
            line: stmt ? getLine(stmt) : null,
          })
        ...
        default:
          ...
          const typeSuffix = `%${validTypes[0]}${nameQualifier ? `/${nameQualifier}` : ''}`;
          const fqPath = `${original.replace(dataPathPrefix, '')}${typeSuffix}`;
          ...
          if (!create) {
            // <fqPath> is not added to the data model by our resolver, hence we need to validate
            // the return value
            this.component.validateType({
              path: stmt ? `${stmt.original}${typeSuffix}` : fqPath,
              value,
              validTypes,
              line: stmt ? getLine(stmt) : null,
            })
  ...
  // {{this.[0]..x.[0].[0]}}
  // {{this.[0]x.[0].[0]}}
  // Todo: The above paths works - find out what they are and implement accordingly
  resolvePath({
    bindParents, stmt, contextList, value, validTypes, create = true,
    syntheticAlias, nameQualifier,
  }) {
    const {
      dataPathRoot, pathSeparator, inlineParameterPrefix, literalType, globalsBasePath,
      resetPathExpression, getOuterInlineBlock, getPathInfo, getRootOriginalFromPathInfo,
      throwError,
    } = TemplatePreprocessor;

    const P_EXPR = 'PathExpression';

    if (!validTypes) {
      validTypes = stmt.validTypes || [literalType];
    }
    ...
    if (stmt != null && inlineBlock != null) {
      ...
      const { requiredParams, optionalParams } = inlineBlock;
      ...
      switch (true) {
        ...
        case [...requiredParams, ...optionalParams].includes(param):
    ...
    let synthetic, targetType, targetValue, canonicalSource, terminate;
    ...
    if (type === P_EXPR) {
      ...
      // First, attempt to resolve as global variable
      ...
      const rootOriginal = getRootOriginalFromPathInfo(pathInfo);
      ...
      targetValue = resolvedPath.targetValue;
      // eslint-disable-next-line prefer-destructuring
      synthetic = resolvedPath.synthetic;
      // eslint-disable-next-line prefer-destructuring
      canonicalSource = resolvedPath.canonicalSource;
      // eslint-disable-next-line prefer-destructuring
    ...
    if (type && type.endsWith('Literal')) {
      if (validTypes.length > 0 && !validTypes.includes('Literal')) {
        throwError(
          `Path: ${prevOriginal} cannot resolve to a Literal value, validTypes=[${validTypes}]`,
          stmt,
        );
      }
    }
  ...
  static isCodeGenObject(value) {
    ...
    return value.constructor.name === 'Object' &&
      ...
      !getKnownTypes().includes(value.type);
  }
  ...
  addParameterizedExpression({
    ...
    const {
      ...
      isRootCtxValue,
      getValue,
      onSubExpressionProcessed,
      getPathFromSyntheticAlias,
    } = TemplatePreprocessor;
        ...
        case param.type === 'PathExpression':
          ...
          if (this.methodNames.includes(param.original)) {
            ...
            if (path.type.endsWith('Literal')) {
              ...
              if (param.transformFn) {
                path.original = param.transformFn(path.original);
              }
          ...
          if (this.methodNames.includes(value.original)) {
            ...
            if (path.type.endsWith('Literal')) {
              ...
              if (value.transformFn) {
                path.original = value.transformFn(path.original);
              }
  ...
  static getPreservedPathExpressionAttibutes() {
    return [
      'loc', 'participantType', 'transformFn', 'isLogicGateCondition', 'rootProxyIndirection',
    ];
  }

  static resetPathExpression({ stmt, original, properties = {} }) {
    const { getPreservedPathExpressionAttibutes } = TemplatePreprocessor;
  ...
  createSubExpression({
    ...
    hash, syntheticAlias, stmt, syntheticNodeId,
  }) {
  ...
  static wrapInTryCatchBlockForLeniency(expr) {
  ...
  createRootProxyIndirection({ stmt, canonicalSource, path, suffix = false, syntheticAlias }) {
    ...
    const synthethicMethodName = createRootProxyIndirection0({
      stmt, canonicalSource, path, suffix, syntheticAlias
    });

    stmt.rootProxyIndirection = true;
  ...
  createRootProxyIndirection0({
    stmt, canonicalSource, path, suffix, syntheticAlias,
  }) {
    const {
      getCallExpression, getMethodFromFunctionDeclaration, getScalarValue, getValue,
      getPathFromSyntheticAlias, isConditionalParticipant, wrapInTryCatchBlockForLeniency,
      createArrowFunctionExpression, getProxyStatement, throwMethodInvocationCannotBeConditional,
    } = TemplatePreprocessor;

    if (!suffix && isConditionalParticipant(stmt)) {
      throwMethodInvocationCannotBeConditional(stmt.original, stmt.loc);
    }
        ...
        declarations: [
          {
            ...
            init: getScalarValue(
              this.component.processLiteralSegment(`${valueVariableName}.${suffix}`),
            ),
          },
        ],
        kind: 'const',
          ...
          argument: getCallExpression({
            ...
            args: [{
              ...
              fn: createArrowFunctionExpression({
                body: [
                  isConditionalParticipant(stmt) ?
                    wrapInTryCatchBlockForLeniency(
                      getExecStatement()
                    ) :
                    {
                      type: 'ReturnStatement',
      ...
      for (let i = 0; i < pairs.length; i++) {
        ...
        if (pair.key === key) {
          ...
          if (type && pair.value.type !== type) {
            throwError(`Expected hash value of type '${type}' for key '${key}'`, stmt);
          }
          return pair.value;
        }
      }
  ...
  createCustomBlockPath({
    ...
    const { async, syntheticNodeId } = stmt;
    assert(syntheticNodeId || !async);

    const ast = (
      async
        ? this.getCustomBlockFuntionAsync : this.getCustomBlockFuntion
    )({ methodName });
  ...
  getCustomBlockFuntionAsync({ methodName }) {
    const {
      validateTypeMethodName, renderBlockMethodName, renderMethodName, getLoaderMethodName,
      getSyntheticNodeIdMethodName, getIdentifier, createPromise, getCallExpression, getValue,
      getOptionsRetrievalStatements, getVariableEnvelope, getShorthandObjectValue,
    } = TemplatePreprocessor;
    return {
      type: 'FunctionDeclaration',
      id: {
  ...
  // eslint-disable-next-line class-methods-use-this
  getCustomBlockFuntion({ methodName }) {
    const {
      validateTypeMethodName, renderBlockMethodName, getIdentifier,
      getCallExpression, getValue, getOptionsRetrievalStatements,
  ...
  getBlockOptions(stmt) {
    const {
      ...
      getLine, ensureNoLiteralParam0, getReservedBlockHashKeys, throwError,
    } = TemplatePreprocessor;
    ...
    // Validate hashKeys
    if (stmt.hash) {
      stmt.hash.pairs.forEach(({ key }) => {
        if (getReservedBlockHashKeys().includes(key)) {
          throwError(
            `The hash key "${key}" is reserved and cannot be used in this ${blockName} block`,
            stmt
          );
        }
      });
  ...
  static throwError(msg, stmt) {
    const { getLine } = TemplatePreprocessor;
    throw Error(`[${getLine(stmt)}] ${msg}`);
  }

  getSyntheticMethodValue({
    stmt, path, method, validTypes = [], line, rewriteErrorMsg = true
  }) {
    const { throwError } = TemplatePreprocessor;
    ...
    try {
      ...
      if (rewriteErrorMsg) {
        throwError(
          `Exception thrown while executing "${path}": ${e.message}`,
          stmt
        );
      } else {
        throw e;
  ...
  static isRootCtxValue(value) {
    const {
      syntheticMethodPrefix, logicGatePathRoot, pathSeparator, hasDataPathFormat
    } = TemplatePreprocessor;
    return hasDataPathFormat(value) || value.startsWith(syntheticMethodPrefix) ||
      value.startsWith(`${logicGatePathRoot}${pathSeparator}`);
  }
  ...
  static isLookupAllowed(targetType) {
    ...
    return [arrayType, objectType].includes(targetType);
  }
  ...
  static toPathExpressionLiteral(value) {
    const {
      dataPathRoot, pathSeparator, literalPrefix
    } = TemplatePreprocessor;
    return `${dataPathRoot}${pathSeparator}${literalPrefix}${value}`
  }

  static throwMethodInvocationCannotBeConditional(original, loc) {
    const { getLine } = TemplatePreprocessor;
    throw Error(
      `{{${original}} cannot have a conditional clause because it is a method invocation, ${getLine({ loc })}`
    );
  }
  ...
  /**
   *  Terminal transform function for processed path expressions on the root context
   */
  pathExpressionTerminalTransform(original, stmt) {

    const {
      hasDataPathFormat, isRootCtxValue, PARTICIPANT_TYPE_CONDITIONAL,
    } = TemplatePreprocessor;
  ...
  visitPathExpressions(ast) {
    ...
    visitNodes({
      ...
      ast,
      consumer: ({ stmt }) => {
        const { processed, original, loc } = stmt;
        ...
        if (!processed) {
          ...
          if (isSubExpression && haslenientMarker) {
            throwMethodInvocationCannotBeConditional(original, loc);
          }
          ...
          // Add transformFn
          stmt.transformFn = (original) => this.pathExpressionTerminalTransform(original, stmt)
        }
      },
    });
  }
  ...
  process0({
    ...
    const {
      ...
      stringifyHandlebarsStatement,
      throwError,
    } = TemplatePreprocessor;

    this.visitPathExpressions(ast);
    ...
    const acceptPathExpressionInCustomCtx = ({ stmt, visitor }) => {
      ...
      if (prev.startsWith(inlineParameterPrefix)) {
        throw Error(`Inline parameter prefixes are not allowed in custom context: ${prev.split('.')[0]}`);
      }
      ...
      let { index, path: original, hasOffset } = pathInfo;
      ...
      if (original.split('.')[0] === '@root') {
        // We want to transform @root to reference the nearest custom context scope qualifier
            ...
            if (hasObjectPrefix({
              ...
              if (!contextObject[rootQualifier]) {
                ...
                if (v.asVariable) {
                  ...
                  // be processed later.
                  // 2. an index qualifier for a context-switching block, in which case it will
                  // be resolved at runtime using the "blockParam" mechanism
                }
              ...
              if (v.type.endsWith('Literal')) {
                ...
                original = this.resolveSyntheticContext({ k, v, original, stmt }).original;
                ...
                original = trimObjectPath({
                  value: this.component.processLiteralSegment({ original }),
                  repl: v.value,
                });
                ...
                original = stmt.transformFn ? stmt.transformFn(original) : original;
      ...
      if (contextObject[rootQualifier]) {
        ...
        if (!allowRootAccess()) {
          throwError(`Root access is not permitted, path: ${prev}`, stmt);
        }

        if (asyncContext && contextObject[rootQualifier].inIterateContext) {
          throwError(`${prev} is unreachable`, stmt);
        }
      ...
      if (stmt.forRoot && getOuterInlineBlock({ bindParents }) == null && !stmt.processed) {
        throwError(`Could not resolve root path: ${prev}`, stmt);
      }
    ...
    const acceptPathExpressionInRootCtx = ({ stmt }) => {
      ...
      const { getDataVariables, toPathExpressionLiteral } = TemplatePreprocessor;
    ...
    const createHtmlWrapper = ({ stmt, tagName = 'div', addStencilClass = true, force }) => {
      ...
      if (canCreateHtmlWrapper()) {
        ...
        // Note: If we are inside an inline block, we need to indicate the inline block as the context, 
        // inorder for replacements to be done (post inline block processing)
        const inlineBlock = getOuterInlineBlock({ bindParents });
        ...
        replacements.push({
          ...
          context: inlineBlock ? inlineBlock.body : null,
        });
      ...
      (stmt.hash || { pairs: [] }).pairs
        .forEach((pair) => {
          if (reservedKeys.includes(pair.key)) {
            throwError(`Hashkey: ${pair.key} not allowed`, stmt);
          }
        });
    };
          ...
          _this.componentAst.body[0].body.body
            .push(
              getMethodFromFunctionDeclaration({
                ...
                // If <stmt.syntheticNodeId>, set addOptionsRetrievalStatements to false because <ast>
                // already contains options retrieval statements
    ...
    ASTParser.prototype.BlockStatement = function (stmt) {
      ...
      const {
        blockTransformHashKey, globalsBasePath, getPathFromSyntheticAlias, getChildPathFromSyntheticAlias,
        getTargetType, isLookupAllowed, throwError, addBlockParamHashKey,
      } = TemplatePreprocessor;
      ...
      const addCustomBlockCtx = () => {
        ...
        _this.updateCustomBlockPath({ stmt });
      ...
      if (inlineBlock) {
        ...
        if (scopeQualifier && !arr.includes(scopeQualifier)) {
          arr.push(scopeQualifier);
        }
        if (indexQualifier && !arr.includes(indexQualifier)) {
      ...
      const resolvePathParam = ({ stmt }) => {
        ...
        if (isSynthetic) {
          ...
          if (contextSwitching) {
            ...
            canonicalSource = stringifyHandlebarsStatement(stmt);
          }
          ...
          path = {
            ...
            original: _this.createSubExpression({
              ...
              syntheticAlias,
              // Notice that we are not passing in stmt here
              // because, it's not necessary, since we already
              // calling resetPathExpression(...) below
            }),
          };
        ...
        if (contextSwitching) {
          ...
          // It's possible for the target of a context-switching block to resolve to a global variable
          // in which case  targetType will be undefined.

          if (!targetType) {
            assert(original.startsWith([dataPathRoot, globalsBasePath].join(pathSeparator)));

            throwError(
              `"${stmt.original}" resolves to a global variable, and cannot be the target of this ${blockName} block`,
              stmt,
            );
          }
        ...
        if (type === 'PathExpression') {
          ...
          if (custom) {
            ...
            // For #with, #else, #if, #unless, we want to deliver the param value to 
            // the helper in the format {path, value}. For conditional blocks, we are 
            // also appending ! so that the underlying value will be returned, not our proxy
            _original = `${_original}!${conditional ? '!' : ''}`;
          }

          _this.allowedPaths.push(_original.replace(`${dataPathRoot}.`, ''));

          resetPathExpression({
            stmt,
            original: stmt.transformFn ? stmt.transformFn(_original) : _original,
            properties: {
              processed: true,
            },
          });
      ...
      if (contextSwitching) {
        ...
        if (canIterate) {
          ...
          // 1. In a data context, the rootProxy need to resolve paths embedded in synthetic function,
    ...
    const processComponentImport = ({ stmt }) => {
      ...
      if (stmt.params.length !== 1 || !alias) {
        throwError(`Component import - Incorrect signature`, stmt);
      }
      ...
      const { type, original, synthetic, targetValue, terminate } = this.resolvePath({
        ...
        contextList,
        bindParents,
        validTypes: [componentRefType],
        nameQualifier: className
      });

      // processComponentImport(...) is not called within inline blocks
      assert(!terminate);

      if (type != 'PathExpression') {
        throw Error(`Unexpected type '${type}', ${getLine(stmt)}`);
      }
      if (synthetic) {
        throw Error(`The alias in a ComponentImport statement must resolve to a data path, ${getLine(stmt)}`);
      }
      if (original.startsWith(syntheticMethodPrefix)) {
        throw Error(`ComponentImport statement cannot exist in a synthetic context, ${getLine(stmt)}`);
      }
      if (original.startsWith(syntheticMethodPrefix)) {
        throw Error(`ComponentImport statement cannot exist in a synthetic context, ${getLine(stmt)}`);
      }
      ...
      // <alias> should resolved a the expected component or <null> in the case of recursive components
      return false;
    };

    const getMetaPaths = () => {
      const { componentImportPath, variablePath, fnHelperName } = TemplatePreprocessor;
      return [componentImportPath, variablePath, fnHelperName];
    };

    const processMetaStatement = ({ stmt }) => {
      const {
        componentImportPath, variablePath, fnHelperName
      } = TemplatePreprocessor;
      ...
      switch (stmt.path.original) {
        case componentImportPath:
          if (isCustomContext()) {
            throw Error(`Component imports are not allowed within a custom context, line ${getLine(stmt)}`);
          } else if (getOuterInlineBlock({ bindParents }) !== null) {
        ...
        case fnHelperName:
          ...
          assert(
            !!stmt.params.length &&
            type == 'StringLiteral' &&
            this.methodNames.includes(original),
    ...
    const addLogicGate = (stmt) => {
      ...
      logicGate.participants = [...new Set(
        logicGate.participants
    ...
    ASTParser.prototype.MustacheStatement = function (stmt) {
      ...
      if (async && !isSubExpression) {
        throwError('Only a SubExpression may be marked as async', stmt);
      }

      if (isSubExpression) {
        stmt.fromMustache = true;

        const pruneLogicGate = () => {
          const prune0 = () => {
          ...
          if (isCustomContext()) {
            ...
            _this.logger.warn(`[${getLine(stmt)}] Pruning logic gate because statement is marked as "async"`);
            prune0();
          }
        }

        // If applicable, prune logic gate
        pruneLogicGate();

        if (async) {

          if (!getClosestAsyncContext()) {
            markPathExpressionsAsAsync({
              body: [stmt.params, ...stmt.hash ? [stmt.hash] : []]
            });
          }
      ...
      if (isCustomContext()) {
        ...
        if (transform) {
          stmt = {
            ...
            path: createPathExpression({ original: invokeTransformHelperName }),
            escaped,
            loc: stmt.loc,
          };
        }

        if (isSubExpression || transform) {
          stmt.type = 'SubExpression';
          ...
          if (stmt.path.processed) {
            _this.allowedPaths.push(stmt.path.original);
          }
        }
    ...
    const getPartialContextList = function ({ stmt, inline }) {
      ...
      const verifyHashKey = (key) => {
        if (getReservedPartialHashKeys().includes(key)) {
          throwError(
            `The hash key "${key}" is reserved and cannot be used in this partial`,
            stmt
          );
        }
      }
      ...
      if (isCustomContext()) {
        ...
        if (partialContextList.length > 1 && !inline) {
          ...
          partialContextList[0][rootQualifier]
            .declaredValue = utils.peek(contextList)[rootQualifier].value;
        }
        ...
        for (const pair of hash.pairs) {
          ...
          // We need to preserve a stringified version of SubExpressions for debug
          // purposes because of the manner they are transformed in the ast
          let canonicalSource;

          if (!pair.value.type.endsWith('Literal')) {

            if (pair.value.type == 'SubExpression') {
              canonicalSource = stringifyHandlebarsStatement(pair.value);
            }
          ...
          contextObject[pair.key] = {
            ...
            index: ctxIndex, canonicalSource,
          };
        }
      }
    ...
    const processPartial = function ({ stmt }) {
      ...
      new TemplatePreprocessor({
        ...
        allowedPaths: _this.allowedPaths,
    ...
    ASTParser.prototype.PartialStatement = function (stmt) {
      ...
      const { loadComponentHelperName, wordPattern, getLine } = TemplatePreprocessor;
      const { componentRefType } = PathResolver;
      ...
      if (stmt.params.length) {
        ...
        for (let i = 0; i < stmt.params.length; i++) {
          ...
          if (!param.original.match(wordPattern)) {
            throw Error(`PartialStatements should not have params, ${getLine(param)}`);
          }
    ...
    ASTParser.prototype.DecoratorBlock = function (stmt) {
      const {
        reservedDecoratorNames, allowOverlappingQualifiersInDecorator,
      } = TemplatePreprocessor;
      ...
      stmt.params.forEach(({ original }) => {
        if (original.endsWith('?')) {
          ...
          optionalParams.push(original);
        } else {
          requiredParams.push(original);
        }
      });
      ...
      if (!allowOverlappingQualifiersInDecorator && parent.blockQualifiers) {
        [...requiredParams, ...optionalParams].forEach(param => {
          if (parent.blockQualifiers.includes(param)) {
            throwError(
              `"${param}" is already a context qualifier inside the inline block, hence cannot be used as a decorator param`,
              stmt,
            );
          }
        })
  ...
  addInlineBlock({
    bindParents, decoratorBindParents, decoratorName, stmt, requiredParams, optionalParams, isCustomCtx,
  }) {
    const {
  ...
  static visitNodes({
    Visitor = handlebars.Visitor, types, ast, consumer,
  }) {
    function ASTParser() {
    }
    ASTParser.prototype = new Visitor();
  ...
  static hasDataPathFormat(path) {
    const { dataPathPrefix, dataPathRoot } = TemplatePreprocessor;
  ...
  static getMetaHelpers() {
    const {
      ...
      setSyntheticNodeIdHelperName, resolveMustacheHelperName, fnHelperName,
    } = TemplatePreprocessor;
    return [
      storeContextBlockName, loadContextBlockName, customEachHelperName, conditionalHelperName,
      startAttrCtxHelperName, endAttrCtxHelperName, startTNBCtxHelperName,
  ...
  static getReservedBlockHashKeys() {
    const {
      blockParamHashKey, rootQualifier,
    } = TemplatePreprocessor;
    return [blockParamHashKey, rootQualifier];
  }

  static getReservedPartialHashKeys() {
    const {
      partialIdHashKey, partialNameHashKey, partialUnsafeHashKey, rootQualifier,
    } = TemplatePreprocessor;
    return [
      // For Custom Ctx:
      partialIdHashKey, partialNameHashKey,
      // For Root Ctx:
      partialUnsafeHashKey,
      // Both
      rootQualifier,
    ];
  }
  ...
  static stringifyHandlebarsStatement(stmt) {
    let arr = [];
    ...
    const accept = (stmt) => {
      ...
      switch (true) {
        case stmt.type == 'PathExpression':
          arr.push(stmt.original);
          break;
        case stmt.type == 'StringLiteral':
          arr.push(`"${stmt.original || stmt.value}"`);
          break;
        case stmt.type.endsWith('Literal'):
        case stmt.type == 'ContentStatement':
          arr.push(`${stmt.original || stmt.value}`);
          break;
        case stmt.type == 'Hash':
          for (let i = 0; i < stmt.pairs.length; i++) {
            const pair = stmt.pairs[i];
            if (i !== 0) {
              arr.push(' ');
            }
            arr.push(`${pair.key}=`);
          ...
          break;
        case stmt.type == 'SubExpression':
          arr.push('(');
          accepSubExpr(stmt)
          arr.push(')');
          break;
        case stmt.type == 'MustacheStatement':
          arr.push('{{');
          accepSubExpr(stmt)
          arr.push('}}');
          break;
        case stmt.type == 'MustacheGroup':
          arr.push('"');
          for (const item of stmt.items) {
            if (item.type == 'PathExpression') {
              arr.push('${');
              accept(item);
              arr.push('}');
            } else {
              // Pretend this is a PathExpression to avoid extra quotes
              accept({
                type: 'PathExpression',
          ...
          break;
        default:
          throw Error(`${stmt.type} not yet supported`);
      }
    }
