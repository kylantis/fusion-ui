...
class TemplatePreprocessor {
  ...
  static fetchDistComponentList(verify = true) {
    const { getComponentsDistPath, getComponentListPath } = TemplatePreprocessor;
  ...
  simulateAppContext() {
    const {
      getComponentClass, getComponentsDistPath, getAppContextObject, getComponentListRelativePath,
    } = TemplatePreprocessor;
    ...
    Object.entries(this.metadata.distComponentList)
      .forEach(([className, assetId]) => {
        ...
        try {
          ...
          this.throwError(`Error occured while loading component: ${className}. To proceed, remove it from ${getComponentListRelativePath()}`);
        }
