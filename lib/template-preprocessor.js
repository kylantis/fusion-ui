...
const TernaryTransformer = require('./ternary-transformer');
...
class TemplatePreprocessor {
  ...
  static detectCircularDependencies = false;
  ...
  static eachBlockPathHashKey = 'path';
  ...
  constructor({
    ...
    const { getMethodNames, getComponentClass } = TemplatePreprocessor;
    utils.addPloyfills();
  ...
  static addComponentGlobals() {
    ...
    for (const className of keys) {
      ...
      const precompiledTemplateFile = pathLib.join(dir, 'template.min.js');
      if (fs.existsSync(precompiledTemplateFile)) {
        const templateScript = fs.readFileSync(
          precompiledTemplateFile,
          'utf8',
        );
        eval(templateScript);
      }
  ...
  getTransientComponentsGlobal() {
    const {
      detectCircularDependencies, getComponentsSrcPath,
      getComponentsDistPath,
      findMatchingComponent, getComponentClass,
    } = TemplatePreprocessor;
    const _this = this;
    return new Proxy({}, {
      get: (obj, prop) => {
        ...
        if (detectCircularDependencies && Object.keys(_this.parents).includes(prop)) {
        ...
        if (assetId
          // If circular dependency detection is enabled, we always have to re-process the template
          // in order to detect all referenced component down the chain
  ...
  getSerializedComponent(className) {
    ...
    const sampleData = require(
      pathLib.join(getComponentsDistPath(), dirName, 'sample.js'),
    );
  ...
  process() {
    ...
    if (!this.isPartial) {
      ...
      // Todo: rename, then remove and invoke from template processor
      TemplatePreprocessor.getTokens({ ast: this.ast });
  ...
  readHeadAttributes() {
    ...
    if (lazyLoadComponentTemplates) {
      this.jsDependencies.push({
        url: `/components/${this.assetId}/template.min.js`,
        moduleType: 'inline',
      });
    }
  }
  ...
  pruneComponentAst({ pruneKey }) {
    ...
    for (let i = 0; i < body.length; i++) {
      if (body[i].key.name.endsWith(pruneKey) && body[i].prune) {
        delete body[i];
      }
    }
  ...
  static createMustacheStatement({ original, hash }) {
    const { createPathExpression } = TemplatePreprocessor;
    const stmt = {
  ...
  getBlockOptions(stmt) {
    ...
    if (!custom) {
      // hbs allows hashes on well-known blocks by default
  ...
  static getTokens({ ast }) {

    const { customEachHelperName, stencilCssClassname, setSyntheticNodeIdHelperName,
    ...
    const getCurrentAttributesList = ({ index }) => {
      ...
      const node = ast.content.children.peek();
      ...
      node.content.attributes.forEach((attribute) => {
        ...
        let v = eval(`${startWrapper.content}${value.content}${endWrapper.content}`);
    ...
    streamTokenizer
      .on('data', (tokens) => {
        ...
        if (startTagEnd >= 0) {
          ...
          if (htmlAttributeList['id'] && htmlAttributeList['id'] !== `{${setSyntheticNodeIdHelperName}}`) {
            if (stmt.inIterateContext || hasDataPaths) {
              throw new Error(`You cannot provide a HtmlElement ID, line ${getLine(stmt)}`);
            }
          }
    ...
    const onMustacheStatement = (stmt) => {
      ...
      const { rawDataPrefix, resolveMustacheHelperName, hasDataPathFormat } = TemplatePreprocessor;
    ...
    ASTParser.prototype.BlockStatement = function (stmt) {
      ...
      // const { getHandleBarsBlockHelpers, customEachHelperName } = TemplatePreprocessor;
      // const custom = ![
      //   ...getHandleBarsBlockHelpers(), customEachHelperName
  ...
  process0({
    ...
    ASTParser.prototype.BlockStatement = function (stmt) {
      ...
      if (contextSwitching) {
        ...
        if (canIterate) {
          ...
          const { eachBlockPathHashKey, customEachHelperName } = TemplatePreprocessor;
          ...
          if (!syntheticAlias) {
            ...
            stmt.hash = {
              ...
              pairs: [{
                type: 'HashPair',
                key: eachBlockPathHashKey,
                value: createStringLiteral(path)
              }]
            }
          }
        ...
        contextObject[rootQualifier] = {
          ...
          ctxSwitching:
            ...
            canIterate,
          inIterateContext: canIterate || !!contextList.peek()[rootQualifier].inIterateContext,
          index: contextList.length,
        };
      ...
      if (stmt.path.original === 'if' && type === 'PathExpression') {

        const { blockNodeIdHashKey, getSyntheticNodeIdMethodName } = TemplatePreprocessor;
        ...
        hash.pairs.push({
          type: 'HashPair',
          key: blockNodeIdHashKey,
          value: createPathExpression({
            original: toCanonical(getSyntheticNodeIdMethodName),
          }),
        });

        _this.dataPaths.push(getSyntheticNodeIdMethodName);
      }
    ...
    const processComponentImport = ({ stmt }) => {
      ...
      const fqPath = `${contextList.peek()[rootQualifier].value
        }${pathSeparator}${alias}`;
    ...
    ASTParser.prototype.MustacheStatement = function (stmt) {
      ...
      if (isSubExpression) {
        ...
        if (!async && !stmt.syntheticNodeId && !stmt.logicGate) {
          createHtmlWrapper({ stmt });
        }
      }
      ...
      if (isSubExpression) {
        ...
        if (stmt.processed) {
          ...
          const { logicGatePathRoot, pathSeparator,
            synthethicMethodPrefix, resolveMustacheHelperName
          } = TemplatePreprocessor;

          if (logicGate &&
            // We can only only data-bind, if there are no sub expressions in this logic
            // gate 
            // Todo: if a subexpression has "iterative" params/hash, indicate and then
            // we can exclude those and then add support or the others
            !logicGate.hasSubExpression &&
            !logicGate.participants
              .filter(({ original }) => original.startsWith(synthethicMethodPrefix)).length
          ) {
            ...
            logicGate.table = logicGate.table.map(item => {
              ...
              item.condition = item.condition.original;
              item.left = (!Number.isNaN(parseInt(item.left, 10))) ? item.left : { key: item.left.original };
              item.right = (!Number.isNaN(parseInt(item.right, 10))) ? item.right : { key: item.right.original };

              return item;
            });

            const gateId = utils.generateRandomString();
            _this.logicGates[gateId] = logicGate;

            const original = `${logicGatePathRoot}${pathSeparator}${gateId}`;
  ...
  static getMetaHelpers() {
    const {
      ...
      customEachHelperName, startAttrCtxHelperName,
