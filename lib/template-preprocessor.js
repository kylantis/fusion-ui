...
class TemplatePreprocessor {
  ...
  static registerProgram({ locSource, templateSrc, program }) {
    ...
    assert(program.type == 'Program' && !program.programId);

    const programId = utils.generateRandomString();
    program.programId = programId;

    registerProgramInfo({ programId, locSource, templateSrc });
  ...
  static setLocSource(program, programId) {
    const { visitNodes, getAllHandlebarsTypes, getProgramInfoRegistry } = TemplatePreprocessor;
    ...
    visitNodes({
      ...
      consumer: ({ stmt }) => {
        const { loc } = stmt;
        // Todo: remove condition to find nodes with no loc, so we can fix because loc should be there
        if (loc) {
  ...
  parseComponentClass({
    ...
    try {
      ...
      this.logger.error(`Error occured while loading "${filePath}"`);
      throwError(e);
    }
  ...
  addParameterizedExpression({
    ...
    const {
      ...
      onSubExpressionProcessed, hasDataPathFormat, createArrowFunctionExpression,
      getLiteralValueFromPathExpression, getScalarValue,
    } = TemplatePreprocessor;
  ...
  static getMethodFromFunctionDeclaration({
    ast: expression, addSyntheticPrefix = true, hasWrapper, addOptionsRetrievalStatements = true,
  }) {
    const {
      syntheticMethodPrefix, renderMethodName, getBlockWrapperIdMethodName,
  ...
  addDataBindTransformations(ast) {
    ...
    // Custom tokens
    const ATTR_VALUE_BLOCK = 'token:attribute-value-block';
    ...
    const getCurrentAttributesList = ({ tokenList, index, loc }) => {
      const allowedTokens = [
        OPEN_TAG_START, ATTR_KEY, ATTR_ASSIGNMENT, ATTR_VALUE, ATTR_VALUE_SEGMENT,
      ...
      // eslint-disable-next-line no-shadow
      const { ast } = constructTree(mergeAttributeValueSegments(arr));
      ...
      node.content.attributes.forEach((attribute) => {
        ...
        let { key, startWrapper, value, endWrapper } = attribute;
    ...
    const splitTokens = (tokens) => {
      ...
      for (const token of [...tokens]) {
        ...
        tokens.splice(
          tokens.indexOf(token), 1, ...segments.map((content) => ({
            type: (type == ATTR_VALUE) ? ATTR_VALUE_SEGMENT : TEXT, content
          }))
        );
      }
      ...
      // Todo: remove
      const originalStmtStack = [...stmtStack];

      const validateToken = (token) => {
        const { type, content } = token;

        // Note: In an attribute context, whitespaces between attributes are not tokenized 
        // hence, we need to ignore it.
        ...
        switch (type) {
          ...
          case ATTR_KEY:
            if (content.includes('{{') && !isHbsToken(token)) {
              ...
              throwError(
                `Attribute key: ${attrKey} is invalid`,
    ...
    streamTokenizer
      .on('data', (tokens) => {
        ...
        if (openTagEnd >= 0) {
          ...
          const hasDataPaths = Object.keys(attributeList).length;

          const { id } = htmlAttributeList;

          // In this context, an arbitraryId is one that we are certain will be rendered twice
          // thereby leading to duplicate Ids on the browser

          const hasArbitraryId = id && !id.includes('{{') && !id.includes('${');

          if (hasArbitraryId && stmt.inIterateContext) {
            throwError(
    ...
    const getContentStmtFromToken = (stmt, token, contextId) => {
      ...
      contextStack.forEach(({ endIndex }, i) => {
        ...
        if (start != undefined) {
          ...
          if (!next || next.startIndex > idx + token.content.length) {
            end = i;
          }
        }
      });
    ...
    const onContentStatement = (stmt, visitor) => {
      ...
      if (!split && containsAnySubstring(original, ['<', '>'])) {
        ...
        const idx = parent.body.indexOf(stmt);
        assert(idx >= 0);

        parent.body.splice(
          idx + 1, 0, ...program.body,
        );
    ...
    const onMustacheStatement = (stmt) => {
      const {
        path: { processed }, dataBinding, createHtmlWrapper, forceHtmlWrapper, metaStatement, canonicalSource,
      } = stmt;
      ...
      streamTokenizer.write(
        `{{${(processed && dataBinding) ? 'bind' : ''}${RANDOM_STR}}}`
      );
    };
    ...
    const isIterateBlock = (original) => {
      return original === 'each' || original === customEachHelperName;
    }
    ...
    ASTParser.prototype.BlockStatement = function (stmt) {
      ...
      const { getCanonicalSource } = TemplatePreprocessor;

      if (isAttributeContext()) {

        if (!isAttributeValueContext()) {
        ...
        // Since we are not visiting the program but rather adding to <statements>, we need add to <canonicalSource>

        stmt.canonicalSource = getCanonicalSource(stmt);

        statements.push(stmt);
        streamTokenizer.write(`{{#${RANDOM_STR}}}`);
    ...
    // Re-implement Program visitation, to support in-place expansion of program.body.
    // This is required due to the way we split content statements
    ASTParser.prototype.Program = function (program) {
  ...
  process0({ contextList, bindParents, program, validateHtml }) {
    ...
    const acceptPathExpressionInRootCtx = ({ stmt }) => {
      ...
      const {
        toPathExpressionLiteral, hasDataPathFormat, getTargetType, isLookupAllowed, getSampleValueForLiteralType,
      } = TemplatePreprocessor;
    ...
    ASTParser.prototype.BlockStatement = function (stmt) {
      ...
      const {
        globalsBasePath, wrapInvocationWithProxyMethodName, dataPathPrefixRegex, getPathFromSyntheticAlias, getChildPathFromSyntheticAlias, getTargetType,
        isLookupAllowed, throwError, addBlockParamHashKey, getIterateDataVariables, getReservedBlockHashKeys, getLine,
      ...
      const processBlockInRootCtx = () => {
        ...
        if (contextSwitching) {
          ...
          original += (isSynthetic || !iterateBlock) ? original.endsWith('_$') ? '' : '_@' : '_$';
    ...
    const getPartial = ({ ctxList, partialName, stmt }) => {
      ...
      const {
        ...
        createAst0, getHashValue, registerProgram, throwError, generateTemplateSource, setLocSource,
        registerProgramInfo, getProgramInfoRegistry,
      } = TemplatePreprocessor;
      ...
      (() => {
        ...
        for (const parent of _this.metadata.parents) {
          ...
          const getProgramId = () => {
            const { templateSrc } = classInfo;

            return programId || (programId = registerProgramInfo({
              templateSrc,
              locSource: createLocSource({ base: parent, fileName })
            }));
          }
            ...
            if (node0.type == DECORATOR) {
              ...
              if (!node0.program.programId) {
                ...
                registerProgram({
                  ...
                  templateSrc: generateTemplateSource(classInfo.templateSrc, node0.program).value,
              ...
              if (decoratorName == partialName) {
                ...
                templateSrc = getProgramInfoRegistry()[node.program.programId].templateSource;
    ...
    const processPartial = function ({ stmt }) {
      ...
      processAst({ ast, ctxList, templateSrc, validateHtml: !inline })
    ...
    ASTParser.prototype.DecoratorBlock = function (stmt) {
      const {
        ...
        getDecoratorParams, getHashValue, visitNodes, createMustacheStatement, createPathExpression, getNonEmptyLiteralTypes,
        getLine, generateTemplateSource,
      } = TemplatePreprocessor;
  ...
  addInlineBlock({
    ...
    defaultParams, isCustomCtx, runtime, eager, isOnRoot, templateSource,
  }) {
    const { dataPathRoot, visitNodes } = TemplatePreprocessor;
  ...
  static getLine(stmt, range = true) {
