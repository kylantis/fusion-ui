...
class TemplatePreprocessor {
  ...
  static generateClassesInfo() {
    ...
    for (const dirName of componentsFolder) {
      ...
      if (!o) {
        ...
        throwError(`[${cd.id.name}] Component class name is already defined in ${o.dir}`);
      }
    }
  ...
  process0({
    ...
    ASTParser.prototype.BlockStatement = function (stmt) {
      ...
      const {
        globalsBasePath, getPathFromSyntheticAlias, getChildPathFromSyntheticAlias, getTargetType,
        isLookupAllowed, throwError, addBlockParamHashKey, getIterateDataVariables, invokeDataPathTransform,
      ...
      const addCustomBlockCtx = () => {
        ...
        if (async) {
          ...
          if (!stmt.syntheticNodeId) {
            throwError(
              'Block must contain valid html markup inorder to be marked as "async"',
              stmt
            )
          }
        }
