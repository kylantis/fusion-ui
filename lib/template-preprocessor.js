...
class TemplatePreprocessor {
  ...
  writeSamplesFileToFileSystem() {
    ...
    const contents = `module.exports=${this.samplesString}`;

    fs.writeFileSync(filePath, contents);
    fs.writeFileSync(
      `${filePath}.br`,
      brotli.compress(new TextEncoder().encode(contents))
    );
  }
  ...
  writeSchemaFileToFileSystem() {
    ...
    const contents = JSON.stringify(this.clientSchema, null, 2);
  ...
  writeBootConfigFileToFileSystem() {
    ...
    const contents = JSON.stringify(this.bootConfig, null, 2);
  ...
  writeMinifiedComponentJsFile({ data, fileName }) {
    ...
    fs.writeFileSync(
      pathLib.join(distPath, `${minifiedFileName}.map`),
      map
    );

    fs.writeFileSync(
      pathLib.join(distPath, `${minifiedFileName}.map.br`), 
      brotli.compress(new TextEncoder().encode(map))
    );
    ...
    fs.writeFileSync(
      pathLib.join(distPath, `${minifiedFileName}.br`),
      brotli.compress(new TextEncoder().encode(_code))
    );
  }
