...
class TemplatePreprocessor {
    ...
    if (parent) {
      ...
      Object.entries(parentSchema.definitions)
        .forEach(([k, v]) => {
          ...
          if ((k === parent && !v.isComponent) || k == `${parent}${recursiveRootSuffix}`) {
  ...
  process0({ contextList, bindParents, program }) {
      ...
      if (isCustomContext()) {
        ...
        const wrapInResolveMustacheHelper = () => {
          ...
          return {
            ...
            createHtmlWrapper: originalMethodName == loadInlineComponentHelperName,
            forceHtmlWrapper: originalMethodName == loadInlineComponentHelperName,
          };
        }
    ...
    ASTParser.prototype.PartialStatement = function (stmt) {
      ...
      const {
        componentRefType, loadInlineComponentHelperName, wordPattern, getReservedPartialHashKeys, throwError, createBooleanLiteral,
      } = TemplatePreprocessor;
