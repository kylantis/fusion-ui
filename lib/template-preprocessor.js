  ...
  addDataBindTransformations() {
    ...
    const getCurrentAttributesList = ({ index }) => {
      ...
      node.content.attributes.forEach((attribute) => {
        ...
        if (!value) {
          // In any case, we are will need to skip because
          // 1. If the key is a mustache, then we cannot safely infer that
          // it emits an attribute-key, as opposed to a kay-value pair
          // 2. If the key is a literal, then there is no need to register
          // this entry in attributes
