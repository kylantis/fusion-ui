...
class TemplatePreprocessor {
  ...
  static addComponentGlobals() {
    ...
    for (const className of keys) {
      ...
      global.components[className] = getComponentClass({
        dir,
        useTestClass: true,
      }).ComponentClass;

      const templateScript = fs.readFileSync(
        pathLib.join(dir, 'template.min.js'),
        'utf8',
      );
      eval(templateScript);
    }
  ...
  getTransientComponentsGlobal() {
    const {
      ...
      getAssetIdFromClassName, getComponentsDistPath,
    ...
    return new Proxy({}, {
      get: (obj, prop) => {
        ...
        if (assetId
          ...
          processFile({
            dir: findMatchingComponent({ className: prop }),
  ...
  process() {
    const {
      rootQualifier, dataPathRoot, getLiteralType, getComponentsDistPath,
    ...
    ASTParser.prototype.SubExpression = function (stmt) {
      ...
      if (isCustomContext()) {
        ...
        if (stmt.type === 'PathExpression') {
          ...
          console.info(`getSyntheticMethodValue ${prev}`);
          console.info(`getSyntheticMethodValue0 ${prev}`);
          ...
          console.info(_this.srcDir);
          console.info('getSyntheticMethodValue END');
      ...
      if (isSubExpression) {
        ...
        if (!async && !stmt.siblingNodeId) {
          ...
          replacements.push({
            ...
            replacementNodes: [
              createContentStatement({
                original: `<div id='${siblingNodeId}' synthetic></div>`,
              }),
            ],
          });
  ...
  createResolver({ path, target }) {
    ...
    return new Proxy(literal ? {} : target, {
      ...
      get(obj, prop) {
        ...
        const value = _this.resolver().resolve(property);

        return _this.createResolver({ path: property, target: value });
      },
    });
  }
