...
class TemplatePreprocessor {
  ...
  static validateType({ path, value, validType, nameQualifier, line, allowEmptyCollection=false }) {
  ...
  resolveGlobalVariable({ pathInfo, original, validType, stmt }) {
    const { getLine, validateType } = TemplatePreprocessor;
  ...
  resolvePathFromContext({
    ...
    switch (true) {
      ...
      case original == `${dataPathRoot}${pathSeparator}` || type.endsWith('Literal'):
        ...
        TemplatePreprocessor.validateType({
  ...
  lookupRootPath({ stmt, original }) {
    const {
      dataPathRoot, pathSeparator, defaultIndexResolver, getLine, getTargetType, validateType,
    } = TemplatePreprocessor;
  ...
  getSyntheticMethodValue({ stmt, source, method, validType, line }) {
    const { throwError, getLine, validateType } = TemplatePreprocessor;
  ...
  process0({ contextList, bindParents, program }) {
    ...
    const getMethodNameHashValue = ({ stmt, key, cleanup = false, allowMultiple }) => {
    ...
    const getPredicate = ({ stmt }) => {
      ...
      return getMethodNameHashValue({ stmt, key: predicateHashKey });
    };

    const getOuterTransform = ({ stmt }) => {
      const { outerTransformHashKey } = TemplatePreprocessor;
      return getMethodNameHashValue({ stmt, key: outerTransformHashKey, allowMultiple: true });
    };

    const getInnerTransform = ({ stmt }) => {
      const { innerTransformHashKey } = TemplatePreprocessor;
      return getMethodNameHashValue({ stmt, key: innerTransformHashKey, allowMultiple: true });
    };

    const getTransform = ({ stmt }) => {
      const { transformHashKey } = TemplatePreprocessor;
      return getMethodNameHashValue({ stmt, key: transformHashKey });
    };
    ...
    const getHook = ({ stmt }) => {
      ...
      return getMethodNameHashValue({ stmt, key: hookHashKey });
    };
    ...
    ASTParser.prototype.BlockStatement = function (stmt) {
      ...
      const {
        ...
        throwError, addBlockParamHashKey, getIterateDataVariables, invokeDataPathTransform, isRootCtxValue, getReservedBlockHashKeys,
        getLine, getBlockSource,
      } = TemplatePreprocessor;
      ...
      if (!stmt.processed && getPredicate({ stmt }) && !canIterate) {
