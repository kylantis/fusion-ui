...
class TemplatePreprocessor {
  ...
  getDeclaredBaseComponent() {
    ...
    if (!baseComponent) {
      throwError(
        `No base component defined in ${pathLib.relative(process.env.PWD, getSrcConfigFile())}`
      );
    }
  ...
  getSerializedComponent(path, className, declaredClassName, loadable) {
    ...
    if (className === BaseComponent.name) {
      ...
      const { parents, isAbstract } = this.metadata;

      if (!isAbstract && parents.includes(className)) {
        const idx = loadableImplClasses.indexOf(this.className);
        assert(idx >= 0);

        loadableImplClasses.splice(idx, 1);
      }
