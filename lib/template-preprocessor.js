...
class TemplatePreprocessor {
  ...
  static addDefaultParamToCustomBlock = true;
  ...
  process0({
    ...
    const acceptPathExpressionInCustomCtx = ({ stmt }) => {
      ...
      const contextObjects = [];
      const asyncContext = getClosestAsyncContext();
          ...
          for (const k of contextKeys) {
            ...
            if (hasObjectPrefix({
              ...
              if (v.type.endsWith('Literal')) {
                ...
                if (path.length) {
                  eval(`this.component.getInput().${path}`);
                }
    ...
    const createHtmlWrapper = ({ stmt, tagName = 'div' }) => {
      ...
      replacements.push({
        ...
        replacementNodes: [
          createContentStatement({
            original: `<${tagName} id='${wrapperNodeId}'>`,
          }),
        ],
      });
      ...
      if (contextSwitching) {
        ...
        contextObject[rootQualifier] = {
          ...
          ctxSwitching:
            ...
            canIterate,
          inIterateContext: canIterate || !!contextList.peek().inIterateContext,
          index: contextList.length,
        };
    ...
    ASTParser.prototype.MustacheStatement = function (stmt) {
      ...
      if (isSubExpression) {
        if (!async && !stmt.wrapperNodeId) {
          stmt.wrapperNodeId = createHtmlWrapper({ stmt });
        }
      }
  ...
  createResolver({ path, target }) {
    ...
    return new Proxy(literal ? {} : target, {
      ...
      get(obj, prop) {
        ...
        const value = _this.resolver.resolve(`${property}%${literalType}`);
