...
class TemplatePreprocessor {
  ...
  resolvePathFromContext({
    ...
    syntheticAlias, nameQualifier, create,
  }) {
    const {
              ...
              if (
                ...
                original = this.createRootProxyIndirection({
                  ...
                  suffix: k === original ? false
                    // Note: processLiteralSegment is false, hence we are responsible
                    // for deciding whether or not to prefix "."
                    // Hence, if <suffix> is an index, do not prefix '.'
  ...
  resolvePath({
    ...
    const {
      ...
      literalType, globalsBasePath, resetPathExpression, getOuterInlineBlock, getPathOffset,
      getLine, getRootOriginalFromPathOffset
    } = TemplatePreprocessor;
  ...
  static getKnownTypes() {
    return [
      ...
      'StringLiteral', 'LogicGate', 'MustacheGroup', 'NullLiteral',
      'UndefinedLiteral',
  ...
  static getPreservedPathExpressionAttibutes() {
    return ['participantType', 'transformFn'];
  }
   ...
   *
   */
  createRootProxyIndirection({
    path, suffix = false, syntheticAlias, processLiteralSegment = true
  }) {
    const { getCallExpression, getProxyStatement, getValue } = TemplatePreprocessor;
    ...
    let synthethicMethodName;

    const expression = getProxyStatement({ path });

    if (suffix) {
      synthethicMethodName = this.createRootProxyIndirection0({
        expression,
        suffix,
  ...
  // This implementation needs to be improved, to ensure that in the event of an
  // error when executing "first${suffix}", the developer is able to trace the error
  // I suggest wrapping in try... catch... with error message indicating the full path
  // we were trying to access...
  createRootProxyIndirection0({
    expression, suffix, processLiteralSegment, syntheticAlias,
  }) {
    const {
      processLiteralSegmentMethod, getCallExpression, getMethodFromFunctionDeclaration,
      getScalarValue, getValue, getPathFromSyntheticAlias,
    } = TemplatePreprocessor;
  ...
  getSyntheticMethodValue({ path, method, validTypes = [], line }) {
  ...
  process0({
    ...
    const acceptPathExpressionInCustomCtx = ({ stmt, visitor }) => {
          ...
          for (const k of contextKeys) {
            ...
            if (hasObjectPrefix({
              ...
              if (v.type.endsWith('Literal')) {
                ...
                original = k === original ?
                  v.value :
                  this.createRootProxyIndirection({
                    stmt,
                    path: v.value,
                    suffix: getSuffix(original),
                  });
      ...
      if (contextObject[rootQualifier]) {
        ...
        if (original !== dataPathRoot
          ...
          // This is specifically for scenarios where the developer may
          // try to resolve @root.[0] where @root is the root data context
          // thereby resulting in original == data[0]
          throw Error(`Invalid path: ${prev}`);
        }
    ...
    ASTParser.prototype.BlockStatement = function (stmt) {
      this.mutating = true;
      const {
        blockTransformHashKey, getPathFromSyntheticAlias, getHandleBarsBlockHelpers
      } = TemplatePreprocessor;
      ...
      const resolvePathParam = ({ blockName, stmt }) => {
        ...
        if (isSynthetic) {
          ...
          const value = _this.getSyntheticMethodValue({
            path: stmt.canonicalSource || method,
        ...
        if (type === 'PathExpression') {
          ...
          if (custom) {
            ...
            // Notice that for conditional blocks, we are also:
            // 1. Indicating that the path (if not synthetic) should be leniently resolved
            // 2. appending ! so that the underlying value will be returned, not 
            // our proxy
            _original = `${_original}!${conditional ? `!${isSynthetic ? '' : '?'}` : ''}`;
          }
