...
class TemplatePreprocessor {
  ...
  static generateClassesInfo() {
    ...
    for (const dirName of componentsFolder) {
      ...
      const isAbstract = getIsAbstractFromClassDeclaration(cd);

      const o = classesInfo[cd.id.name];

      if (!o) {
        classesInfo[cd.id.name] = {
          dir, classDeclaration: cd, isAbstract, parents: [], children: []
        }
      } else {
  ...
  static getIsAbstractFromClassDeclaration(cd) {
    ...
    if (
      arr.length != 1 || arr[0].type != 'ReturnStatement' ||
      ![true, false].includes(arr[0].argument.value)
    ) {
      throwError(
    ...
    return arr[0].argument.value;
  }

  getDefaultContextList() {
    const { dataPathRoot, rootQualifier, getLiteralType } = TemplatePreprocessor;
