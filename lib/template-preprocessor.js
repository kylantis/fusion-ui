...
class TemplatePreprocessor {
    ...
    resolvePathFromContext({ contextList, contextIndex, original, validTypes = [], lookup = true }) {
        ...
        if (!type) {
            ...
            let rootValue = contextList[contextIndex][rootQualifier].value;
    ...
    resolvePath({ contextList, value, validTypes = [] }) {
        ...
        if (type == 'PathExpression') {
            ...
            original = this.processLiteralSegments({ original });
    ...
    processLiteralSegments({ original }) {
    ...
    getBlockOptions(stmt) {
        ...
        switch (stmt.path.original) {
            ...
            default:
                custom = true;
                deleteBlockParams = false;

                if (!this.methodNames.includes(stmt.path.original)) {
                    throw new Error(`Unknown method: ${stmt.path.original}`);
                }

                if (!this.helpers.includes(stmt.path.original)) {
                    this.helpers.push(stmt.path.original);
                }
    ...
    process0({
        ...
        ASTParser.prototype.BlockStatement = function (stmt) {
            ...
            const { rootQualifier, customBlockPrefix } = TemplatePreprocessor;
            ...
            if (isSynthetic) {
                ...
                const i = _this.dataPaths.indexOf(path.original);
                _this.dataPaths.splice(i, 1);
            ...
            if (type == 'PathExpression') {
                ...
                if (custom) {
                    __path = `${customBlockPrefix}${__path}`;
                }
            ...
            if (!custom) {
                // For custom helper blocks, we don't
                // visit the sub-program
