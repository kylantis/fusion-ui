...
class TemplatePreprocessor {
  ...
  constructor({
    ...
    const { getMethodNames, validateMethodNames } = TemplatePreprocessor;
    utils.addPloyfills();
    ...
    if (!component) {
      this.logger.info(`\x1b[32m[${pluginName}/${componentName} => ${this.assetId}]\x1b[0m`);
      ...
      validateMethodNames(getMethodNames({
        component,
      }));
    }
    ...
    this.methodNames = getMethodNames({
  ...
  static validateMethodNames(methodNames) {
  ...
  process() {
    ...
    if (!this.isPartial) {
      ...
      // this.logger.log(JSON.stringify(this.ast));
    }
  }
  ...
  static getPathOffset({ original, contextList }) {
    ...
    if (original.startsWith('@')) {
      ...
      if (
        ...
        || !getHandleBarsDataVariables().includes(original)
      ) {
  ...
  resolvePathFromContext({
    ...
    const {
      rootQualifier, pathSeparator: separator,
      hasObjectPrefix,
  ...
  resolvePath({
    ...
    const {
      inlineParameterPrefix, processLiteralSegmentMethod, hasObjectPrefix,
    ...
    if (
      ...
      this.logger.info(`Added inline parameter '${stmt.parts[0]}' for inline block: ${inlineBlock.params[0].original}`);
  ...
  getObjectValue(json) {
    const { getIdentifier, isAstObject } = TemplatePreprocessor;
    const envelope = {
      type: 'ObjectExpression',
      properties: [],
    };
    for (const k in json) {
      if ({}.hasOwnProperty.call(json, k)) {
        ...
        envelope.properties.push({
          ...
          key: getIdentifier(k),
          computed: false,
          value: isAstObject(v) ? v : this.getValue(v),
          kind: 'init',
          method: false,
          shorthand: false,
        });
  ...
  createDataPathIndirection({
    ...
    const {
      ...
      hasDataPathFormat, getProxyStatement,
    } = TemplatePreprocessor;
  ...
  updateCustomBlockHeaders({ stmt }) {
    ...
    const scopeQualifier = this.registerCtxRoot({ stmt });
  ...
  static getAllValidTypes() {
    const { literalType, arrayType, objectType } = TemplatePreprocessor;
    return [literalType, arrayType, objectType];
  }

  getBlockOptions(stmt) {
    const {
      getAllValidTypes, ensureNoHash, getLine, arrayType, objectType,
    } = TemplatePreprocessor;
    const {
    ...
    switch (stmt.path.original) {
      ...
      case 'each':
        validTypes = [indexQualifier ? arrayType : objectType];
  ...
  process0({
    ...
    visitNodes({
      ...
      ast,
      consumer: ({ stmt }) => {
        const { validatePath, isRootCtxValue } = TemplatePreprocessor;
    ...
    const acceptPathExpressionInCustomCtx = ({ stmt }) => {
      ...
      if (prev.startsWith(inlineParameterPrefix)) {
        throw new Error(`Inline parameter: ${prev.split('.')[0]} not allowed in custom context`);
      }
    ...
    const acceptPathExpressionInRootCtx = ({ stmt }) => {
      ...
      return type ? resetPathExpression({
        stmt,
        original,
        properties: {
          type,
    ...
    ASTParser.prototype.BlockStatement = function (stmt) {
      ...
      if (isCustomContext()) {
        ...
        if (!getHandleBarsBlockHelpers().includes(stmt.path.original)) {
          ...
          if (getContextSwitchingHelpers().includes(stmt.path.original)) {
            const contextObj = {};
      ...
      const resolvePathParam = ({ blockName, stmt }) => {
        ...
        if (type === 'PathExpression') {
          // eslint-disable-next-line no-underscore-dangle
          let _original = `${contextList.length === 1 ? `${dataPathRoot}.` : ''}${original}`;
    ...
    ASTParser.prototype.MustacheStatement = function (stmt) {
      ...
      if (processed) {
        ...
        original = `${contextList.length === 1 ? `${dataPathRoot}.` : ''}${original}`;
        _this.dataPaths.push(original);

        resetPathExpression({ stmt: stmt.path, original });
      }
    ...
    const getPartialContextList = ({ stmt, inline }) => {
      ...
      if (isCustomContext()) {
        // Dynamically generate and add blockParamHashKey
        // This is used to resolve @root inside the partial
        contextObject[_this.registerCtxRoot({ stmt })] = {
          lookup: true,
          asVariable: true,
        };
    ...
    const getPartialAst = ({ stmt }) => {
      ...
      for (const blockName in inlineBlocks) {
        if (blockName === partialName) {
          let block = inlineBlocks[blockName];

          // We want to clone the ast, but maintain the reference
    ...
    // eslint-disable-next-line no-shadow
    const recurseAst = ({ ast, contextList, bindParents }) => {
    ...
    const processPartial = ({ stmt }) => {
      ...
      if (getOuterInlineBlock({ bindParents }) != null) {
        this.logger.info(`{{> ${partialName} }} - AOT partial hash resolution phase`);
      ...
      this.logger.info(`{{> ${partialName} }} - Inlining phase`);
      ...
      // Get identifier node - this is used by to add a
      // 'storeContext' block if this custom context
      ...
      recurseAst({
        ast,
        contextList: getPartialContextList.bind(this)({ stmt, inline }),
        bindParents,
      });
        ...
        if (inline && !ast.marker.inlined) {
          if (!ast.isCustomCtx) {
            visitNodes({
              ...
              consumer: ({ stmt }) => {
                let { original } = stmt;
                // 1. Replace "data." with "" if applicable
                original = original.replace(new RegExp(`^${dataPathRoot}.`), '');
                // 2. Prefix partial hash references with @
                if (stmt.parameter) {
                  original = `@${original}`;
                }
                resetPathExpression({
          ...
          const storeContextBlock = {
            ...
            hash: {
              pairs: [{
                key: partialIdHash,
                value: { type: 'StringLiteral', original: contextId },
              }, {
                key: partialIsRootHash,
                value: { type: 'BooleanLiteral', original: ast.isCustomCtx },
              }],
            },
            program: {
              type: 'Program',
          ...
          replacements.push({
            parent: ast.parent.body,
            replacementIndex: ast.parent.body.indexOf(
              ast.marker,
            ),
        ...
        const loadCtxBlock = {
          ...
          hash: {
            pairs: [
              ...
              {
                key: partialNameHash,
                value: { type: 'StringLiteral', original: partialName },
              },
            ],
          },
          program: {
            type: 'Program',
            body: inline ? [] : ast.body,
          },
        };
    ...
    ASTParser.prototype.DecoratorBlock = function (stmt) {
      ...
      this.mutating = true;
      return _this.addInlineBlock({ bindParents, stmt });
    };
  ...
  addInlineBlock({ bindParents, stmt }) {
    ...
    // the same name
    if (Object.keys(getAvailableInlineBlocks({ bindParents }))
      .filter(name => name === blockName).length) {
      this.logger.warn(`Inline block: '${blockName}' already exists: ${getLine(stmt)}, skipping`);
    }
    ...
    decorators[param.original] = {
      ...stmt.program,
      marker,
    };

    this.logger.info(`Decorator: ${param.original}, requiredParams: ${stmt.requiredParams}`);
    return marker;
  }
  ...
  static getReservedPartialHashes() {
    ...
    return [partialIdHash, partialNameHash, partialIsRootHash];
  }
