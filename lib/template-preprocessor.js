...
class TemplatePreprocessor {
  ...
  resolvePath({
    ...
    // Check if this is a user global variable
    ...
    // Check if this is a configuration field
    if (contextList.length == 1 && this.component.getConfigurationProperties().includes(stmt.original)) {
      throw Error(`${stmt.original} is reserved as a configuration property, ${getLine(stmt)}`)
    }
  ...
  process0({
    ...
    ASTParser.prototype.BlockStatement = function (stmt) {
      ...
      if (hookMethod && !_this.methodNames.includes(hookMethod.original)) {
        throw Error(`Unknown method: ${hookMethod.original} specified as ${hookHashKey}, ${getLine(stmt)}`);
      }
  ...
  getHtmlErrors0(src, fileName, messages, validator) {
    ...
    const { validateHtmlString } = TemplatePreprocessor;

    if (!validator) {
      validator = this.getHtmlValidator();
    }
  ...
  */
  getComponent({ componentSrc, instantiate = true } = {}) {
    ...
    const { addGlobals, makeRequire, clearRequireCache, evalScript } = TemplatePreprocessor;
