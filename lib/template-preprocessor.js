...
class TemplatePreprocessor {
  ...
  static minifyComponentClass = false;
  ...
  writeComponentJsToFileSystem() {
    const {
      minifyComponentClass, minifyAsset,
    } = TemplatePreprocessor;
    const distPath = this.getDistPath();

    // Write test js
    fs.writeFileSync(pathLib.join(distPath, 'index.test.js'), this.componentSrc);
    ...
    if (minifyComponentClass) {
      componentSrc = minifyAsset({ data: componentSrc, type: 'js' });

      // Add source URL
      // Todo: .info find a lasting solution to this issue
      // componentSrc += `\n//# sourceURL=/${pathLib.relative(getDistPath(), mainFile)}`;
    }

    fs.writeFileSync(pathLib.join(distPath, 'index.js'), componentSrc);
  }
  ...
  getTransientComponentsGlobal() {
    const {
      enableComponentCaching, getComponentsSrcPath, getAssetIdFromClassName, getComponentsDistPath,
      findMatchingComponent, getComponentClass, getSkipFile,
    } = TemplatePreprocessor;
  ...
  static getAnyScalarComponent() {
    ...
    if (fs.existsSync(configFile)) {
      ...
      scalarComponent = scalarComponents.length ? scalarComponents[
        utils.getRandomInt(0, scalarComponents.length - 1)
      ] : null;
    }

    if (!scalarComponent) {
      throwError(`No scalar components were defined`);
    }
    ...
    if (className === BaseComponent.name) {
      ...
      if (componentSource.isAbstract) {
        ...
        const arr = implClasses
          .filter(n => !this.metadata.componentSources[n].recursive);

        if (!arr.length) {
          // <implClasses> does not contain any non-recursive components
          // Return null, else we will end up with a circular dependency error
          return null;
        }
  ...
  process() {
    const {
      rootQualifier, dataPathRoot, reservedComponentClassNames, getLiteralType, getComponentList,
      getComponentListPath, getMethodNames, parseScript, getComponentMainClass,
    } = TemplatePreprocessor;
    ...
    try {
      ...
      const snapshotFile = pathLib.join(distPath, 'index.snapshot.js');
      const mainFile = pathLib.join(distPath, 'index.js');

      if (fs.existsSync(snapshotFile) && s.existsSync(mainFile)) {
        // Overwrite the js file, with the last stable version
        fs.writeFileSync(
          snapshotFile, fs.readFileSync(mainFile),
        );
      } else {
        // Prune component
  ...
  loadComponentSources() {
    ...
    const {
      getComponentsSrcPath, getComponentClass, getComponentParents, isAbstractComponent
    } = TemplatePreprocessor;
  ...
  static getComponentClass({
    reference, disableRequire, requirePath, data, dir, clearCache = true, useTestClass = false, lenient,
  } = {}) {
    ...
    if (isDataVariable) {
      ...
      switch (true) {
        ...
        case !getDataVariables().includes(dataVariableName):
          ...
          break;
        case index == 0:
          // At index 0, only @root is allowed
          throwError(
  ...
  static isResolvableInScope(pathInfo) {
    const { path, onRoot } = pathInfo;
    return !onRoot && path != '@root';
  }

  resolveGlobalVariable({ pathInfo, stmt }) {
    const {
      dataPathRoot, pathSeparator, globalsBasePath, wordPattern, getTargetType, throwError,
      getLine,
    } = TemplatePreprocessor;

    const { path, index } = pathInfo;
    ...
    if (first && first.match(wordPattern)) {
      const types = this.component.getGlobalVariableTypes();

      if (Object.keys(types).includes(first)) {
  ...
  static throwCannotAccessIterateContextInAsync(targetPath, stmt) {
    const { throwError } = TemplatePreprocessor;
    assert(stmt);

    throwError(
      `"${stmt.original}" cannot resolve to "${targetPath}" from inside an async block`,
      stmt,
    )
  }

  resolvePathFromContext({
    contextList, inlineBlock, pathInfo, original, validType, stmt, syntheticAlias, nameQualifier, create,
  }) {
    const {
      rootQualifier, pathSeparator, dataPathPrefixRegex, dataPathRoot, syntheticMethodPrefix, literalType,
      objectType, appendSuffix, hasDataPathFormat, hasObjectPrefix, defaultIndexResolver, getTargetType, getLine,
      getSuffix, isResolvableInScope, throwCannotAccessIterateContextInAsync
    } = TemplatePreprocessor;
        ...
        for (const k of contextKeys) {
          ...
          if (
            ...
            if (asyncContext && contextObject[rootQualifier].inIterateContext) {
              throwCannotAccessIterateContextInAsync(v.value, stmt);
            }
            ...
            if (type.endsWith('Literal')) {
              ...
              original = v.value;
              value = this.getSampleValueForType(literalType);
    ...
    if (!type) {
      ...
      if (asyncContext && contextObject[rootQualifier].inIterateContext) {
        throwCannotAccessIterateContextInAsync(rootValue, stmt);
      }
    ...
    switch (true) {
      ...
      case original == `${dataPathRoot}${pathSeparator}` || type == literalType:
        assert(value);
  ...
  resolvePath({
    ...
    const {
      inlineParameterPrefix, literalType, resetPathExpression, getOuterInlineBlock, getPathInfo,
    } = TemplatePreprocessor;

    if (!validType) {
      validType = stmt.validType || literalType;
    }
    ...
    const globalVariable = this.resolveGlobalVariable({ pathInfo, stmt: { ...stmt, validType } });
  ...
  addParameterizedExpression({
    ...
    params = [], hash = { pairs: [] }, syntheticAlias, syntheticNodeId,
  }) {
    if (!pruneKey) {
      assert(context == null);
    ...
    const {
      ...
      onSubExpressionProcessed, hasDataPathFormat, invokeDataPathTransform,
    } = TemplatePreprocessor;
    ...
    if (context) {
      ...
      if (syntheticAlias) {
        ...
        _stmt.argument = getCallExpression({
          ...
          args: [{
            ...
            loc: stmt.loc,
            canonicalSource: `${invokeMethodName}(...)`,
          }]
            .map(getValue, this),
      ...
      this.componentAst.body[0].body.body
        .push({
          ...
          // In some cases (e.g. logicGate), this method may need to be pruned, in which 
          // case <stmt.prune> will likely be true
  ...
  getHelperOptionsFromHash({
    ...
    const {
      ...
      getMethodFromFunctionDeclaration, isRootCtxValue, getLine, getValue, getDefaultHelperHash, onSubExpressionProcessed,
      invokeDataPathTransform,
    } = TemplatePreprocessor;
  ...
  createSubExpression({
    ...
    const { methodNameSuffix } = stmt;

    const methodName = syntheticAlias ? getPathFromSyntheticAlias(syntheticAlias)
  ...
  createRootProxyIndirection0({ stmt, canonicalSource, path, suffix, syntheticAlias }) {
    ...
    const lenientResolution = isConditionalParticipant(stmt);
  ...
  getBlockParam(stmt, index) {
    const { getReservedBlockParamNames } = TemplatePreprocessor;
    ...
    if (getReservedBlockParamNames().includes(qualifier)) {
      throw Error(
  ...
  getCustomBlockFuntionAsync({ methodName, stmt }) {
    const {
      ...
      getOptionsRetrievalStatements, getVariableEnvelope, getLine,
    } = TemplatePreprocessor;
    return {
      type: 'FunctionDeclaration',
      id: {
  ...
  // eslint-disable-next-line class-methods-use-this
  getCustomBlockFuntion({ methodName, stmt }) {
    const {
      validateTypeMethodName, renderBlockMethodName, getIdentifier, getCallExpression,
      getValue, getOptionsRetrievalStatements, getVariableEnvelope, getLine,
    } = TemplatePreprocessor;
    return {
      type: 'FunctionDeclaration',
      id: {
  ...
  getBlockOptions(stmt) {
    const {
      arrayType, objectType, mapType, addDefaultParamToCustomBlock, getLine, ensureNoLiteralParam0,
  ...
  static throwError(msg, stmt) {
    const { getLine, reference } = TemplatePreprocessor;
    throw Error(`[${reference} ${getLine(stmt)}] ${msg}`);
  }
  ...
  getSyntheticMethodValue({
    stmt, path, method, validType, line, rewriteErrorMsg = true
  }) {
    const { throwError } = TemplatePreprocessor;
  ...
  addDataBindTransformations() {
    ...
    const {
      customEachHelperName, stencilCssClassname, startTNBCtxHelperName, startAttrCtxHelperName, endAttrCtxHelperName,
      createContentStatement, createMustacheStatement, getLine,
    } = TemplatePreprocessor;
    ...
    const onMustacheStatement = (stmt) => {
      const { rawDataPrefix } = TemplatePreprocessor;

      const { path, params, dataBinding } = stmt;
  ...
  process0({
    ...
    const getContextId = (contextObject) => {
      return Object.entries(contextObject)
        .filter(([k, { scope }]) => scope)
        .map(([k, { contextId }]) => contextId)[0];
    }
    ...
    const acceptPathExpressionInCustomCtx = ({ stmt, visitor }) => {
      ...
      const globalVariable = this.resolveGlobalVariable({ pathInfo, stmt });
      ...
      if (hasRootPrefix) {
        if (!inlineBlock) {
          const contextId = contextList[0][rootQualifier].declaredValue;
      ...
      // Note: <forRoot> indicates that <stmt> must be resolved on the root context
    ...
    const acceptPathExpressionInRootCtx = ({ stmt }) => {
      ...
      const { toPathExpressionLiteral, invokeDataPathTransform, hasDataPathFormat } = TemplatePreprocessor;
    ...
    ASTParser.prototype.BlockStatement = function (stmt) {
      ...
      const {
        globalsBasePath, getPathFromSyntheticAlias, getChildPathFromSyntheticAlias,
        getTargetType, isLookupAllowed, throwError, addBlockParamHashKey, getIterateDataVariables,
        invokeDataPathTransform, isRootCtxValue, getReservedBlockHashKeys
      } = TemplatePreprocessor;
      ...
      if (!custom) {
        ...
        if (htmlErrors.length) {
          ...
          if (transform) {
            throwError(
              `A transform cannot be specified for this block due to invalid html markup`,
              stmt,
            );
          }
          ...
          if (conditional && (type.endsWith('Literal') || global.components[targetType])) {

            // Disable data binding if the target of this conditional block is a 
            // 1. Literal
            // 2. Component, because our RootProxy does not maintain dataPathHooks for
            // paths that resolve to a component instance 
    ...
    const processComponentImport = ({ stmt }) => {
      ...
      // processComponentImport(...) is not called within inline blocks, see PartialStatement...
      assert(!terminate);
    ...
    ASTParser.prototype.MustacheStatement = function (stmt) {
      ...
      const { loadComponentHelperName, isRootCtxValue, getReservedMustacheHashKeys } = TemplatePreprocessor;
      ...
      if (isCustomContext()) {
        ...
        if (isSubExpression || transform) {
          ...
          const { type, original } = stmt.path;

          assert(type == 'PathExpression');

          if (isRootCtxValue(original)) {
            _this.allowedPaths.push(original);
          }
        }
      ...
      const wrapInResolveMustacheHelper = (original) => {
        const { resolveMustacheHelperName, literalPrefix } = TemplatePreprocessor;

        // Note: If literalPrefix, then this is not needed
        original += '!!';
        ...
        return {
          ...
          dataBinding: !original.includes(literalPrefix)
        };
      }
      ...
      if (isSubExpression) {
        ...
        if (stmt.processed) {
          ...
          stmt = wrapInResolveMustacheHelper(original);
        } else {
          stmt.type = 'MustacheStatement';
        }
      } else {
    ...
    const getPartialContextList = function ({ stmt }) {
      ...
      if (isCustomContext()) {
        ...
        partialContextList[0][rootQualifier]
          .declaredValue = getContextId(utils.peek(contextList));
    ...
    const processPartial = function ({ stmt }) {
      ...
      const { reference } = TemplatePreprocessor;
      ...
      TemplatePreprocessor.reference = `${reference}/${partialName}`
    ...
    ASTParser.prototype.PartialStatement = function (stmt) {
      ...
      const { loadComponentHelperName, wordPattern, getLine, getReservedPartialHashKeys } = TemplatePreprocessor;
      const { componentRefType } = PathResolver;
      ...
      return ASTParser.prototype.MustacheStatement.call(this, {
        ...
        escaped: false,
        // Todo: should we pass a null hash instead?
        hash: stmt.hash,
        loc: stmt.loc,
    ...
    ASTParser.prototype.DecoratorBlock = function (stmt) {
      const {
        reservedDecoratorNames, allowOverlappingQualifiersInDecorator, isConditionalParticipant,
      } = TemplatePreprocessor;
  ...
  static getReservedBlockHashKeys() {
    const {
      blockParamHashKey, rootQualifier, getDataVariables
    } = TemplatePreprocessor;
    return [
  ...
  static getReservedPartialHashKeys() {
    const {
      partialIdHashKey, partialNameHashKey, rootQualifier, getDataVariables,
    } = TemplatePreprocessor;
    return [
    ...
    const accept = (stmt) => {
      ...
      switch (true) {
        ...
        case stmt.type == 'MustacheStatement':
          ...
          arr.push('}}');
          break;
        case stmt.type == 'BlockStatement':
          arr.push('{{#');
          accepSubExpr(stmt)
          arr.push('}}');
          stmt.program.body.forEach(accept);
          if (stmt.inverse) {
            arr.push('{{else}}');
            stmt.inverse.body.forEach(accept);
          }
          arr.push('{{/');
          accept(stmt.path);
          arr.push('}}');
