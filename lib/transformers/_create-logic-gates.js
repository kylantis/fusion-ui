...
class LogicalExprTransformer {
    ...
    static DOM_PARTICIPANT_TYPE = this.PARTICIPANT_TYPE_ALL;
    ...
    getExpression({ stmt, root = false }) {
        ...
        const assertNotOperator = (stmt) => {
            if (IsOperator(stmt)) {
                throw Error(`Illegal ${stmt.original}, ${getLine(stmt)}`);
            }
            return true;
        }
                ...
                case leftStart === undefined:
                    if (type === P_EXPR && original == '?') {
                        ...
                        const assertNotAndOr = (path) => {
                            assert(
                                ![OR, AND].includes(
                                    (typeof path.original == 'string' ? path.original : '').toUpperCase()
                                ),
                                `Illegal [OR, AND] ${path}`
                            );
                        }
                    ...
                    if (
                        ...
                        if (requireParenthesis) {
                            if (hasNot) {
                                assert(
                                    params.length == 1,
                                    `NOT operator should be enclosed in parenthesis, e.g. (not x), ${getLine(stmt)}`
                                );
                            }
                        }
                        ...
                        if (hasUnion || hasBool) {
                            assert(
                                params.length == 2,
                                `Union and boolean operators should be enclosed in parenthesis, e.g. (x eq b) or (x and b), ${getLine(stmt)}`
                            );
                        }
    ...
    static isLogicGateCondition(stmt) {
        const {  PARTICIPANT_TYPE_CONDITIONAL } = LogicalExprTransformer;
        const { participantType } = stmt;
        return participantType && participantType.includes(
            PARTICIPANT_TYPE_CONDITIONAL
        )
    }

    registerLogicGateParticipants(stmt, consumer) {

        const {
            T_EXPR, B_EXPR, P_EXPR, PARTICIPANT_TYPE_ALL, PARTICIPANT_TYPE_CONDITIONAL,
            PARTICIPANT_TYPE_TERNARY, getVisitor,
        } = LogicalExprTransformer;
                        ...
                        if (!this.preprocessor.methodNames.includes(stmt.original)) {
                            ...
                            switch (true) {
                                case stmt[conditionalKey]:
                                    addType(stmt, PARTICIPANT_TYPE_CONDITIONAL);
                                    break;

                                case stmt[ternaryKey]:
                                    addType(stmt, PARTICIPANT_TYPE_TERNARY);
                                    break;
                            }

                            addType(stmt, PARTICIPANT_TYPE_ALL);
    ...
    addLogicGate({ stmt }) {
        const {
            T_EXPR, B_EXPR, DOM_PARTICIPANT_TYPE, getVisitor
        } = LogicalExprTransformer;
        const { visitNodes } = this.preprocessor.constructor;
    ...
    addLogicGatesAndHelper() {
        ...
        const registerLogicGateParticipants = (stmt) => {
            visitNodes({
                Visitor: getVisitor(),
                types: [T_EXPR],
                ast: {
                    type: 'Program',
                    body: [
                        stmt.path, ...stmt.params, ...(stmt.hash ? [stmt.hash] : []),
                    ],
                },
                consumer: ({ stmt }) => {
                   this.registerLogicGateParticipants(stmt);
                    return false;
                }
            });
        }
        ...
        ASTParser.prototype.TernaryExpression = function (stmt) {
            ...
            if (stmt.source) {
                ret.canonicalSource = stmt.source;
            }
        ...
        ASTParser.prototype.MustacheStatement = function (stmt) {
            ...
            if (stmt.path.type === T_EXPR) {
                ...
                registerLogicGateParticipants(stmt);
            }
