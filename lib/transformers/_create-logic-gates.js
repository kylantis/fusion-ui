...
class LogicalExprTransformer {
    ...
    addLogicGate({ stmt }) {
        const { T_EXPR, B_EXPR, P_EXPR, getVisitor } = LogicalExprTransformer;
        ...
        const accept = (stmt) => {
            ...
            // as participants?
            // The above is not necessary because in the root context, sub expressions are consolidated
            // into a single synthetic function, and then blockData will be used from then on

            if (type !== T_EXPR) {
                return stmt;
            }

            const isDataPath = ({ type, original }) => type === P_EXPR &&
            ...
            if (stmt.ternaryPath) {
                ...
                const addParticipants = (stmt) => {
                    switch (stmt.type) {
                        case B_EXPR:
                            addParticipants(stmt.left);
                            addParticipants(stmt.right);
                            break;
                        case T_EXPR:
                            [...stmt.condition].forEach(
                                addParticipants
                            );
                            break;
                        default:
