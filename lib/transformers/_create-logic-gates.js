...
class LogicalExprTransformer {
    ...
    explodeOperators() {
        ...
        const { ast } = this;
        const { MUST_GRP, getVisitor } = LogicalExprTransformer;
    ...
    getExpression({ stmt, root = false }) {
        ...
        if (root) {
            expr.source = stmt.source || stringifyHandlebarsNode(stmt);
        } else if (stmt.source) {
        ...
        for (let i = 0; i < params.length; i++) {
            ...
            if (root) {
                stmt.params[i].source = stmt.source || stringifyHandlebarsNode(stmt.params[i]);
            }
            ...
            if (expr.type == T_EXPR) {
                ...
                expr.condition = expr.condition
                    .map(stmt => {
                        ...
                        const { type, path, params, isCondition, invert, loc, hash } = stmt;
