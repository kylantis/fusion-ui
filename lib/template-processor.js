...
module.exports.processFile = ({
  ...
  return new Promise((resolve) => {
    ...
    component.load({ parent: 'parent' }).then(() => {
      // Sometimes, the promise returned by the load method may resolve,
      // even when the component is not fully mounted. This is common
      // for components that contain async custom blocks

      const intervalId = setInterval(() => {
        const isPending = util.inspect(component.promise).includes('pending');
        ...
        if (!isPending) {
          ...
          resolve({
            assetId,
            templateData: templatePrecompiled,
          });
        }
      }, 500);
    });
