...
module.exports.processFile = ({ dir, fromGulp = false, Preprocessor }) => {
  const fs = require('fs');
  ...
  if (!componentName.match(/^[a-zA-Z]+\w+$/g)) {
    throw new Error(`Component name: ${componentName} must be a valid word`);
  }
