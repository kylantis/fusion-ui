...
module.exports.processFile = ({
  ...
  // const abstractComponentAssetId = 'abstract_component';

  // // Ensure that the "abstract-component" has been compiled
  // if (assetId != abstractComponentAssetId && 
  //   !fs.existsSync(path.join(process.env.pwd, 'dist', 'components', abstractComponentAssetId), 'utf8')) {
  //   throw Error(`You need to compile the abstract component first`);
  // }
  ...
  return Promise.all(
    (processor.preprocessors || []).map(p => p.createModels())
  ).then(() => Promise.all([
