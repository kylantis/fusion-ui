...
const TemplatePreprocessor = require('../../template-preprocessor');
...
const { pkgName } = TemplatePreprocessor.getSrcConfig();
...
class FusionJavaRenderer extends JavaRenderer {
  ...
  emitSourceStructure() {
        ...
        switch (type) {
          ...
          case 'enum':
            ...
            if (this.preprocessor) {
              metadataObject.enum = true;
            } else {
              // this is a shared enum
            }
            break;
          case 'union':
    ...
    if (Object.keys(classesMetadata).length) {
      const { assetId } = this.preprocessor;

      const classesMetadataFile = this.getClassesMetadataFile(assetId);
      const dir = pathLib.dirname(classesMetadataFile);
  ...
  toCanonicalFieldType(type) {
    if (Array.isArray(type)) {
      return type.map(part => this.toCanonicalFieldType(part));
  ...
  emitMainClassDefinition(c, simpleName, metadataObject) {
    ...
    const enumImports = this.getEnumImports(className, c);
    metadataObject.enumImports = enumImports.map(n => this.classNamer(n));

    imports = [...new Set([
      ...imports,
      ...componentImports
        .map((n) => this.toFqComponentClassName(n)),
      ...enumImports.map(n => this.toFqEnumClassName(n)),
    ])];
    ...
    this.emitBlock([
      ...
      this.forEachClassProperty(c, 'leading-and-interposing', (name, jsonName, p) => {
        ...
        this.emitBlock([`${accessModifier} `, `${typeName} `, setterName, '(', fieldType, ` ${key})`], () => {
          this.emitLine(`super.setFieldValue(new ${fieldSetHandleClassName}<${genericFieldType}>("${key}", ${key}));`);
          this.emitLine('return this;');
        });

        const canonicalFieldType = this.toCanonicalFieldType(fieldType);

        // metadataObject.getters[this.sourcelikeToString(getterName)] = {
        //   returnType: canonicalFieldType,
        // };

        metadataObject.setters[this.sourcelikeToString(setterName)] = {
          paramType: canonicalFieldType,
          paramName: key,
        };
      });
      ...
      if (isComponentClass) {
        ...
        // Override setter methods from parent(s)

        const componentClassNames = Object.keys(TemplatePreprocessor.fetchDistComponentList(false))
          .map(c => this.classNamer(c));

        const getImportForClassName = simpleName => {
          for (const i of imports) {
            const parts = i.split('.');
            const _simpleName = parts[parts.length - 1];

            if (_simpleName == simpleName) {
              return i;
            }
          }
          return null;
        ...
        parents.forEach(parent => {
          const classesMetadata = this.getClassesMetadata(parent);
          const { enumImports, setters } = classesMetadata[this.classNamer(parent)];

          const toParamType = segment => {
            if (Array.isArray(segment)) {
              return segment.map(s => toParamType(s)).join('');
            }

            switch (true) {
              case componentClassNames.includes(segment):
                this.addMainClassDefferedImport(this.toFqComponentClassName(segment), imports);
                break;

              case segment == 'List<':
                this.addMainClassDefferedImport(LIST_CLASS, imports);
                break;

              case segment.startsWith('Map<'):
                this.addMainClassDefferedImport(MAP_CLASS, imports);
                const keyType = segment.substring(segment.indexOf('<') + 1, segment.lastIndexOf(', '))
                return `Map<${toParamType(keyType)}, `;

              case segment == '>' || this.getJavaPrimitiveTypes().includes(segment):
                break;

              default:
                const primitiveFqClassName = this.getPrimitiveFqClassName(segment);

                if (primitiveFqClassName != null) {
                  this.addMainClassDefferedImport(primitiveFqClassName, imports);

                } else if (enumImports.includes(segment)) {
                  const i = getImportForClassName(segment);
                  const fqClassName = this.toFqEnumClassName(segment);

                  if (fqClassName != i) {
                    if (i == null) {
                      this.addMainClassDefferedImport(fqClassName, imports)
                    } else {
                      return fqClassName;
                    }
                  }

                } else if (classesMetadata[segment]) {
                  const i = getImportForClassName(segment);
                  const fqClassName = `${this.getComponentPackageName(parent)}.${segment}`;
                  ...
                  throw Error(`Unknown class '${segment}' in parent component '${parent}'`);
                }
            }
          ...
          Object.entries(setters)
            .forEach(([setterName, { paramType, paramName }]) => {

              this.ensureBlankLine();
              this.emitLine('@Override');
              this.emitBlock(
                [`public ${typeName} ${setterName}(${toParamType(paramType)} ${paramName})`],
                () => {
                  this.emitLine(
                    `super.${setterName}(${paramName});`
                  );
                  this.emitLine('return this;');
                });
            });
    ...
    if (isComponentClass) {
      ...
      fs.writeFileSync(
        pathLib.join(
          process.cwd(), 'dist', 'components', assetId, '.mainClass',
        ),
  ...
  getJavaPrimitiveTypes() {
    return [
      'float', 'double', 'short', 'long', 'int', 'char', 'boolean', 'byte'
    ];
  }

  getClassesMetadataFile(assetId) {
    return pathLib.join(
      process.cwd(), 'dist', 'components', assetId, 'model_factory_metadata',
      'java', 'classesMetadata.json'
    );
  }

  getClassesMetadata(className) {
    const assetId = this.preprocessor.getAssetIdFromClassName(className);
    const metadataFile = this.getClassesMetadataFile(assetId);

    return JSON.parse(fs.readFileSync(metadataFile, 'utf8'));
  }

  getPrimitiveFqClassName(typeName) {
    switch (typeName) {
      case 'String':
  ...
  getFqClassName(typeName) {
    const primitiveFqClassName = this.getPrimitiveFqClassName(typeName);

    if (primitiveFqClassName != null) {
      return primitiveFqClassName;
    }
  ...
  addMainClassDefferedImport(fqClassName, _imports) {
    if (_imports.includes(fqClassName)) {
      return;
    }

    _imports.push(fqClassName);
  ...
  emitEventSignature(eventName, _imports) {
    ...
    imports
      .forEach(i => {
        this.addMainClassDefferedImport(i, _imports);
      })
