...
class JacksonJavaRenderer extends JavaRenderer {
  ...
  emitClassDefinition(c, simpleName) {
    ...
    const { behaviours: behavioursFn, events: eventsFn } = component;

    const behaviours = behavioursFn() || [];
    ...
    this.emitBlock([
      ...
      isComponentClass ? ` extends ${parent == null ? baseComponentClassName : classNamer(parent.name)}` : ''
    ], () => {
