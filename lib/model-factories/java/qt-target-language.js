...
class FusionJavaRenderer extends JavaRenderer {
  ...
  emitRpcClass() {
    const { component, className } = this.preprocessor;
    ...
    this.ensureBlankLine();
    this.emitBlock(['public class ', rpcClassName], () => {
      ...
      component.getBehaviours()
        .forEach(behaviourName => {
          ...
          this.ensureBlankLine();
          this.emitBehaviourMethod(behaviourName);
        });
    });
  ...
  emitBehaviourMethod(behaviourName) {
    ...
    this.emitBlock(
      ...
      () => {
        this.emitLine(
          `context.invokeBehaviour("${behaviourName}"${parameterNames ? `, ${parameterNames}` : ''});`
        );
      });
  }
