...
class FusionJavaRenderer extends JavaRenderer {
  ...
  getPaths(c) {
    ...
    this.forEachClassProperty(c, 'none', (name, jsonName, p) => {
      const path = this.getPath0(this.descriptionForClassProperty(c, jsonName));
      paths[path] = this.javaType(false, p.type);
    });
    return paths;
  }

  getPath0(description) {
    let { path } = JSON.parse(
      this.sourcelikeToString(description)
    );
    return typeof path == 'string' ? path :
      // Path at index 0 represents the sharedType path, which is what we need for the purpose
      // of looking up config params
      path[0];
  }
  ...
  getConfig(path) {
    return this.getCollectionConfig(path) || this.getScalarConfig(path) || {};
  }

  isFieldEmittable(path) {
    const { clientOnly } = this.getConfig(path);
    return !clientOnly;
  }
  ...
  getFieldAccessModifier(path) {
    ...
    let { visibility = 'public' } = this.getConfig(path);
  ...
  emitMainClassDefinition(c, simpleName) {
    ...
    this.emitBlock([
      ...
      this.forEachClassProperty(c, 'none', (name, jsonName, p) => {
        ...
        const path = this.getPath0(description);
        const isEmittable = this.isFieldEmittable(path);
        ...
        this.emitLine('private ', this.getFieldType(description, p), ' ', name, `${initialValue};`);
      });
  ...
  emitEnumDefinition(e, enumName) {
    ...
    const description = this.descriptionForType(e);
