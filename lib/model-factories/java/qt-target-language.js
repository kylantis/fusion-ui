...
class FusionJavaRenderer extends JavaRenderer {
  ...
  emitBehaviourMethod(_imports, behaviourName) {
    ...
    addToImports(`${basePkg}.${behaviorInvokeHandleClassName}`);
  ...
  emitEventMethod(typeName, isAbstract, _imports, eventName, finalizers, base) {
    ...
    const emitListenerMethod = (listenerClass, consumerGetter) => {
      if (!clientOnly) {
        addToImports(`${basePkg}.${eventListenerHandleClassName}`);
        addToImports(listenerClass ? 'java.util.function.Consumer' : `${basePkg}.${functionClassName}`);
    ...
    const emitDispatchMethod = (parametersString, parameterNames) => {
      if (!clientOnly && allowServerDispatch) {
        addToImports(`${basePkg}.${eventDispatchHandleClassName}`);
