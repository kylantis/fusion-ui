...
const remoteDataNodeClassName = 'RemoteDataNode';

const pkgName = `${basePkg}.components.${APP_ID}`;

const noOpClassName = generateRandomString(6);
const remoteDataNodeClassNamePrefix = 'Remote';
class FusionJavaRenderer extends JavaRenderer {
  ...
  emitClassDefinitions(c, n) {
    ...
    if (!isAbstract && !isEnum) {
      ...
      // Write the respective RemoteDataNode class
      this.emitRemoteDataNodeClassDefinition(c, n);
    }
  }
  ...
  emitMainClassDefinition(c, simpleName) {
      ...
      const classImport = () => {
        switch (true) {
          ...
          case isMapType(fieldType):
            // HashMap is already imported by default
            return [];
    ...
    if (isComponentClass) {
      ...
      if (ownEvents.length || baseEvents.length) {
    ...
    this.emitBlock([
      ...
      isComponentClass ? '' : ` implements ${dataNodeClassName}`,
    ], () => {
      ...
      if (isComponentClass) {
        ...
        baseEvents.forEach(eventName => {
          this.emitEventMethod(typeName, imports, eventName, null, true);
        });

        ownEvents.forEach(eventName => {
          this.emitEventMethod(typeName, imports, eventName, finalizers);
        });
      ...
      this.ensureBlankLine();
      // this.emitLine('@Override');
      this.emitBlock(['public Map<String, List<Object>> getPropertyMap()'], () => {

        this.emitLine('var m = new HashMap<String, List<Object>>();');
        this.ensureBlankLine();
        ...
        this.forEachClassProperty(c, 'none', (name, jsonName, p) => {
          ...
          this.emitLine(`m.put("`, name, `", Arrays.asList("${path}", this.`, name, `));`);
        });

        this.ensureBlankLine();
        this.emitLine('return m;');
      });
  ...
  emitRemoteDataNodeClassDefinition(c, simpleName) {

    const className = Array.from(simpleName._unstyledNames)[0];

    const parentName = this.sourcelikeToString(simpleName);
    const typeName = `${remoteDataNodeClassNamePrefix}${parentName}`;
    ...
    imports.push(`${basePkg}.${remoteDataNodeClassName}`);
    ...
    this.emitBlock([
      'public', ' class ', typeName, ' extends ', parentName, ' implements ', remoteDataNodeClassName
    ], () => {
      ...
      this.emitBlock(['public String getClassName(String type)'], () => {
        this.emitLine(`return "${remoteDataNodeClassNamePrefix}" + type;`);
      });
  ...
  emitEventMethod(typeName, _imports, eventName, finalizers, base) {
    ...
    const emitListenerMethods = (paramTypeString, consumerGetter) => {
      this.ensureBlankLine();
      this.emitBlock([`public ${typeName} on${eventNameAlias}(${paramTypeString ? paramTypeString : 'Consumer<Object[]>'} listener)`], () => {
  
        this.emitLine(`super.on("${eventName}", ${consumerGetter ? `${consumerGetter}(listener)` : 'listener'});`);
        this.emitLine(`return this;`);
      });
  
  
      this.ensureBlankLine();
      this.emitBlock([`public ${typeName} on${eventNameAlias}Once(${paramTypeString ? paramTypeString : 'Consumer<Object[]>'} listener)`], () => {
  
        this.emitLine(`super.once("${eventName}", ${consumerGetter ? `${consumerGetter}(listener)` : 'listener'});`);
        this.emitLine(`return this;`);
      });
    }
    ...
    emitListenerMethods(eventClassName, getConsumerMethodName);
