...
class FusionJavaRenderer extends JavaRenderer {
  ...
  emitSourceStructure() {
    const { getClassesMetadataFile } = FusionJavaRenderer;
    ...
    const classesMetadataFile = getClassesMetadataFile(assetId);
  ...
  static getClassesMetadataFile(assetId) {
    return pathLib.join(
  ...
  emitMainClassDefinition(c, simpleName, metadataObject) {
    ...
    this.emitBlock([
      ...
      if (!hasGenericSuperclass) {
        // Override setters in parent, change returnType to this component class
