...
class FusionJavaRenderer extends JavaRenderer {
  ...
  emitMainClassDefinition(c, simpleName) {
    ...
    const enumImports = [...new Set(this.enumTypes[className] || [])]
    ...
    this.emitBlock([
      ...
      this.forEachClassProperty(c, 'leading-and-interposing', (name, jsonName, p) => {
        ...
        const fieldType = this.javaType(false, p.type);
        ...
        if (Array.isArray(fieldType)) {
          ...
          let isMap = fieldType[0] == 'Map<String, ';
          ...
          switch (true) {
            ...
            case isMap:
              this.emitBlock(['public ', `${typeName} `, `add${singularClause}`, '(String name, ', singularType, ' value)'], () => {
                this.emitLine('this.', getterName, '().put(name, value);');
                this.emitLine('return this;');
              });

              this.emitBlock(['public ', `${typeName} `, `remove${singularClause}`, '(String name)'], () => {
  ...
  emitDataNodeClassDefinition(c, simpleName) {
    ...
    this.emitBlock([
      ...
      if (isComponentClass) {
        ...
        this.emitBlock(['public ', typeName, '(String path)'], () => {
          this.emitLine(`assert path != null;`);
      ...
      this.forEachClassProperty(c, 'leading-and-interposing', (name, jsonName, p) => {
        ...
        let fieldTypeName = this.sourcelikeToString(this.javaType(false, p.type));
        ...
        this.emitLine('@Override');
        this.emitBlock([`public ${fieldTypeName} `, getterName, '()'], () => {

          switch (true) {
            case this.getEnumClasses().includes(fieldTypeName):
              this.emitLine(`${fieldTypeName} value = ${fieldTypeName}.forValue((String) this.getProperty("`, name, `", "String"));`);
              break;

            default:
              this.emitLine(`@SuppressWarnings("unchecked")`);
              this.emitLine(`${fieldTypeName} value = (${fieldTypeName}) this.getProperty("`, name, `", "${fieldTypeName}");`);
              break;
          }
        ...
        this.emitLine('@Override');
        this.emitBlock([`public ${typeName} `, setterName, `(${fieldTypeName} value)`], () => {
