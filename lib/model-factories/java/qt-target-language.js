...
class FusionJavaRenderer extends JavaRenderer {
  ...
  getFieldType(description, p) {
    ...
    visitCollectionGenericTypes(
      this.getPathFromPropertyDescription(description),
      typeName,
      (p, t) => {
        if (isMapType(t)) {
  ...
  emitMainClassDefinition(c, simpleName) {
    ...
    const iterableClass = 'java.lang.Iterable';
    ...
    if (isComponentClass) {
      ...
      this.emitLine(defferedImportsMarker);
      ...
      parameterEntries.forEach(([k, v], i) => {
        ...
        if (v && v.type) {
          ...
          if (typeof type == 'string') {
            switch (type) {
              case 'string':
                parametersSignature[k] = 'String';
                break;
              case 'number':
                parametersSignature[k] = 'Double';
                break;
              case 'boolean':
