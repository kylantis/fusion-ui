...
class ModelFactory {
  ...
  static getBaseComponentClass() {
    return `
    package ${basePkg};

    import java.util.function.Consumer;

    public abstract class ${baseComponentClassName} implements ${dataNodeClassName} {
  ...
  static getRemoteDataNodeInterface() {
    ...
    public interface ${remoteDataNodeClassName} extends ${dataNodeClassName} {
      String getSessionId();
      String getPath();
  ...
  static getDataNodeInterface() {
    ...
    public interface ${dataNodeClassName} {
      String getAssetId();
      Map<String, List<Object>> getPropertyMap();
    }`;
  }
  ...
  static getRemoteDataNodeSrcFile() {
    const { getRemoteDataNodeInterface, getSharedClassesDir } = ModelFactory;
    const baseDir = getSharedClassesDir(basePkg);

    const file = pathLib.join(baseDir, `${remoteDataNodeClassName}.java`);
    if (!fs.existsSync(file)) {
      fs.writeFileSync(
        file,
        getRemoteDataNodeInterface()
      );
    }
    return file;
  }
  ...
  static getDataNodeSrcFile() {
    const { getDataNodeInterface, getSharedClassesDir } = ModelFactory;
    const baseDir = getSharedClassesDir(basePkg);
  ...
  static getStubClasses() {
    const { 
      getBaseComponentSrcFile, getRemoteDataNodeSrcFile, getDataNodeSrcFile,
    } = ModelFactory;
    return [
      getBaseComponentSrcFile(),
      getRemoteDataNodeSrcFile(),
      getDataNodeSrcFile(),
    ]
  }
