...
class JacksonJavaRenderer extends JavaRenderer {
  ...
  emitClassDefinition(c, className) {
    ...
    const importedTypes = (this.componentTypes[typeName] || []);

    if (isComponentClass ||
      importedTypes.includes(baseComponentClassName)
    ) {
      imports.push(`${baseComponentPkg}.${baseComponentClassName}`);
    }

    // Add imports for componentTypes
    imports = [
      ...imports,
      ...importedTypes
    ...
    this.emitBlock(['public', ' class ', className, isComponentClass ? ` extends ${baseComponentClassName}` : ''], () => {
      ...
      if (!this._options.lombok) {
        this.forEachClassProperty(c, 'leading-and-interposing', (name, jsonName, p) => {
          ...
          // Todo: Getter for boolean attribute should be is..., not get...
