...
class PathResolver {
  ...
  resolve({ path, create = true }) {
    ...
    const {
      ...
      typeProperty, minLengthConfigProperty, toCanonical, getRandomWord,
      getValue
    } = PathResolver;
  ...
  getWrappedValue(wrapper) {
    const {
      ...
      defaultLiteralType, getRandomWord,
    } = PathResolver;
    ...
    const { [minLengthConfigProperty]: minLength, defaultKeys = [] } = this.config.collections[path] || {};
    const scalarConfig = this.config.scalars[path];
