...
class PathResolver {
  ...
  resolvePathFromParent(canonicalPath) {
    ...
    const { metadata, className, constructor: { getConfig } } = this.preprocessor;
    ...
    const parentConfig = getConfig(parent.name);
    ...
    switch (true) {
      ...
      default:
        if ([...new Set(keys.map(toRootProperty))].includes(toRootProperty(canonicalPath))) {
          throw Error(`Unknown path: ${canonicalPath}`);
        }
    }
  ...
  resolve({ path, create = true }) {
    ...
    const {
      ...
      typeProperty, minLengthConfigProperty, keyTypeConfigProperty, toCanonical,
      getRandomWord, getValue
    } = PathResolver;
    ...
    let { [typeProperty]: existingType } = this.resolvePathFromParent(canonicalPath) || {};

    if (existingType) {
      // This path exists on the parent

      if (type != existingType) {
        throw Error(`${canonicalPath} is a ${existingType} and cannot be resolevd as a ${type}`);
      }

      return getValue({ type: existingType });
    }
    ...
    if (
      ...
      if (
        ...
        if (config[keyTypeConfigProperty] === undefined) {
          config[keyTypeConfigProperty] = 'String';
        }
  ...
  getMapKeys(path, keyType, minLength, defaultKeys) {
    ...
    const getAnyValue = () => {
      switch (keyType.toLowerCase()) {
        ...
        default:
          return enumValues[utils.getRandomInt(0, enumValues.length - 1)];
      }
    };
    ...
    keys.forEach(k => {
      if (!isValidValue(k)) {
        throw Error(`Invalid key '${k}' was specified for map: ${path}`);
      }
    });
  ...
  getLiteralValue({ path }) {
    const { typeConfigProperty, toNumber } = PathResolver;

    const scalarConfig = this.config.scalars[path];
    ...
    if (type && type.constructor.name == 'Object') {
      ...
      type = this.config.scalars[type.$ref][typeConfigProperty];
    }
  ...
  getWrappedValue(wrapper) {
    ...
    switch (wrapper[typeProperty]) {
      ...
      case componentRefType:
        ...
        let { [typeConfigProperty]: type, defaults, isNull } = this.config.scalars[path];
        ...
        if (className == BaseComponent.name) {
          ...
          assert(type == className, `Expected type "${className}" not "${type}" for path: ${path}`)
          if (isNull) {
            className = null;
          }
        }
      ...
      default:
        ...
        if (path) {
          ...
          return this.getLiteralValue({ path });
