...
class PathResolver {
  ...
  pruneConfig() {
    ...
    const { componentRefType, literalType, typeProperty } = PathResolver;
  ...
  unwrapObject({ wrapper, root = false }) {
    ...
    keys.forEach((key) => {
      ...
      if (root && (key == globalsBasePath)) {
        throw Error(`Property: '${globalsBasePath}'' cannot be defined on the root of the data model`);
      }
