...
class PathResolver {
  ...
  getLiteralValue(path, config) {
    ...
    if (type.startsWith('String')) {
      ...
      assert(!defaults.length && !allowed.length, `[${path}]: Incorrect signature for path "${path}": ${JSON.stringify(scalarConfig)}`);
    }
    ...
    const throwUnknownValue = (t, v) => {
      throw Error(`[${path}] Unknown value "${JSON.stringify(v)}" in config.scalara. Exppected type "${t}"`);
    }
