...
class PathResolver {
  ...
  static reservedProperties = [
    ...
    'path',
    'path0',
    // See  RootProxy.getReservedObjectKeys()
    '$isMap', '$isNull', '$raw',
  ...
  resolve({ path, create = true, stmt }) {
    try {
      ...
      this.throwError(e.message, stmt)
    }
  }
  ...
  createDynamicResolver({ path, target }) {
    const {
      typeProperty, valueProperty, pathProperty, objectType, arrayType, toRealPath,
    } = PathResolver;
    ...
    const getFqPath = (path, obj, prop) => {
      ...
      if (prop.startsWith('$_') && !(obj instanceof Map)) {
        _this.throwError(`${obj.constructor.name}: "${path}" cannot be accessed like a map`);
      }
    ...
    return new Proxy(literal ? {} : target, {
      ...
      set(obj, prop, value) {
        ...
        if (!setterSchema) {
          // Todo: Create a ResolverError class, which should be used here as well as in PathResolver
          _this.throwError(`Attempting to assign unknown schema object to "${fqPath}"`);
        }
