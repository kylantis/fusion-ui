...
class PathResolver {
  ...
  throwError(msg, stmt) {
    ...
    throw new SchemaError(`[${className}] ${msg}`);
  }
  ...
  static getEmptyConfigObject() {
    ...
    return {
      ...
      loadAfterCompile: true, hookAccessRate: 3,
    }
  }
  ...
  resolve0({ path, create = true }) {
    ...
    switch (true) {
      ...
      case fqPath.startsWith(globalVariablePrefix):
        ...
        return this.preprocessor.getSampleValueForType(globalVariableType || literalType);
    }
  ...
  unwrapObject({ wrapper, root = false }) {
    const {
      valueProperty, pathProperty, typeProperty, reservedRootProperties, getReservedProperties,
    } = PathResolver;
  ...
  getWrappedValue(wrapper, config = this.config) {
    ...
    switch (wrapper[typeProperty]) {
      ...
      case componentRefType:
        ...
        return className ? this.preprocessor.getSerializedComponent(className, wrapper[nameProperty], loadable) : null;
