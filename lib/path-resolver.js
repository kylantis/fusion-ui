...
class PathResolver {
        ...
        switch (true) {
          case segment === 'size' && !!this.config.useCollectionLengthProperty:
            ...
            switch (parentWrapper[typeProperty]) {
              ...
              case objectType:
                throw Error(`${parent || "ROOT"} is an object, and cannot be accessed like a map - ${fqPath}`);
  ...
  getLiteralValue(path, config) {
    ...
    let {
      [typeConfigProperty]: type, defaultValue, defaults = [], allowed = [], min = 0, max = 100,
    } = scalarConfig;
    ...
    assert(
      type && (type.toLowerCase() == 'string') || (!defaults.length && !allowed.length),
      `[${path}]: Incorrect signature for path "${path}": ${JSON.stringify(scalarConfig)}`
    );
    ...
    switch (type.toLowerCase()) {
      ...
      case 'boolean':
        ...
        setDefaultValue({ defaults: ["undefined"] });
      ...
      case 'number':
        ...
        setDefaultValue({ defaults: arr.length ? arr : ["undefined"] });
