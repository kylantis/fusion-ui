...
class PathResolver {
  ...
  static getEmptyConfigObject() {
    ...
    return {
      ...
      clientOnlyPaths: [], printHtmlWarnings: false,
    }
  }
  ...
  getParentClassName() {
    return this.preprocessor.metadata.parents[0];
  }
  ...
  getParentConfig() {
    ...
    const { srcDir } = this.preprocessor.getNonNullConfig(this.getParentClassName());

    const path = pathLib.join(srcDir, 'config.json');
  ...
  resolvePathFromParent(canonicalPath) {
    ...
    const parentConfig = this.preprocessor.getNonNullConfig(parentClassName);
    ...
    switch (true) {
      ...
      default:
        if ([...new Set(keys.map(toRootProperty))].includes(toRootProperty(canonicalPath))) {
          throw Error(`Could not find path: ${canonicalPath} on parent: ${parentClassName.name}`);
        }
    }
    ...
    switch (wrapper[typeProperty]) {
      ...
      case componentRefType:
        ...
        if (type != componentRefType) {
          ...
          if (type != className && !global.components[className].prototype instanceof global.components[type]) {
            throw Error(
