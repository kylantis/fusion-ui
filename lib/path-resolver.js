...
class PathResolver {
  ...
  static getEmptyConfigObject() {
    return {
      interfaces: [], enableTypeMerging: false, collections: {}, scalars: {},
      nullifyRandomly: [], useArrayLengthProperty: false, escapeDynamicHtml: false
    }
  }
  ...
  loadConfig0() {
    ...
    if (fs.existsSync(path)) {
      ...
      return getEmptyConfigObject();
    };
  }

  loadConfig() {
    const config = this.loadConfig0();
  ...
  resolve({ path, create = true }) {
    ...
    const {
      ...
      typeProperty, minLengthConfigProperty, toCanonical, getRandomWord, getValue
    } = PathResolver;
    ...
    switch (true) {
      ...
      // Global variable
      case fqPath.startsWith(RootCtxRenderer.getGlobalsBasePath()):
    ...
    if (
      ...
      // transformed to other type(s) by add addValue(...) 
      this.data[fqPath][typeProperty] != literalType
    ) {
      ...
      if (
        ...
        if (config[minLengthConfigProperty] == undefined) {
          config[minLengthConfigProperty] = defaultCollectionSize;
        }
  ...
  addValue({ fqPath, type, name }) {
    const {
      ...
      arrayType, toCanonical, nameProperty, pathProperty, singularTypeConfigProperty,
      toNumber, getDefaultCollectionTypeName,
      getPathRegex, defaultCollectionSize, minLengthConfigProperty
    } = PathResolver;
    ...
    for (let i = 0; i < fqPathArr.length; i++) {
      ...
      for (let j = 0; j < segments.length; j++) {
        ...
        switch (true) {
          case segment === 'length' && !!this.config.useArrayLengthProperty:
            putValue = false;
            ...
            switch (true) {
              case parentType == literalType ||
               // A previous call to .length resulted in an empty array
                (parentType == arrayType && !parentValue.length):
  ...
  putValue({
    ...
    const {
      // eslint-disable-next-line no-unused-vars
      getWrapper, emptyString, pathProperty,
      getPathRegex,
    } = PathResolver;
    ...
    eval(`this.data["${original}"] = ${lhs}`)[pathProperty] = global.clientUtils.toCanonicalPath(original);
  }
  ...
  getReservedRootPropertyNames() {

    const { getSharedTypePrefix } = PathResolver;
    const { globalsBasePath } = this.preprocessor.constructor;

    return [globalsBasePath, getSharedTypePrefix()];
  }
  ...
  unwrapObject({ wrapper, root = false }) {
    ...
    keys.forEach((key) => {
      ...
      const rootReservedProps = this.getReservedRootPropertyNames();

      if (root && rootReservedProps.filter(prop => key.startsWith(prop)).length) {
        throw Error(`Root property: '${key}' cannot start with any of the following: ${rootReservedProps}`);
      }
  ...
  getWrappedValue(wrapper) {
    const {
      ...
      defaultLiteralType, getRandomWord, getLiteralValue,
    } = PathResolver;

    const path = wrapper[pathProperty];
    const { nullifyRandomly } = this.config;
    ...
    const { [minLengthConfigProperty]: minLength } = this.config.collections[path] || {};
    ...
    switch (wrapper[typeProperty]) {
      ...
      case componentRefType:
        ...
        if (!this.config.scalars[path]) {
          this.config.scalars[path] =
        ...
        const { [typeConfigProperty]: type, defaults, isNull } = this.config.scalars[path];
        ...
        if (className == BaseComponent.name) {
          ...
          assert(type == className, `Unknown type: ${type} for path: ${path}`)
          if (isNull) {
            className = null;
          }
        }
      ...
      case mapType:
        ...
        for (let i = 0; i < minLength; i++) {
          newObject[getRandomWord()] = mapObject[mapKeys[0]];
        }
      ...
      default:
        if (wrapper[valueProperty] === '[]') {
          ...
          // Todo: this will not work because, it is possible
          // for an each block to only contain data variable lookups
          //in which case the '[]' will be left unchanged,
