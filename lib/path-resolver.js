...
class PathResolver {
  ...
  static reservedRootProperties = [
    // There are reserved fields in the BaseComponent class
    'id', 'assetId',
    // This is prefixed by default to sharedType names
    this.getSharedTypePrefix(),
    // This is a reserved hashKey used during inline component rendering
    'ref',
    // See RootProxy.createDataPathHooksObject()
    'set',
  ];
  ...
  static getEmptyConfigObject() {
    ...
    return {
      ...
      printHtmlWarnings: false, loadAfterCompile: true,
    }
  }
  ...
  substituteEnums(config) {
    Object.entries(config.scalars).forEach(([key, value]) => {
      ...
      if (typeof value.allowed == 'string') {
        ...
        if (!value.allowed) {
          this.preprocessor.throwError(`Unknown enumName "${variableName}" specified in config.scalars["${key}"]`);
        }
  ...
  resolvePathFromParent(canonicalPath) {
    ...
    for (const parent of this.getParents()) {
      ...
      if (srcConfig.useCollectionLengthProperty && canonicalPath0.endsWith('.length')) {
      ...
      const ensureNotLengthProperty = (type) => {
        if (isLengthProperty) {
          throw Error(`"${canonicalPath0}" is a "${type}" and should not be accessed like an array (i.e. ${canonicalPath0}.length)`);
        }
      }
      ...
      switch (true) {
        ...
        default:
          if ([...new Set(keys.map(toRootProperty))].includes(toRootProperty(canonicalPath0))) {
            throw Error(`Could not find path "${canonicalPath}" on parent "${parent}"`);
          }
      }
    }
  ...
  resolve({ path, create = true, stmt }) {
    try {
      ...
      this.preprocessor.throwError(e.message, stmt)
    }
  }
  ...
  resolve0({ path, create = true }) {
    ...
    switch (true) {
      ...
      // Data variable
      case utils.peek(fqPathArr).startsWith('@'):
    ...
    if (resolvedFromParent) {
      ...
      if (![
        ...
        throw Error(`${canonicalPath} is a ${type0} and cannot be resolved as a ${type}`);
      }
      ...
      wrapper[valueProperty] = (() => {
        switch (type0) {
          ...
          default:
            throw Error(`Unknown type: ${type0}`);
        }
      })();
      ...
      if ([arrayType, mapType].includes(type)) {
        ...
        if (value !== Object(value)) {
          ...
          if (wrapper[typeProperty] != type) {
            throw Error(`${canonicalPath} is of type "${wrapper[typeProperty]}" and cannot be resolved as type "${type}"`);
          }
  ...
  addValue({ fqPath, type, name }) {
    const {
      ...
      toCanonical, toNumber, getSingularTypeName0, getPathRegex,
    } = PathResolver;
    ...
    for (let i = 0; i < fqPathArr.length; i++) {
      ...
      for (let j = 0; j < segments.length; j++) {
        ...
        if (existingWrapper &&
          ...
          switch (true) {
            ...
            case existingValue.constructor.name !== newValue.constructor.name
              ...
              // for similar paths
              throw Error(`${original} is an ${existingValue.constructor.name} not ${newValue.constructor.name}`);
        ...
        const ensureLastSegment = () => {
          if (original != fqPath) {
            throw Error(`Unknown Path "${original}"`);
          }
        }

        const ensureLiteralType = () => {
          if (type != literalType) {
            throw Error(`Expected "${original}" to have a targetType of "${literalType}" instead of ${type}`);
          }
        }
        ...
        switch (true) {
          case segment === 'size' && !!this.config.useCollectionLengthProperty && fqPathArr.length >= 2:
            ensureLiteralType();
            ensureLastSegment();
            ...
            switch (parentWrapper[typeProperty]) {
              ...
              case objectType:
                throw Error(`${parent || "ROOT"} is an object, and cannot be accessed like a map`);

              case mapType:
                break;

              case arrayType:
                throw Error(`${parent} is an array, and cannot be accessed like an object`);
            }
            break;

          case segment === 'length' && !!this.config.useCollectionLengthProperty && fqPathArr.length >= 2:
            ensureLiteralType();
            ensureLastSegment();
          ...
          case !!segment.match(hasIndex):
            ...
            switch (true) {
              ...
              case parentType == objectType:
                throw Error(`${parent} is an object, and cannot be accessed like an array`);

              case parentType == mapType:
                throw Error(`${parent} is a map, and cannot be accessed like an array`);
              ...
              case parentType == arrayType:
                ...
                if (newArray) {
                  ...
                  switch (existingArray[0][typeProperty]) {
                    ...
                    case objectType:
                      throw Error(`${parent} is an array of objects, and cannot be accessed like an array of literals`);
                    case mapType:
                      throw Error(`${parent} is an array of maps, and cannot be accessed like an array of literals`);
                  }
                } else {
            ...
            switch (parentWrapper[typeProperty]) {
              ...
              case mapType:
                ...
                if (parentWrapper[valueProperty][k] === undefined) {
                  throw Error(`Unknown property: ${k}. ${parent} is a map, and cannot be accessed like an object`);
                }
  ...
  static getSingularTypeName(wrapper) {
    const {
      typeProperty, pathProperty, valueProperty, objectType, arrayType, mapType, mapDefaultKey,
    ...
    switch (wrapper[typeProperty]) {
      case arrayType:
        childWrapper = wrapper[valueProperty][0];
        break;
      case mapType:
        childWrapper = wrapper[valueProperty][mapDefaultKey];
        break;
    }

    if (childWrapper[typeProperty] != objectType) {
      return undefined;
    }

    return getSingularTypeName0(wrapper[pathProperty]);
  }

  static getCollectionPath(canonicalPath) {
    const rgx = /[(\.\$_)|(_\$)]+$/g;
    return canonicalPath.replace(rgx, '');
  }

  static getSingularTypeName0(canonicalPath) {
    const { getAlias, getCollectionPath } = PathResolver;

    const typeName = utils.peek(getCollectionPath(canonicalPath).split('.'));
  ...
  unwrapObject({ wrapper, root = false }) {
    const {
      valueProperty, pathProperty, typeProperty, reservedProperties, reservedRootProperties,
    } = PathResolver;
  ...
  getMapKeys(path, collectionConfig) {
    const {
      minLengthConfigProperty, keyTypeConfigProperty, getRandomWord
    } = PathResolver;
  ...
  getLiteralValue(path, config) {
    const { typeConfigProperty, getRandomWord } = PathResolver;

    const scalarConfig = config.scalars[path];
    const hasDefault = !!scalarConfig.defaults;

    let {
      [typeConfigProperty]: type, defaultValue, defaults = [], allowed = [], min, max,
    } = scalarConfig;
    ...
    if (!type) {
      throw Error(`[${path}] Scalar config must contain a type`);
    }
    ...
    const throwUnknownValue = (t, v) => {
      throw Error(`[${path}] Unknown value ${JSON.stringify(v)} in config.scalars. Expected type "${t}"`);
    }
  ...
  isClientOnly(path, config = this.config) {
    const { scalars, clientOnlyPaths = [] } = config;
    const scalarConfig = scalars[path] || {};
    return scalarConfig.clientOnly || clientOnlyPaths.includes(path);
  }
  ...
  getWrappedValue(wrapper, config = this.config) {
    const {
      ...
      defaultLiteralType, getRandomWord, getCollectionConfig0,
    } = PathResolver;
    ...
    if (!this.processing) {
      switch (true) {
        case nullifyRandomly && nullifyRandomly.includes(path):
          ...
          break;
        case (nullify && nullify.includes(path)) || this.isClientOnly(path):
          return null;
      }
    }

    const scalarConfig = config.scalars[path];
    const collectionConfig = getCollectionConfig0(config, path);
    ...
    switch (wrapper[typeProperty]) {
      ...
      case componentRefType:
        ...
        if (defaults && defaults.length) {
          ...
          if (className && !this.getComponentClass(className)) {
            throw Error(`Unknown component: '${className}' specified in config.scalars.${path}.defaults`);
          }
        }
        ...
        if (type != componentRefType) {
          ...
          if (!this.getComponentClass(type)) {
            throw Error(`Unknown type "${type}" specified in config.scalars.${path}.${typeConfigProperty}`);
          }
        ...
        if (loadable !== undefined && typeof loadable != 'boolean') {
          throw Error(`Unknown value "${loadable}" specified in config.scalars.${path}.loadable`);
        }
        ...
        return className ? this.preprocessor.getSerializedComponent(className, loadable) : null;
      ...
      default:
        ...
        if (path) {
          ...
          const v = this.getLiteralValue(path, config);
