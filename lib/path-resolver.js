...
class PathResolver {
      ...
      if ([arrayType, mapType].includes(type)) {
        ...
        if (value !== Object(value)) {
          ...
          if (!wrapper[nameProperty]) {
            ...
            wrapper[nameProperty] = scopeQualifier;
  ...
  addValue({ fqPath, type, name }) {
    const {
      ...
      arrayType, toCanonical, nameProperty, singularTypeConfigProperty,
      toNumber, getDefaultCollectionTypeName, mapDefaultKey,
      getPathRegex,
    } = PathResolver;
  ...
  getMapKeys(path, keyType, minLength, defaultKeys) {
    ...
    let enumValuesPool = enumValues.filter(k => !keys.includes(k));
    ...
    const getAnyValue = () => {
      switch (keyType.toLowerCase()) {
        ...
        default:
          assert(enumValueIndex <= enumValuesPool.length);
    ...
    switch (wrapper[typeProperty]) {
      ...
      case componentRefType:
        ...
        if (className == BaseComponent.name) {
          ...
          assert(type == className, `Expected type "${className}" not "${type}" for path: ${path}. To fix, update config.scalars.${path} to "${className}"`)
          if (isNull) {
            className = null;
          }
        }
