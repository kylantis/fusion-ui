...
class PathResolver {
  ...
  loadConfig() {
    ...
    let config = {
      superclass: null, enableTypeMerging: false,
      collections: {}, scalars: {}, nullifyRandomly: []
    };
  ...
  pruneConfig() {
    ...
    const {
      componentRefType, literalType, typeProperty, getSharedTypePrefix
    } = PathResolver;
    ...
    for (const k in this.config.collections) {
      if ({}.hasOwnProperty.call(this.config.collections, k)) {
        ...
        if (
          // Shared types are managed by SchemaGenerator
          !k.startsWith(getSharedTypePrefix())
        ) {
    ...
    const scalarTypes = [componentRefType, literalType];

    const scalarKeys = new Set([
      ...Object.keys(this.data)
        .filter(k => !k.startsWith('@'))
        .filter(k => scalarTypes.includes(this.data[k][typeProperty]))
        .map(k => global.clientUtils.toCanonicalPath(k))
    ]);

    for (const k in this.config.scalars) {
      if ({}.hasOwnProperty.call(this.config.scalars, k)) {
        if (!scalarKeys.has(k) &&
          // Shared types are managed by SchemaGenerator
          !k.startsWith(getSharedTypePrefix())
        ) {
          delete this.config.scalars[k];
        }
      }
    }
  }
  ...
  resolve({ path, create = true }) {
    ...
    const {
      ...
      typeProperty, toCanonical, getRandomWord, getValue
    } = PathResolver;
  ...
  putValue({
    ...
    // this.preprocessor.logger.info(`^${getPathRegex(parent)}`);
    // this.preprocessor.logger.info(original);
  ...
  static getLiteralValue({ path, config }) {
    ...
    const hasDefault = !!config.defaults;
  ...
  getWrappedValue(wrapper) {
    ...
    switch (wrapper[typeProperty]) {
      ...
      case componentRefType:
        ...
        const { type, defaults, isNull } = this.config.scalars[path];
      ...
      default:
        ...
        if (path) {
          ...
          return getLiteralValue({ path, config: this.config.scalars[path] });
