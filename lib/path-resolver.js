...
class PathResolver {
  ...
  static reservedRootProperties = ['id', 'assetId'];
  ...
  static getEmptyConfigObject() {
    return {
      ...
      useArrayLengthProperty: false, escapeDynamicHtml: false,
  ...
  unwrapObject({ wrapper, root = false }) {
    ...
    keys.forEach((key) => {
      ...
      const rootReservedProps = this.getReservedRootPropertyPrefxes();

      if (root) {
        if (rootReservedProps.filter(prop => key.startsWith(prop)).length) {
          throw Error(`Root property: '${key}' cannot start with any of the following: ${rootReservedProps}`);
        }
        if ([...reservedProperties, ...reservedRootProperties].includes(key)) {
  ...
  getWrappedValue(wrapper) {
    ...
    const { nullifyRandomly } = this.config.sample || {};
      ...
      case mapType:
        ...
        if (!this.processing) {
          ...
          for (let i = 0; i < minLength; i++) {
            ...
            mapObject[getRandomWord()] = utils.deepClone(
              mapObject[mapKeys[0]]
            );
          }
